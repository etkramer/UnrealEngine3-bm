...
TCHAR FScriptCompiler::GetLeadingChar()
{
	// Skip blanks.
	TCHAR c;
	Skip1: do c=GetChar(); while( c==TEXT(' ') || c==TEXT('\t') || c==TEXT('\r') || c==TEXT('\n') );
...
INT FScriptCompiler::EmitOuterContext()
{
	...
	if( !OuterProp )
	{
		for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(UObject::StaticClass()); It; ++It )
		{
	...
	VariableSizeType bSize=0;		// byte at this fixup address must be patched with actual size afterward.
	Writer << bSize;

	INT FixupPos = TopNode->Script.Num() - sizeof(VariableSizeType);
...
// 			else
// 			{
// 				ScriptErrorf(SCEL_Expression, TEXT("Illegal interface constant"));
// 			}
// 	
// 			break;
// 		}
...
{
	VariableSizeType B = (Size & MAX_VARIABLE_SIZE);
	if( B != Size )
	{
		ScriptErrorf(SCEL_Limit,  TEXT("%s: Variable is too large (%i bytes, %i max)"), Tag, Size, (VariableSizeType)MAX_VARIABLE_SIZE );
	}
	Writer << B;
...
UBOOL FScriptCompiler::GetToken( FToken& Token, const FPropertyBase* Hint/*=NULL*/, UBOOL NoConsts/*=FALSE*/ )
{
	...
	if( (c>='A' && c<='Z') || (c>='a' && c<='z') || (c=='_') )
	{
		...
		if ( !NoConsts )
		{
			...
			else if (Token.TokenName == NAME_NameOf)
			{
				...
				if (TypeToken.TokenType != TOKEN_Identifier)
				{
			...
			if( PeekSymbol(TEXT("'")) )
			{
					...
					&&	MatchSymbol(TEXT("'")) )
				{
					...
					if( !MatchSymbol(TEXT("'")) )
					{
						ScriptErrorf(SCEL_Formatting,  TEXT("Missing single quote after %s name"), *Type->GetName() );
					}
					CheckInScope( Ob );
	...
	else if ( !NoConsts && ((c>='0' && c<='9') || ((c=='+' || c=='-') && (p>='0' && p<='9'))) )
	{
		...
		} while( (c>=TEXT('0') && c<=TEXT('9')) || c==TEXT('.') || c==TEXT('X') || c == TEXT('x') || (c>=TEXT('A') && c<=TEXT('F')) );
...
UClass* FScriptCompiler::GetQualifiedClass( const TCHAR* Thing )
{
	...
	if( ClassName[0] )
	{
		Result = FindClass(ClassName);//FindObject<UClass>( ANY_PACKAGE, ClassName );
		if( !Result )
		{
...
UBOOL FScriptCompiler::HasMemberProperties( const UStruct* Scope )
{
	...
	checkSlow(Scope);
	TFieldIterator<UProperty,CASTCLASS_UProperty,FALSE> It(Scope);
	...
	do
	{
		...
		else if ( Token.Matches( NAME_Var ) )
		{
			...
			if( MatchSymbol(TEXT("(")) )
			{
				...
				{
					EdCategory = FName( Category.Identifier );
				}
				else
				{
...
)
{
	...
	&&	Cast<UFunction>(Field) && Cast<UFunction>(Field)->HasAnyFunctionFlags(FUNC_Singular))
	{
	...
	// unless we're compiling the right side of an assigment
	...
	if ( Field != NULL && Field->GetClass() == UFunction::StaticClass() && (((UFunction*)Field)->FunctionFlags&FUNC_Delegate) &&
	...
	else if (Field && Cast<UProperty>(Field, CLASS_IsAUProperty))
	{
		...
		{
			ScriptWarnf(SCWL_Level3, TEXT("Reference to deprecated property '%s'"), *Property->GetName());
		}
		...
		if( OuterSizeFixup )
		{
			TopNode->Script(OuterSizeFixup) = (VariableSizeType)ResultType.GetSize();
		}
	...
	else if ( RequiredType.Type == CPT_Delegate )
	{
		if ( Field && Field->GetClass()==UFunction::StaticClass() )
		{
			...
			{
				ScriptErrorf(SCEL_Restricted,  TEXT("'%s' mismatches delegate '%s'"), *Function->FriendlyName.ToString(), *RequiredType.Function->FriendlyName.ToString() );
			}

			// Check all individual parameters.
			INT Count=0;
			for( TFieldIterator<UProperty,CASTCLASS_UProperty> DestinationParameter(RequiredType.Function), SourceParameter(Function); 
		...
		else
		{
			if ( Field == NULL )
			{
				...
				{
					ScriptErrorf(SCEL_Unknown,  TEXT("Delegate assignment failed (%s): Invalid function '%s'"), *RequiredType.Function->FriendlyName.ToString(), ResultType.Identifier );
				}
	...
	else if( Field && Field->GetClass()==UFunction::StaticClass() )
	{
		// Function.
		...
		if ( MatchSymbol(TEXT("(")) )
		{
			...
			{
				ScriptErrorf(SCEL_Restricted,  TEXT("Can't access private function '%s' in '%s'"), *Function->GetName(), *Function->GetOwnerClass()->GetName() );
			}
			if(	Function->FunctionFlags & FUNC_Protected )
			{
				...
				if( !Class->IsChildOf( Function->GetOwnerClass() ) )														// if we are not a subclass of the function's class.
				{
					if( Function->GetOwnerClass()->IsChildOf( Class ) )														// if we are a superclass of the function's class.
					{
						...
						{
							ScriptErrorf(SCEL_Restricted,  TEXT("Can't access protected function '%s' in '%s' (not in scope)"), *Function->GetName(), *Function->GetOwnerClass()->GetName() );
						}
						...
						{
							ScriptErrorf(SCEL_Restricted,  TEXT("Can't access private function '%s' in '%s' (%s::%s is not in scope)"), *ScopedFunction->GetName(), *ScopedFunction->GetOwnerClass()->GetName(), *Function->GetOwnerClass()->GetName(), *Function->GetName() );
						}
					}
					else
					{
						ScriptErrorf(SCEL_Restricted,  TEXT("Can't access protected function '%s' in '%s'"), *Function->GetName(), *Function->GetOwnerClass()->GetName() );
					}
				}
			}
			...
			{
				ScriptErrorf(SCEL_Restricted,  TEXT("Function '%s' is not static"), *Function->GetName() );
			}
			...
			&&	(Function->NumParms>=2) )
			{
				TFieldIterator<UProperty,CASTCLASS_UProperty> It(Function);
			...
			for( It; It && (It->PropertyFlags&(CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++It,++Count )
			{
				...
				if( Count!=0 && !MatchSymbol(TEXT(",")) )
				{
					...
					{
						ScriptErrorf(SCEL_Expression,  TEXT("Call to '%s': missing or bad parameter %i"), *Function->GetName(), Count+1 );
					}
			...
			if( !Temp.Matches(TEXT(")")) )
			{
				...
				if (Temp.Matches(TEXT(",")))
				{
					ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/, TEXT("Call to '%s': Function only takes %i parameter(s)"), Token.Identifier, (INT)Function->NumParms - (Function->GetReturnProperty() != NULL ? 1 : 0));
					///return FALSE;
				}
				else
				{
...
UBOOL FScriptCompiler::CompileDynamicCast( const FToken& Token, FToken& ResultType )
{
	...
	if( !MatchSymbol(TEXT("(")) && !PeekSymbol(TEXT("<")) )
		return 0;
	UClass* DestClass = FindClass(Token.Identifier);//FindObject<UClass>( ANY_PACKAGE, Token.Identifier );
	if( !DestClass )
	{
	...
	if( DestClass==UClass::StaticClass() )
	{
		if( MatchSymbol(TEXT("<")) )
		{
			...
			if( !GetIdentifier(Token) )
				return 0;
			MetaClass = FindClass(Token.Identifier);//FindObject<UClass>( ANY_PACKAGE, Token.Identifier );
...
)
{
	...
	else
	{
		...
		&&	MatchSymbol(TEXT("(")) )
		{
			...
			{
				ScriptErrorf(SCEL_Restricted,  TEXT("No need to cast '%s' to itself"), *GetPropertyName(ToType.Type).ToString() );
			}
			else
			{
	...
	for( ; ; )
	{
		...
		if ( MatchSymbol(TEXT(".")) )
		{
			...
			if( Token.IsDynamicArray() )
			{
				...
				else if (MatchIdentifier(NAME_AddItem))
				{
					...
					// when the array is accessed through a NULL context
					{
						...
						//@note: if wSkip is changed to no longer be a WORD, execDynArrayAddItem must be updated to read the correct type from the stream
				...
				else if (MatchIdentifier(NAME_InsertItem))
				{
					...
					// when the array is accessed through a NULL context
					{
						...
						//@note: if wSkip is changed to no longer be a WORD, execDynArrayInsertItem must be updated to read the correct type from the stream
						WORD wSkip = TopNode->Script.Num() - NullContextSkipOverBegin.CodeTop;
						Writer << wSkip;
				...
				else if (MatchIdentifier(NAME_RemoveItem))
				{
					...
					// when the array is accessed through a NULL context
					{
						...
						//@note: if wSkip is changed to no longer be a WORD, execDynArrayRemoveItem must be updated to read the correct type from the stream
						WORD wSkip = TopNode->Script.Num() - NullContextSkipOverBegin.CodeTop;
						Writer << wSkip;
			...
			else if( Token.Type==CPT_Struct )
			{
				...
				for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(Token.Struct); It && !Member; ++It )
				{
	...
	if( GetToken(OperToken,NULL,1) )
	{
		if( OperToken.TokenName != NAME_None )
		{
			...
			{
				for( TFieldIterator<UFunction, CASTCLASS_UFunction> It(Nest[i].Node); It; ++It )
				{
			...
			if( OperLinks.Num()>0 && Precedence<MaxPrecedence )
			{
				...
				if( NumParms==3 || IsPreOperator )
				{
                    ...
                    {
						ScriptErrorf(SCEL_Unknown/*SCEL_Expression*/,  TEXT("Bad or missing expression after '%s': '%s'"), *OperToken.TokenName.ToString(), NewResultType.Identifier );
                    }
				}
				...
				UBOOL AnyLeftValid=0, AnyRightValid=0, ObjectComparisonBaseSuccess=0;
				for( INT i=0; i<OperLinks.Num(); i++ )
				{
					// See how good a match the first parm is.
					UFunction*  Node      = OperLinks(i);
					INT			ThisMatch = 0;
					TFieldIterator<UProperty,CASTCLASS_UProperty> It(Node);
				...
				if( BestMatch == MAXINT )
				{
					...
					const UBOOL bSpecialBoolComparison =	(OperToken.TokenName == NameEqualEqual || OperToken.TokenName == NameNotEqual) 
														&&	(bValidStructComparison || bValidDelegateComparison);
					if ( bSpecialBoolComparison )
					{
						...
						else if ( bValidDelegateComparison )
						{
							...
							{
								ScriptErrorf(SCEL_Restricted, TEXT("Mismatched delegates in %s; expected '%s', got '%s'"), *OperToken.TokenName.ToString(), 
							...
							if ( NewResultType.TokenFunction != NULL )
							{
								...
								{
									ScriptErrorf(SCEL_Restricted,  TEXT("'%s' mismatches delegate '%s' in %s"), *NormalFunction->FriendlyName.ToString(), *DelegateFunction->FriendlyName.ToString(), *OperToken.TokenName.ToString() );
								}

								// Check all individual parameters.
								INT Count=0;
								for( TFieldIterator<UProperty,CASTCLASS_UProperty> DestinationParameter(DelegateFunction), SourceParameter(NormalFunction); 
					...
					{
						ScriptErrorf(SCEL_Restricted,  TEXT("Right type is incompatible with '%s'"), *OperToken.TokenName.ToString() );
					}
					else if( AnyRightValid && !AnyLeftValid )
					{
						...
						ScriptErrorf(SCEL_Restricted,  TEXT("Left type is incompatible with '%s'"), *OperToken.TokenName.ToString() );
					}
					else
					{
						ScriptErrorf(SCEL_Restricted,  TEXT("Types are incompatible with '%s'"), *OperToken.TokenName.ToString() );
					}
				}
				else if( Matches > 1 )
				{
					ScriptErrorf(SCEL_Restricted,  TEXT("Operator '%s': Can't resolve overload (%i matches of quality %i)"), *OperToken.TokenName.ToString(), Matches, BestMatch );
				}
				else
				{
					...
					check(BestOperLink);
					TFieldIterator<UProperty,CASTCLASS_UProperty> It(BestOperLink);
					...
					if( BestOperLink->NumParms==3 || IsPreOperator )
					{
						if( OperParm2->PropertyFlags & CPF_OutParm )
						{
							...
							{
								ScriptErrorf(SCEL_Restricted, TEXT("Illegal to use operator %s to modify the length of a dynamic array"), *OperToken.TokenName.ToString());
							}
...
void FScriptCompiler::PushNest( ENestType NestType, FName ThisName, UStruct* InNode )
{
	...
	{
		for( TFieldIterator<UField,CASTCLASS_None,FALSE> It(TopNode); It; ++It )
		{
	...
	switch( NestType )
	{
		...
		case NEST_Function:
			...
			else if ( Pass == PASS_Compile )
			{
				...
				for ( TFieldIterator<UProperty,CASTCLASS_UProperty> It(func); It; ++It )
				{
					UProperty* prop = *It;
...
void FScriptCompiler::PopNest( ENestType NestType, const TCHAR* Descr )
{
				...
				for ( TMap<UClass*,UProperty*>::TIterator It(TopClass->Interfaces); It; ++It )
				{
					...
					for( UClass* Interface = It.Key(); Interface; Interface=Interface->GetSuperClass() )
					{
						...
						if ( !TopNode->IsChildOf( Interface ) )
						{
							...
							for( TFieldIterator<UFunction,CASTCLASS_UFunction,FALSE> InterfaceFunction(Interface); InterfaceFunction; ++InterfaceFunction )
							{
								...
								for( TFieldIterator<UFunction> ClassFunction(TopNode); ClassFunction; ++ClassFunction )
								{
									if ( ClassFunction->GetFName() == InterfaceFunction->GetFName() )
									{
										...
										INT Count = 0;
										for( TFieldIterator<UProperty,CASTCLASS_UProperty> It1(*InterfaceFunction),It2(*ClassFunction); Count<ClassFunction->NumParms; ++It1,++It2,Count++ )
										{
	...
	else if ( Pass == PASS_Compile )
	{
		...
		else if( NestType==NEST_Function )
		{
			...
			if( !(TopFunction->FunctionFlags & FUNC_Native) )
			{
 				...
 				if( LocalProperties )
				{
					...
					for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(TopNode); It; ++It )
					{
		...
		{
			const UBOOL bNestedIterator = Nest[NestLevel - 2].NestType == NEST_ForEach 
				//@fixme - technically, we should do this for dynamic array iterators as well so
				// that executing one of them through a NULL context doesn't crash
				&& (Nest[NestLevel - 2].NestFlags&NESTF_DynamicArrayIter) == 0;

			// Perform next iteration.
			Writer << EX_IteratorNext;

			// Here's the end of the loop - set the values for the jump target placeholders
			// to the current number of bytes
			if ( !bNestedIterator )
			{
			...
			// normally, when a NULL context is encountered while attempting to execute an iterator, the stack code pointer
			// should be incremented to point to the EX_IteratorPop bytecode, so that the iterator knows to exit (this is why
			// we emit the EX_IteratorPop *after* setting the jump target addresses.  However, in the case of nested iterators, this
			// would result in the outer iterator processing the EX_IteratorPop bytecode and breaking immediately.  This would also leave
			// the bytecode stream in the wrong location, resulting in a random function being called (though it usually goes to execUndefined).
			// So in this case, we want the iterator to jump past the EX_IteratorPop bytecode as well.
			if ( bNestedIterator )
			{
				TopNest->SetJumpTargetValue( FIXUP_IteratorEnd, TopNode->Script.Num() );
				TopNest->SetJumpTargetValue( FIXUP_IteratorSkip, TopNode->Script.Num() );

				// we need to emit an extra bytecode because the iterator code [for the outer iterator] is going to
				// increment the code pointer, expecting to find EX_IteratorNext/Pop (which will not be the case if there
				// is additional code inside the iterator loop after the internal iterator)
				Writer << EX_Nothing;
			}
		}
	...
	for( FJumpTargetPlaceholder* Fixup = Nest->JumpPlaceholderList; Fixup!=NULL; Fixup=Fixup->Next )
	{
		...
		else
		{
			...
			if ( Fixup->Type == FIXUP_IteratorSkip )
			{
				...
				// be incremented by the skip-over value AFTER we've read the skip-over (WORD) and return value size (VariableSizeType) from the bytestream
				Target = Nest->JumpTargets[Fixup->Type] - (Fixup->iCode + sizeof(WORD) + sizeof(VariableSizeType));
			}
			else
			{
...
static UProperty* GroupProperty( UStruct* Scope, UProperty* Previous, QWORD PropertyFlags, FName ClassName )
{
	...
	if( bMovable )
	{
		for ( TFieldIterator<UProperty,CASTCLASS_UProperty,FALSE> It(Scope); It; ++It )
		{
			UProperty* Property = *It;
...
)
{
	...
	if( MatchSymbol(TEXT("[")) )
	{
		...
		if( !PeekSymbol(TEXT("]")) )
		{
			...
			if ( VarProperty.GetSize() > MAX_VARIABLE_SIZE )
			{
				ScriptErrorf(SCEL_Limit, TEXT("%s %s: Array variable too large; try reducing the size of the type [if possible] or reducing the size of the array (%i bytes, %i max)"), HintText, VarToken.Identifier, VarProperty.GetSize(), (VariableSizeType)MAX_VARIABLE_SIZE);
			}
		}
...
// 	if ( !Skip && !NoArrays && !IsFunction && Scope->GetClass() != UFunction::StaticClass() )
	...
	else if( !Skip )
	{
	    ...
	    for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(Scope); It && It.GetStruct()==Scope; ++It )
		{
			Prev = *It;
		}
		if( VarProperty.ArrayDim==0 )
		...
		else if( VarProperty.Type==CPT_Byte )
		{
			if ( Class->HasAnyClassFlags(CLASS_Native) && !HasMemberProperties(Scope) )
			{
				...
				if ( SuperScope != NULL )
				{
					...
					for( TFieldIterator<UProperty,CASTCLASS_UProperty,FALSE> It(SuperScope); It; ++It )
					{
...
void FScriptCompiler::CompileClassDeclaration( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	for( ; ; )
	{
		...
		{
			ScriptWarnf(SCWL_Level1, TEXT("SafeReplace: keyword is deprecated"));
		}
...
void FScriptCompiler::CompileFunctionDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	if( HasReturnValue )
	{
		...
		GetVarNameAndDim( TopNode, ReturnType, ReturnValueFlags, TRUE, TRUE, TEXT("ReturnValue"), TEXT("Function return type"), NAME_None, FALSE );
		if ( !bReparsingClass )
		{
			TopFunction->NumParms++;
		}
		...
		for( INT i=NestLevel-2; i>=1; i-- )
		{
			for( TFieldIterator<UFunction> Function(Nest[i].Node); Function; ++Function )
			{
					...
					&&	((Function->FunctionFlags ^ TopFunction->FunctionFlags) & (FUNC_Operator | FUNC_PreOperator))==0 )
				{
					...
					// Check all individual parameters.
					INT Count=0;
					for( TFieldIterator<UProperty,CASTCLASS_UProperty> CurrentFuncParam(TopFunction),SuperFuncParam(*Function); Count<Function->NumParms; ++CurrentFuncParam,++SuperFuncParam,++Count )
					{
...
void FScriptCompiler::CompileIgnoreDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	for( ; ; )
	{
			...
			||	IgnoreFunction.TokenName.GetIndex() >= NAME_PROBEMAX )
		{
			for( INT i=NestLevel-2; i>=1; i-- )
			{
				for( TFieldIterator<UFunction,CASTCLASS_UFunction> Function(Nest[i].Node); Function; ++Function )
				{
...
void FScriptCompiler::CompileVariableDeclaration( FToken& Token, UBOOL& NeedSemiColon )
{
	...
	if( MatchSymbol(TEXT("(")) )
	{
		...
		if( !MatchSymbol(TEXT(")")) )
		{
			ScriptErrorf(SCEL_Unknown/*SCEL_Formatting*/,  TEXT("Missing ')' after editable category") );
		}
	}
...
void FScriptCompiler::CompileCommand( FToken& Token, UBOOL& NeedSemicolon )
{
	...
	if( Token.Matches(NAME_Switch) )
	{
		...
		FScriptLocation HighRetry;
		EmitSize( TopNest->SwitchType.GetSize(), TEXT("'Switch'") );
		MoveCompiledCode(LowRetry,HighRetry);
...
//			FPropertyBase Property(Return,CPRT_AssignmentReference);
	...
	else if( Token.Matches(NAME_ForEach) )
	{
		...
		TopNest->NestFlags |= NESTF_DynamicArrayIter;
...
void FScriptCompiler::PostParse( UStruct* Node )
{
	...
	if( ThisFunction )
	{
		...
		for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(ThisFunction); It; ++It )
		{
	...
	if( ThisState )
	{
		for( TFieldIterator<UFunction,CASTCLASS_UFunction> Function(ThisState); Function; ++Function )
		{
			if
...
void FScriptCompiler::CompileSecondPass( UStruct* Node )
{
	...
	if( TopFunction )
	{
		...
		for ( TFieldIterator<UProperty,CASTCLASS_UProperty> It(TopFunction); It; ++It )
		{
			UProperty* prop = *It;
	...
	if( Node->Line!=INDEX_NONE && Node!=Class )
	{
		...
		else if( TopFunction )
		{
			...
			for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(TopFunction); It && (It->PropertyFlags&(CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++It )
			{
				UProperty* Property = *It;
...
)
{
	...
	try
	{
		...
		if( Pass == PASS_Parse && !bReparsingClass )
		{
			...
			for( TFieldIterator<UProperty,CASTCLASS_UProperty,FALSE> It(Class); It; ++It )
			{
				if( It->IsLocalized() )
				{
			...
			UBOOL bFoundAuto = 0;
			for (TFieldIterator<UState, CASTCLASS_UState> It(Class); It && !bFoundAuto; ++It)
			{
...
static UBOOL ParseScripts( FClassTree& AllClasses, FScriptCompiler& Compiler, UClass* Class, UBOOL MakeAll, UBOOL Booting, UBOOL MakeSubclasses )
{
	...
	for( INT NameIndex=0; NameIndex<Class->DependentOn.Num(); NameIndex++)
	{
		...
		if( !DependsOnClass )
		{
			Compiler.ScriptErrorf(SCEL_Class, TEXT("Failed to find DependsOn class '%s' while parsing '%s'"),*Class->DependentOn(NameIndex).ToString(),*Class->GetName() );
			return FALSE;
		}
		else
		{
...
static void CompileStructDefaults( UScriptStruct* StructToCompile, TLookupMap<UScriptStruct*>& ScriptStructs, FScriptCompiler& Compiler, UClass* Class )
{
	if ( StructToCompile )
	{
		...
		for( TFieldIterator<UStructProperty,CASTCLASS_UStructProperty,FALSE> It(StructToCompile); It; ++It )
		{
...
static void ExportNativeHeaders( UObject* CurrentPackage, FClassTree& AllClasses )
{
	...
	{
		UBOOL bQuiet = !ParseParam(appCmdLine(),TEXT("VERBOSE"));
