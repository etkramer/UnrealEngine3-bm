...
UBOOL FObjectPair::ShouldComparePropertyValue( UProperty* Property, UStruct* CurrentOwnerStruct, UStruct* OriginalOwnerStruct, UProperty** MatchingProperty/*=NULL*/ )
{
	...
	UProperty* OriginalProperty = FindFieldWithFlag<UProperty,CASTCLASS_UProperty>(OriginalOwnerStruct, Property->GetFName());
		...
		if ( !bShouldSkipProperty )
		{
			...
			if ( ShouldComparePropertyValue(Property, CurrentStruct, OriginalStruct, &OriginalProperty) )
			{
				...
				if ( InheritedValueBase != NULL && Property->Offset + Property->ElementSize * Property->ArrayDim <= InheritedDataCount )
				{
					...
					{
						if ( Property->Matches(InheritedValueBase, CurrentValue, ArrayIndex, FALSE, PPF_DeepComparison) )
						{
...
void FStructPair::CompileChildMap()
{
	...
	{
		for ( TFieldIterator<UStruct,CASTCLASS_None,FALSE> It(OriginalStruct); It; ++It )
		{
			UStruct* Struct = *It;
	...
	{
		for ( TFieldIterator<UStruct,CASTCLASS_None,FALSE> It(CurrentStruct); It; ++It )
		{
			UStruct* Struct = *It;
			FString StructId = Struct->GetPathName(Struct->GetOwnerClass());
			FStructPair* Pair = ChildMap->Find(*StructId);
...
void FComparisonResults::ExportObjectData( FScriptPatchExporter& Writer, INT ExportMapIndex )
{
...
#else
	Writer.Logf(TEXT("\t\tSize = %i;\r\n"), ObjectData.SerialSize);
#endif
	Writer.Logf(TEXT("\t\tDataName = TEXT(\"%s\");\r\n"), *ObjectData.ObjectName.ToString());
	...
	Writer.Log(TEXT("\t\tData = ObjectData;\r\n\t}\r\n"));
#else
	Writer.Logf(TEXT("\t\tappMemcpy(ObjectData,bytes,%i*sizeof(BYTE));\r\n"), Data.Num());
	Writer.Log(TEXT("\t\tData = ObjectData;\r\n\t}\r\n"));

	// remainder of patch class
	Writer.Log(TEXT("private:\r\n"));
	Writer.Logf(TEXT("\tBYTE\tObjectData[%i];\r\n};\r\n"), Data.Num());
#endif
}
...
void FComparisonResults::ExportModifiedScript( FScriptPatchExporter& Writer )
{
	for ( INT i = 0; i < StructsModified.Num(); i++ )
	{
		...
		Writer.Logf(TEXT("\t\tappMemcpy(&Data(0),bytes,%i*sizeof(BYTE));\r\n\t}\r\n\r\n"), Bytecode.Num());
#else
		Writer.Logf(TEXT("\t\tappMemcpy(PatchBytes,bytes,%i*sizeof(BYTE));\r\n"), Bytecode.Num());
		Writer.Log(TEXT("\t\tData = PatchBytes;\r\n\t}\r\n\r\n"));

		// remainder of patch class
		Writer.Log(TEXT("private:\r\n"));
		Writer.Logf(TEXT("\tBYTE\tPatchBytes[%i];\r\n};\r\n"), Struct->Script.Num());
#endif
	}
}
...
void FComparisonResults::ExportPackageData(FScriptPatchExporter& Writer)
{
	...
	for ( INT i = 0; i < ExportsAdded.Num(); i++ )
	{
		...
		if ( Export._Object != NULL )
		{
			...
			Writer.Logf(TEXT("\t\tnew(Exports) FObjectExport(BuildExport(FName(TEXT(\"%s\")), %I64i, %d, %i, %i, %i, %i));\r\n"),
				*Export.ObjectName.ToString(), Export.ObjectFlags, Export.ExportFlags, Export.ClassIndex, Export.SuperIndex, Export.OuterIndex, Export.ArchetypeIndex);
		}
	}
...
void FScriptPatchExporter::SaveToFile()
{
	...
	FString HeaderPathName = FString::Printf(TEXT("..\\Development\\Src\\Core\\Inc\\%sScriptPatches.h"), bNonIntelByteOrder ? TEXT("Swapped") : TEXT(""));
...
INT FScriptPatchWorker::Main( const FString& Params )
{
	...
	bNonIntelScriptPatch = Switches.ContainsItem(TEXT("nonintel"));
	if ( !InitCommandlet() )
	{
		return 1;
	}
...
// 	NonNativeScriptPackageNames.AddItem(TEXT("WarfareGameContentWeapons"));
	...
	if ( bNonIntelScriptPatch )
	{
		// when generating a patch for a non-PC platform, remove editor-only packages from the list of native packages
...
void FScriptPatchWorker::UpdatePackageCache( const TArray<FFilename>& PackagePathnames )
{
	for ( INT PackageIndex = 0; PackageIndex < PackagePathnames.Num(); PackageIndex++ )
	{
		...
		if ( bNonIntelScriptPatch && IsNonNativeScriptPackage(PackageName) )
		{
...
{
	// probably don't even need to do this part once we've found all the places where package names are requested without using ResolveName or appending the GScriptPatchPackageSuffix
	GPackageFileCache->ClearPackageCache();
