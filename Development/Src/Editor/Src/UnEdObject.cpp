...
static FString ReadStructValue( const TCHAR* pStr, const TCHAR*& SourceTextBuffer, TCHAR* CurrentLineBuffer, INT LineBufferSize )
{
	if ( *pStr == '{' )
	{
		INT bracketCnt = 0;
		...
		do
		{
			...
			{
				// parse a new line
				if (ParseLine(&SourceTextBuffer,CurrentLineBuffer,LineBufferSize,TRUE))
				{
			...
			if( *Str )
			{
				...
				if ( ArrayOp != ADO_None )
				{
					...
					else if( ArrayOp == ADO_Add || ArrayOp == ADO_Remove || ArrayOp == ADO_Replace )
					{
						...
						FString ValueText = ReadStructValue(Str, SourceText, StrLine, StrLineLength);
						Str = *ValueText;
				...
				else
				{
					...
					FString FullText = ReadStructValue(Str, SourceText, StrLine, StrLineLength);
