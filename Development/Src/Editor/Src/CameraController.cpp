...
{
	FCameraControllerUserImpulseData FinalUserImpulse;
	...
	// Movement
	UpdatePosition( FinalUserImpulse, DeltaTime, MovementSpeedScale, InOutCameraEuler, InOutCameraPosition );



	// Rotation
	UpdateRotation( FinalUserImpulse, DeltaTime, InOutCameraEuler );



	// FOV
	UpdateFOV( FinalUserImpulse, DeltaTime, InOutCameraFOV );
...
void FEditorCameraController::UpdateRotation( const FCameraControllerUserImpulseData& UserImpulse, const FLOAT DeltaTime, FVector &InOutCameraEuler )
{
	FVector RotateImpulseEuler =
		FVector( UserImpulse.RotateYawImpulse,
		UserImpulse.RotatePitchImpulse,
		UserImpulse.RotateRollImpulse );

	FVector RotateVelocityModifierEuler =
		FVector( UserImpulse.RotateYawVelocityModifier,
		UserImpulse.RotatePitchVelocityModifier,
		UserImpulse.RotateRollVelocityModifier );
	...
	for( INT CurRotationAxis = 0; CurRotationAxis < 3; ++CurRotationAxis )
	{
		...
		if( Config.bUsePhysicsBasedRotation )
		{
		...
		if( Abs( RotationVelocity ) > Config.MaximumRotationSpeed )
		{
			if( RotationVelocity < 0.0f )
			{
				RotationVelocity = -Config.MaximumRotationSpeed;
			}
			else
			{
				RotationVelocity = Config.MaximumFOVSpeed;
			}
		}
...
void FEditorCameraController::UpdateFOV( const FCameraControllerUserImpulseData& UserImpulse, const FLOAT DeltaTime, FLOAT& InOutCameraFOV )
{
	...
	if( Abs( FOVVelocity ) > Config.MaximumFOVSpeed )
	{
		if( FOVVelocity < 0.0f )
		{
			FOVVelocity = -Config.MaximumFOVSpeed;
		}
		else
		{
			FOVVelocity = Config.MaximumFOVSpeed;
