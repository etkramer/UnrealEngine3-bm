	...
	PendingIndexDataStride(0)
{
	...
	// Enable InvertedZ for better depth precision
	GUsesInvertedZ = TRUE;
	...
	if(SUCCEEDED(Direct3D->GetDeviceCaps(AdapterIndex,DeviceType,&DeviceCaps)))
	{
		if ( (DeviceCaps.PixelShaderVersion & 0xff00) < 0x0200 )
		{
			appErrorf(
				NAME_FriendlyError,
				TEXT("This game requires at least Shader Model 2.0")
				);
		}
	...
	// -forceshadermodel2 or -sm2 will force the shader model 2 path even on sm3 hardware
	if (ParseParam(appCmdLine(),TEXT("FORCESHADERMODEL2"))
		|| ParseParam(appCmdLine(),TEXT("SM2")))
	{
		warnf(TEXT("Forcing Shader Model 2 Path."));
		GRHIShaderPlatform = SP_PCD3D_SM2;
	...
	// WRH - 2007/09/01 - HACK to detect cooking since GIsCooking is not set early enough anymore. This should be removed
	// when fixed because this detection scheme is not very robust
	if (GIsCooking
		|| appStristr(appCmdLine(), TEXT("CookPackages")) != NULL
		|| appStristr(appCmdLine(), TEXT("PrecompileShaders")) != NULL
		|| ParseParam(appCmdLine(),TEXT("FORCESHADERMODEL3"))
		|| ParseParam(appCmdLine(),TEXT("SM3"))
		)
	{
		//Force SM3 when cooking
		//This avoids a dependency on the SM2 cache being up to date when the engine is only cooking
	...
	if(Viewports.Num())
	{
		...
		if(bRecreateDevice)
		{
			...
			if(Direct3DDevice)
			{
				...
				// After device restart we want to rescale all RTs, its necessary on ATI x800 series
				// where we use Attenuation Buffer instead Color Buffer when doing modulated shadows
				// see ShadowRendering.cpp 
				if ( GSceneRenderTargets.GetBufferSizeX() > 0 && GSceneRenderTargets.GetBufferSizeY() > 0 )
				{
					GSceneRenderTargets.SetBufferSize( NewDeviceSizeX, NewDeviceSizeY );
				}
			...
			else
			{
				...
				// Display a message if running on sm2 in editor mode
				if ( !GIsEpicInternal && !GIsUnattended && GIsEditor && GRHIShaderPlatform == SP_PCD3D_SM2 )
				{
					FString GameName;
					GConfig->GetString(TEXT("URL"), TEXT("GameName"), GameName, GEngineIni);

					switch( appMsgf( AMT_YesNo, LocalizeSecure(LocalizeError(TEXT("RunningOnSM2"),TEXT("UnrealEd")), *GameName ) ) )  
					{
					case 0:		// No
						appRequestExit( TRUE );
