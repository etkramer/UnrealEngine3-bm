 ...
 */
#if !SHIPPING_PC_GAME && _MSC_VER && !XBOX
...
#define LINE_CHECK_TRACE_SCRIPT(flags, stack) do { LineCheckTracker::CaptureLineCheck(flags, NULL, stack); } while(0)
	...
	#define LINE_CHECK_TICK			__noop
	#define LINE_CHECK_DUMP			__noop
	#define LINE_CHECK_RESET		__noop
	#define LINE_CHECK_TOGGLE		__noop
	#define LINE_CHECK_TRACE		__noop
	#define LINE_CHECK_TRACE_SCRIPT __noop
#else
	...
	#define LINE_CHECK_TRACE_SCRIPT(flags, stack)
