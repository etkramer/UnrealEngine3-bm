...
public:
	...
	/**
	 * Called from the game thread to see if this texture should be re-allocated.
	 */
	UBOOL IsOrphan()
	{
#if XBOX && USE_XeD3D_RHI
		FXeTextureBase* TextureBase = (FXeTextureBase*)Texture2DRHI;
		return (TextureBase != NULL) && (TextureBase->IsOrphan());
#else
		return FALSE;
#endif
	}
...
private:
	 ...
	 * @param bShouldPrioritizeAsyncIORequest	- Whether the Async I/O request should have higher priority
	 */
	void UpdateMipCount( UBOOL bShouldPrioritizeAsyncIORequest );
	/**
