...
struct FLensFlareRenderElement
{
	 ...
	 *	The material(s) to use for the flare element.
	 */
	TArrayNoInit<FMaterialRenderProxy*> LFMaterials;
	...
	FLensFlareRenderElement(const FLensFlareElement& InElement, UBOOL bSelected)
	{
		appMemzero(this, sizeof(FLensFlareRenderElement));
		CopyFromElement(InElement, bSelected);
	}

	~FLensFlareRenderElement();

	void CopyFromElement(const FLensFlareElement& InElement, UBOOL bSelected);
...
public:
    ...
    virtual void ReleaseDynamicRHI()
    {
		for (LensFlareViewOcclusionMap::TIterator It(OcclusionMap); It; ++It)
		{
			...
			Data.OcclusionQuery.SafeRelease();
		}
