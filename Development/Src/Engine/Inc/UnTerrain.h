...
static inline INT Terrain_GetPaddedSize(INT Size)
{
	...
	{
		return appRoundUpToPowerOfTwo(Size);
	}
	else
	{
		return Size;
...
//    DECLARE_CLASS(FTerrainMaterialResource,UMaterial,0,Engine)
	...
	FTerrainMaterialResource() : Mask(0), bParametersCached(FALSE) {}
	...
	virtual UBOOL RequiresSimpleStaticLighting(EMaterialShaderPlatform MatPlatform) const;
