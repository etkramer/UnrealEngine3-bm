...
public:
	...
	*/
	...
	mutable TArray<INT> SampleRemapping;
...
FORCEINLINE void CopyDecalRenderDataToStaticReceiverData(FStaticReceiverData& Dst, const FDecalRenderData& Src, const FDecalInteraction* DecalInteraction)
{
	Dst.Vertices		= Src.Vertices;
	...
	{
		FLightMap1D* SourceLightMap1D = const_cast<FLightMap1D*>(Src.LightMap1D->GetLightMap1D());
		Dst.LightMap1D = SourceLightMap1D->DuplicateWithRemappedVerts( DecalInteraction->DecalState.SampleRemapping );
	}
	else
	{
		Dst.LightMap1D	= Src.LightMap1D;
