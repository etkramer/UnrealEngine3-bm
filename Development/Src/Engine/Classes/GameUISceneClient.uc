...
event InitializeSceneClient()
{
	...
	if ( OnlineSub != None )
	{
		...
		if ( OnlineSub.PlayerInterface != None )
		{
			...
			OnlineSub.PlayerInterface.AddLoginChangeDelegate(NotifyLoginStatusChanged0,0);
			OnlineSub.PlayerInterface.AddLoginChangeDelegate(NotifyLoginStatusChanged1,1);
			OnlineSub.PlayerInterface.AddLoginChangeDelegate(NotifyLoginStatusChanged2,2);
			OnlineSub.PlayerInterface.AddLoginChangeDelegate(NotifyLoginStatusChanged3,3);
		}
		else
		{
 ...
 *	MaxPlayersAllowed)
 */
event SynchronizePlayers( optional int MaxPlayersAllowed=MAX_SUPPORTED_GAMEPADS, optional bool bAllowJoins=true, optional bool bAllowRemoval=true )
{
	...
	if ( IsAllowedToModifyPlayerCount() )
	{
		for ( ControllerId = 0; ControllerId < MAX_SUPPORTED_GAMEPADS; ControllerId++ )
		{
			...
			if ( IsGamepadConnected(ControllerId) )
			{
				if ( PlayerIndex == INDEX_NONE && bAllowJoins )
				{
					...
					if ( GamePlayers.Length < MaxPlayersAllowed )
					{
					...
					{
						`log(`location @ "unable to create player for gamepad" @ ControllerId @ "because the max player count has been reached (" $ MaxPlayersAllowed $ ")");
					}
				}
			}
		...
		while ( GamePlayers.Length > Max(1, MaxPlayersAllowed) )
		{
...
function NotifyLoginStatusChanged0() { NotifyLoginStatusChanged(0, GetLoginStatus(0)); }
function NotifyLoginStatusChanged1() { NotifyLoginStatusChanged(1, GetLoginStatus(1)); }
function NotifyLoginStatusChanged2() { NotifyLoginStatusChanged(2, GetLoginStatus(2)); }
function NotifyLoginStatusChanged3() { NotifyLoginStatusChanged(3, GetLoginStatus(3)); }
function NotifyLoginStatusChanged( int ControllerId, ELoginStatus Status )
{
	local UIScene Scene;

	`log(`location @ `showvar(ControllerId) @ `showenum(ELoginStatus,Status,LoginStatus) ,,'RON_DEBUG');
