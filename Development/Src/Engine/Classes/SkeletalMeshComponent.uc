 ...
 */
native final function HideBone( int BoneIndex, bool bTermBodiesBelow );
...
event PlayParticleEffect( const AnimNotify_PlayParticleEffect AnimNotifyData )
{
	...
	if( ( AnimNotifyData.bIsExtremeContent == TRUE ) )// && ( WorldInfo.GRI.ShouldShowGore() == FALSE ) )
	{
		return;
	}
	...
	else
	{
		...
		PSC.SetRotation( rot(0,0,1) );
		PSC.ActivateSystem();
		PSC.OnSystemFinished = SkelMeshCompOnParticleSystemFinished;
		//WorldInfo.MyEmitterPool.SpawnEmitter( AnimNotifyData.PSTemplate, Location, rot(0,0,1) );
	}
}
