...
function OnShowFriendInviteUI(UIAction_ShowFriendInviteUI Action)
{
	...
	if (OnlineSub != None && OnlineSub.PlayerInterfaceEx != None)
	{
		if (OnlineSub.PlayerInterfaceEx.ShowFriendsInviteUI(class'UIInteraction'.static.GetPlayerControllerId(Action.PlayerIndex),Action.RemotePlayerNetId) == false)
		{
...
function OnShowFeedbackUI(UIAction_ShowFeedbackUI Action)
{
	...
	if (OnlineSub != None)
	{
		if (OnlineSub.PlayerInterfaceEx != None)
		{
			if (OnlineSub.PlayerInterfaceEx.ShowFeedbackUI(class'UIInteraction'.static.GetPlayerControllerId(Action.PlayerIndex),Action.RemotePlayerNetId) == false)
			{
...
function OnShowGamerCardUI(UIAction_ShowGamerCardUI Action)
{
	...
	if (OnlineSub != None)
	{
		if (OnlineSub.PlayerInterfaceEx != None)
		{
			if ( !OnlineSub.PlayerInterfaceEx.ShowGamerCardUI(class'UIInteraction'.static.GetPlayerControllerId(Action.PlayerIndex),Action.RemotePlayerNetId) )
			{
...
function OnSetControllerId( UIAction_SetControllerId Action )
{
	...
	if ( Action != None && Action.GamepadID >= 0 && Action.GamepadID < MAX_SUPPORTED_GAMEPADS )
	{
		...
		PlayerOwner = GetPlayerOwner(Action.PlayerIndex);
		if ( PlayerOwner == None )
		{
			PlayerOwner = GetPlayerOwner();
		}
 ...
 * Handler function for the SetControllerId action.
 */
function OnBecomePrimaryPlayer( UIAction_BecomePrimaryPlayer Action )
{
	...
	if ( Action != None && UIController != None && Action.PlayerIndex > 0 && Action.PlayerIndex < UIController.GetPlayerCount() )
	{
		...
		if ( PlayerOwner != None )
		{
			...
			while ( NextPlayer != None && NextPlayer != PlayerOwner )
			{
				...
				UIController.GamePlayers.Remove(0, 1);
			...
			while ( OtherPlayers.Length > 0 )
			{
				...
				UIController.GamePlayers.InsertItem(1, NextPlayer);
...
final function Add_UIAnimKeyFrameCompletedHandler( delegate<OnUIAnim_KeyFrameCompleted> KeyFrameCompletedDelegate )
{
	...
	&&	KeyFrameCompletedDelegates.Find(KeyFrameCompletedDelegate) == INDEX_NONE )
	{
...
final function Add_UIAnimTrackCompletedHandler( delegate<OnUIAnim_TrackCompleted> TrackCompletedDelegate )
{
	...
	&&	TrackCompletedDelegates.Find(TrackCompletedDelegate) == INDEX_NONE )
	{
