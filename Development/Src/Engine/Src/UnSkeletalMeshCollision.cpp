...
void USkeletalMesh::UpdatePerPolyKDOPs()
{
	...
	for(INT ChunkIndex = 0;ChunkIndex < LODModel->Chunks.Num();ChunkIndex++)
	{
		...
		if(Chunk.GetNumRigidVertices() > 0)
		{
			...
			{
				const FGPUSkinVertexBase* SrcRigidVertex = LODModel->VertexBufferGPUSkin.GetVertexPtr(Chunk.GetRigidVertexBufferIndex()+VertIdx);
		...
		if(Chunk.GetNumSoftVertices() > 0)
		{
			...
			{
				const FGPUSkinVertexBase* SrcSoftVertex = LODModel->VertexBufferGPUSkin.GetVertexPtr(Chunk.GetSoftVertexBufferIndex()+VertIdx);
			...
			for(INT ChunkIndex = 0;ChunkIndex < LODModel->Chunks.Num();ChunkIndex++)
			{
				...
				if(bPerPolyUseSoftWeighting)
				{
					for(INT i=0; i<Chunk.SoftVertices.Num(); i++)
					{
						...
						if( RelevantBones.ContainsItem( Chunk.BoneMap(SV.InfluenceBones[0]) )||
							RelevantBones.ContainsItem( Chunk.BoneMap(SV.InfluenceBones[1]) )||
							RelevantBones.ContainsItem( Chunk.BoneMap(SV.InfluenceBones[2]) )||
							RelevantBones.ContainsItem( Chunk.BoneMap(SV.InfluenceBones[3]) ) )
						{
							CollisionToGraphicsVertMap.AddItem(VertIndex);
						}
