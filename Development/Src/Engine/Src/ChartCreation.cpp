...
void UEngine::DumpFPSChartToLog( FLOAT TotalTime, FLOAT DeltaTime, INT NumFrames )
{
	...
	// Dump hitch data
	{
		...
		debugf( TEXT( "Hitch frames bound by game thread:  %i  (%0.1f%%)" ), TotalGameThreadBoundHitches, TotalHitchCount > 0 ? ( ( FLOAT )TotalGameThreadBoundHitches / ( FLOAT )TotalHitchCount * 100.0f ) : 0.0f );
		debugf( TEXT( "Hitch frames bound by render thread:  %i  (%0.1f%%)" ), TotalRenderThreadBoundHitches, TotalHitchCount > 0 ? ( ( FLOAT )TotalRenderThreadBoundHitches / ( FLOAT )TotalHitchCount * 0.0f ) : 0.0f  );
		debugf( TEXT( "Hitch frames bound by GPU:  %i  (%0.1f%%)" ), TotalGPUBoundHitches, TotalHitchCount > 0 ? ( ( FLOAT )TotalGPUBoundHitches / ( FLOAT )TotalHitchCount * 100.0f ) : 0.0f );
...
void UEngine::DumpFPSChart( UBOOL bForceDump )
{
	...
	if( GSentinelRunID == -1 )
	{
		...
		if( ( bAutoStartSentinelRun == TRUE ) || ( bForceDump == TRUE ) )
		{
			if( GWorld != NULL )
			{
				...
				if( GameInfo != NULL )
				{
					GameInfo->BeginSentinelRun( GameInfo->SentinelTaskDescription, GameInfo->SentinelTaskParameter, GameInfo->SentinelTagDesc );
				}
			}
		}
	}
	...
	ParseUBOOL( appCmdLine(), TEXT("-CaptureFPSChartInfo="), bFPSChartIsActive ) || ParseUBOOL( appCmdLine(), TEXT("-gCFPSCI="), bFPSChartIsActive );

	UBOOL bMemoryChartIsActive = FALSE;
	ParseUBOOL( appCmdLine(), TEXT("-CaptureMemoryChartInfo="), bMemoryChartIsActive ) || ParseUBOOL( appCmdLine(), TEXT("-gCMCI="), bMemoryChartIsActive );
...
{
	return TEXT( "MemoryUsed: ,PhysicalTotal, PhysicalMemUsed, VirtualMemUsed, GPUTotal, GPUMemUsed, AudioMemUsed, TextureMemUsed, NovodexMemUsed, VertexLightingMemUsed, StaticMeshVertexMemUsed, StaticMeshIndexMemUsed, SkeletalMeshVertexMemUsed, SkeletalMeshIndexMemUsed, VertexShaderMemUsed, PixelShaderMemUsed, NumAllocations, AllocationOverhead, AllignmentWaste" );
}
