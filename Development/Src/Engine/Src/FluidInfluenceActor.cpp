...
void AFluidInfluenceActor::CheckForErrors()
{
...
void UFluidInfluenceComponent::PostEditChange(UProperty* PropertyThatChanged)
{
...
void UFluidInfluenceComponent::Tick( FLOAT DeltaSeconds )
{
	...
	if ( bActive && GetOwner() )
	{
		if ( FluidActor == NULL )
		{
			...
			const TArray<UFluidSurfaceComponent*>* FluidSurfaces = Scene->GetFluidSurfaces();
			if ( !IsTouching(CurrentFluidActor) && FluidSurfaces )
			{
				CurrentFluidActor = NULL;
				for( TArray<UFluidSurfaceComponent*>::TConstIterator FluidIt(*FluidSurfaces); FluidIt; ++FluidIt )
				{
