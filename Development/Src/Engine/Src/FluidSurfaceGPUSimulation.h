...
struct FFluidSimulateParams
{
	...
	FFluidSimulateParams(FLOAT InDampingFactor, FLOAT InTravelSpeed)
...
struct FFluidNormalParams
{
	...
	FFluidNormalParams(FLOAT InHeightScale) :
...
private:
	...
	const static EPixelFormat FluidHeightTextureFormat = PF_R16F;
#else
	const static EPixelFormat FluidHeightTextureFormat = PF_R32F;
#endif
	
	/* Ideally we would want to use a one channel, high precision format that is alpha blendable for applying forces. */
	const static EPixelFormat FluidHeightSurfaceFormat = PF_R32F;

	/* x and y of the GPU simulated detail grid normal, needs to be filterable. */
	const static EPixelFormat FluidNormalFormat = PF_G16R16F_FILTER;
