...
void UWorld::TickWorldRBPhys(FLOAT DeltaSeconds)
{
    ...
    const FLOAT maxSubstep = Info->PhysicsProperties.PrimaryScene.TimeStep;
    const FLOAT maxDeltaTime = Info->MaxPhysicsDeltaTime;
    ...
    // diverge a lot more.)
    const FLOAT physDT = DeltaSeconds > maxDeltaTime ? maxDeltaTime : DeltaSeconds;

	TickRBPhysScene(UseScene, physDT, maxSubstep, Info->PhysicsProperties.PrimaryScene.bFixedTimeStep);
#endif // WITH_NOVODEX
}
...
// NxUserScheduler interface
	...
	BYTE GCurrentEpicPhysDataVersion = 16;
...
void InitGameRBPhys()
{
	...
	{
		// Load local PhysXCore.dll
		#if !defined( _DEBUG )
		LoadLibrary( L"PhysXLocal/PhysXLoader.dll" );
		#else
		LoadLibrary( L"PhysXLocal/PhysXLoaderDEBUG.dll" );
		#endif
	}
	else
	{
		// Load installed PhysXCore.dll to use the PhysX hardware
		#if !defined( _DEBUG )
		if (NULL == LoadLibrary( L"PhysXLoader.dll" ))
		{
			debugf(TEXT("Unable to load installed PhysXLoader.dll, using local copy"));
			LoadLibrary( L"PhysXLocal/PhysXLoader.dll" );
		}
		#else
		if (NULL == LoadLibrary( L"PhysXLoaderDEBUG.dll" ))
		{
			LoadLibrary( L"PhysXLocal/PhysXLoaderDEBUG.dll" );
		}
		#endif
	...
	check(GNovodexCooking);

#if USE_QUICKLOAD_CONVEX
	GNovodeXQuickLoad = static_cast<NxExtensionQuickLoad *>(NxCreateExtension(NX_EXT_QUICK_LOAD));
	check(GNovodeXQuickLoad);
	NxUserAllocator * QLAllocator = GNovodeXQuickLoad->wrapAllocator(*Allocator);
	GNovodexSDK = NxCreatePhysicsSDK(NX_PHYSICS_SDK_VERSION, QLAllocator, Stream);
	// Init the cooker. Sets params to defaults.
	GNovodexCooking->NxInitCooking(&GNovodexSDK->getFoundationSDK().getAllocator(), Stream);
	GNovodeXQuickLoad->initialize(*GNovodexSDK, GNovodexCooking);
#else
	GNovodexSDK = NxCreatePhysicsSDK(NX_PHYSICS_SDK_VERSION, Allocator, Stream);
...
FRBPhysScene* CreateRBPhysScene(const FVector& Gravity)
{
	...
	if(ScenePair.PrimaryScene)
	{
		...
		CompartmentDesc.deviceCode = NX_DC_PPU_AUTO_ASSIGN; // No CPU cloth compartments yet...
		if( !SceneProperties.CompartmentCloth.bUseHardware )
		{
			debugf( TEXT("Cannot create CPU cloth compartment.  Will be created in HW if available.") );
		}
...
//		NxCellConfig::setSceneParamInt(NovodexScene, NxCellConfig::NX_CELL_SCENE_PARAM_SPU_CLOTH, 0);
	...
	NewRBPhysScene->PhysXDestructibleManager = new FPhysXDestructibleManager();
...
void DestroyRBPhysScene(FRBPhysScene* Scene)
{
	...
	}
	{
		...
		delete Scene->PhysXDestructibleManager;
		delete Scene->PhysXEmitterManager;
		Scene->PhysXDestructibleManager = NULL;
...
void TickRBPhysScene(FRBPhysScene* Scene, FLOAT DeltaTime, FLOAT MaxSubstep, UBOOL bUseFixedTimestep)
{
	...
	Scene->PhysXDestructibleManager->TickManager( DeltaTime );
	...
	{
		NumSubSteps = ::Min(SceneProperties.PrimaryScene.MaxSubSteps, Info->MaxPhysicsSubsteps);
	}
	else
	{
		NumSubSteps = appCeil(DeltaTime/MaxSubstep);
		NumSubSteps = Clamp<INT>(NumSubSteps, 1, ::Min(SceneProperties.PrimaryScene.MaxSubSteps, Info->MaxPhysicsSubsteps));
...
void FRBPhysScene::AsyncRayCheck(const FVector& Origin, const FVector& Direction, FLOAT Distance, UBOOL bDynamicObjects, FAsyncLineCheckResult* Result)
{
	...
	if(ScenePair)
	{
		...
		bool bHit = ScenePair->SceneQuery->raycastAnyShape(nRay, TestTypes, -1, Distance*U2PScale, 0, 0, Result);
...
UBOOL ExecRBCommands(const TCHAR* Cmd, FOutputDevice* Ar)
{
	...
	else if( ParseCommand(&Cmd, TEXT("APEXVIS")) )
	{
		// stall all scenes to make sure we don't simulate when calling setParameter
		NxU32 NbScenes = GNovodexSDK->getNbScenes();
		for (NxU32 i = 0; i < NbScenes; i++)
		...
		UBOOL bFoundFlag = TRUE;
		if (ParseCommand(&Cmd, TEXT("SUPPORT")))
		{
			if( GWorld && GWorld->RBPhysScene && GWorld->RBPhysScene->PhysXDestructibleManager )
			{
				bNewValue = !GWorld->RBPhysScene->PhysXDestructibleManager->GetFlag( DMF_VisualizeSupport );
				GWorld->RBPhysScene->PhysXDestructibleManager->SetFlag( DMF_VisualizeSupport, bNewValue );
			}
		}
		else
		{
