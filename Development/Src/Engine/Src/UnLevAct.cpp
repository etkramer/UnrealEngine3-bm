...
#define SHOW_MULTI_PAWN_SPAWN_FRAMES (TRUE && !FINAL_RELEASE)
...
)
{
	...
	if( (Template->bCollideWorld || (Template->bCollideWhenPlacing && (GetNetMode() != NM_Client))) && !bNoCollisionFail )
	{
		...
		{
			debugf( NAME_DevSpawn, TEXT("SpawnActor failed because of collision at the spawn location [%s]"), *Class->GetName() );
			return NULL;
		}
	}
...
#if SHOW_MULTI_PAWN_SPAWN_FRAMES
	...
	if( Actor->bCollideActors && !Actor->IsA(AProjectile::StaticClass()) && !Actor->IsA(ADroppedPickup::StaticClass()) && !Actor->IsA(APawn::StaticClass()) && !Actor->IsA(AFracturedStaticMeshPart::StaticClass()) && !Actor->IsA(ACrowdAgent::StaticClass()) )
	{
 ...
 */
APlayerController* UWorld::SpawnPlayActor(UPlayer* Player, ENetRole RemoteRole, const FURL& URL, FString& Error, BYTE InNetPlayerIndex)
{
	Error=TEXT("");
...
#if !FINAL_RELEASE && MOVEACTOR_STATS
...
public:
	...
	~FScopedMoveActorTimer()
	{
		if(GShouldLogOutAFrameOfMoveActor)
		{
			...
			DOUBLE TakeTime = appSeconds() - StartTime;
			...
			if(Depth == 0)
			{
				for(INT MoveIdx=0; MoveIdx<Moves.Num(); MoveIdx++)
				{
					...
					debugf(TEXT("MOVE%s - %s %5.2fms %d %d"), *Indent, *Move.Actor->GetName(), Move.Time * 1000.f, Move.bDidLineCheck, Move.bDidEncroachCheck);
				}
...
#endif //  !FINAL_RELEASE && MOVEACTOR_STATS
...
)
{
	...
	const UBOOL bMovementOccurred = TRUE;
	DWORD TraceFlags = 0;
...
void AWorldInfo::PostEditChange(UProperty* PropertyThatChanged)
{
	...
	{
		if (PropertyThatChanged->GetName()==TEXT("bAllowModulateBetterShadows"))
		{
