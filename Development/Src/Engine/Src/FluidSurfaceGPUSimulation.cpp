...
void FFluidGPUResource::InitDynamicRHI() 
{
	for (INT i = 0; i < NumHeightfields; i++)
	{
		...
		HeightSurfaces[i] = RHICreateTargetableSurface(DetailResolution, DetailResolution, FluidHeightSurfaceFormat, HeightTextures[i], 0, TEXT("FluidHeight"));
	}

	NormalTexture = RHICreateTexture2D(DetailResolution, DetailResolution, FluidNormalFormat, 1, TexCreate_ResolveTargetable, NULL);
	NormalSurface = RHICreateTargetableSurface(DetailResolution, DetailResolution, FluidNormalFormat, NormalTexture, 0, TEXT("FluidNormal"));
	...
	FLOAT DetailHeightScale)
{
	...
	else if (NumIterations > 0)
	{
		...
		for (INT Iteration = 0; Iteration < NumIterations; ++Iteration)
		{
			...
			FFluidSimulateParams SimulateParams(DampFactor, DetailTravelSpeed);
			Simulate(SimulateParams);
		}

		// Generate normals off of the simulated heightfield
		FFluidNormalParams NormalParams(DetailHeightScale);
...
void FFluidGPUResource::ApplyForce(const FFluidForceParams& FluidForceParams, UBOOL bApplyToCurrentHeight)
{
	...
	SCOPED_DRAW_EVENT(FluidEventView)(DEC_SCENE_ITEMS,TEXT("ApplyForce"));
	...
	//@todo - implement so that we are not writing to the same render target we are reading from (d3d debug warning)
	...
	FResolveParams ResolveParams(appTrunc(MinX), appTrunc(MinY), appTrunc(MinX + SizeX), appTrunc(MinY + SizeY));
...
void FFluidGPUResource::Simulate(const FFluidSimulateParams& FluidSimulateParams)
{
	...
	DrawDenormalizedQuad(
		1, 1,
		DetailResolution - 2, DetailResolution - 2,
		1, 1,
		DetailResolution - 2, DetailResolution - 2,
		DetailResolution, DetailResolution,
		DetailResolution, DetailResolution
...
public:
	...
	{
		FVector2D CellSize(1.0f / (FLOAT)FluidResource.DetailResolution, 1.0f / (FLOAT)FluidResource.DetailResolution);
		SetPixelShaderValue(GetPixelShader(), CellSizeParameter, CellSize);

		SetPixelShaderValue(GetPixelShader(), HeightScaleParameter, FluidNormalParams.HeightScale);

		FLOAT SplineMargin = 0.3f;
