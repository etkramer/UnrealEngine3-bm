...
#if !SHIPPING_PC_GAME
				debugf(NAME_Error, TEXT("Control channel failed to send: %s"), *Text);
#endif
		...
		while (QueuedMessages.Num() > 0 && !Closing)
		{
			...
			else
			{
				...
				else
				{
					...
					// so there is no point in queueing it as it will just fail again
#if !SHIPPING_PC_GAME
					debugf(NAME_Error, TEXT("Control channel failed to send: %s"), *QueuedMessages(0));
#endif
					appErrorfSlow(TEXT("Control channel bunch overflowed"));
					debugf(NAME_Error, TEXT("Control channel bunch overflowed"));
					Connection->Close();
					...
					for (INT j = 0; j < Chan->ReplicatedActorProperties.Num(); j++)
					{
						...
						if (*ActorRef == Actor)
						{
							*ActorRef = NULL;
							...
							debugfSlow( NAME_DevNetTraffic, TEXT("Clearing Recent ref to irrelevant Actor %s from channel %i (property %s, Actor %s)"), *Actor->GetName(), Chan->ChIndex,
										*Chan->ReplicatedActorProperties(j)->GetPathName(), *Chan->Actor->GetName() );
						}
					}
				}
			}
...
void UActorChannel::ReceivedBunch( FInBunch& Bunch )
{
	...
	UFunction* RepNotifyFunc = NULL, *DataBindingUpdateFunc = NULL;
	...
	while (FieldCache || bJustSpawned)
	{
		...
		TArray<UProperty*> RepNotifies, DataBindings;
		...
		while( FieldCache && (ReplicatedProp=Cast<UProperty>(FieldCache->Field,CLASS_IsAUProperty))!=NULL )
		{
			...
			else
			{
				...
				debugfSlow( NAME_DevNetTraffic, TEXT("Received out-of-date %s"), *ReplicatedProp->GetName() );
				DestActor  = NULL;
				DestRecent = NULL;
			}
			...
			{
				RepNotifies.AddItem(ReplicatedProp);
			}

			if ( DestActor != NULL && ReplicatedProp->HasAnyPropertyFlags(CPF_DataBinding) )
			{
				DataBindings.AddItem(ReplicatedProp);
			}
		...
		if (!bIsServer)
		{
			if ( bHasReplicatedProperties )
			{
				...
				if ( DataBindings.Num() > 0 )
				{
					if ( DataBindingUpdateFunc == NULL )
					{
						DataBindingUpdateFunc = Actor->FindFunctionChecked(ENGINE_ReplicatedDataBinding);
					}

					// now issue updates for any databinding properties we received.
					Actor_eventReplicatedDataBinding_Parms DataBindingUpdateParms(EC_EventParm);
					for ( INT PropIdx = 0; PropIdx < DataBindings.Num(); PropIdx++ )
					{
						DataBindingUpdateParms.VarName = DataBindings(PropIdx)->GetFName();
						Actor->ProcessEvent(DataBindingUpdateFunc, &DataBindingUpdateParms);
						if ( Actor == NULL )
						{
							// script destroyed actor
							return;
						}
					}
				}
		...
		if( FieldCache && Cast<UFunction>(FieldCache->Field,CLASS_IsAUFunction) )
		{
			...
			for( TFieldIterator<UProperty,CASTCLASS_UProperty> Itr(Function); Itr && (Itr->PropertyFlags & (CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++Itr )
			{
