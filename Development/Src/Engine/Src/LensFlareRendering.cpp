...
void FLensFlareRenderElement::CopyFromElement(const FLensFlareElement& InElement, UBOOL bSelected)
{
	...
	Size = InElement.Size;
	if (InElement.LFMaterials.Num() > 0)
	{
		LFMaterials.AddZeroed(InElement.LFMaterials.Num());
		for (INT MatIndex = 0; MatIndex < InElement.LFMaterials.Num(); MatIndex++)
		{
			UMaterialInterface* Mat = InElement.LFMaterials(MatIndex);
			if (Mat && Mat->CheckMaterialUsage(MATUSAGE_LensFlare))
			{
				LFMaterials(MatIndex) = Mat->GetRenderProxy(bSelected);
			}

			if (LFMaterials(MatIndex) == NULL)
			{
				LFMaterials(MatIndex) = GEngine->DefaultMaterial->GetRenderProxy(bSelected);
			}
		}
	}
	...
	VertexData(NULL)
{
	...
	if (InLensFlareComp && InLensFlareComp->Template)
	{
		...
		{
			SourceElement.CopyFromElement(LensFlare->SourceElement, InProxy->GetSelected());
		}
		...
		for (INT ElementIndex = 0; ElementIndex < LensFlare->Reflections.Num(); ElementIndex++)
		{
			...
			{
				FLensFlareRenderElement* NewElement = new(Reflections)FLensFlareRenderElement(LensFlare->Reflections(ElementIndex), InProxy->GetSelected());
			}
			else
			{
...
UBOOL FLensFlareDynamicData::GetElementValues(FVector& ScreenPosition, FVector& SourcePosition, const FSceneView* View, 
	FLOAT DistanceToSource, FLensFlareRenderElement* Element, FLensFlareElementValues& Values)
{
	check(Element);
...
void FLensFlareDynamicData::SortElements()
{
	...
	if ((SourceElement.LFMaterials.Num() > 0) && (SourceElement.LFMaterials(0)))
	{
...
FLensFlareSceneProxy::~FLensFlareSceneProxy()
{
	...
	ViewOcclusionResources.ReleaseResource();
}
...
UBOOL FLensFlareSceneProxy::UpdateAndRenderOcclusionData(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex,DWORD Flags)
{
	...
	if (State->GetPrimitiveCoveragePercentage(PrimitiveSceneInfo->Component, LocalCoveragePercentage) == TRUE)
	{
		...
		for (INT Index = 0; Index < 8; Index++)
		{
			...
			{
				if (PixelLoc.X < MinScreenPos.X)		{	MinScreenPos.X = PixelLoc.X;	}
				if (PixelLoc.Y < MinScreenPos.Y)		{	MinScreenPos.Y = PixelLoc.Y;	}
				if (PixelLoc.X > MaxScreenPos.X)		{	MaxScreenPos.X = PixelLoc.X;	}
				if (PixelLoc.Y > MaxScreenPos.Y)		{	MaxScreenPos.Y = PixelLoc.Y;	}
			}
		}

		// Clamp them to the 0..1 range
		MinScreenPos.X = Clamp<FLOAT>(MinScreenPos.X, 0.0f, View->SizeX);
		MinScreenPos.Y = Clamp<FLOAT>(MinScreenPos.Y, 0.0f, View->SizeY);
		MaxScreenPos.X = Clamp<FLOAT>(MaxScreenPos.X, 0.0f, View->SizeX);
		MaxScreenPos.Y = Clamp<FLOAT>(MaxScreenPos.Y, 0.0f, View->SizeY);
		...
		if ((Abs(Diff) > GEngine->LensFlareMaxOcclusionIncrement) && (LocalCoveragePercentage > 0.0f))
		{
			CoveragePercentage += (Diff * GEngine->LensFlareOcclusionStepSize);
		}
		else
		{
		...
		// Map it using the user-proved percentage mapping
