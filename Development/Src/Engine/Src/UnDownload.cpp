...
#if PS3
		extern UBOOL GWriteFilesToHost;
		UBOOL SavedGWriteFilesToHost = GWriteFilesToHost;
		GWriteFilesToHost = FALSE;
#endif
...
//@todo JoshA -- Please fix this properly
		if (FileSize > 0)
		{
			...
			if (!RecvFileAr)
			{
				GWriteFilesToHost = SavedGWriteFilesToHost;
...
void UDownload::DownloadDone()
{
	...
	else
	{
		...
		if( !*Error )
		{
			extern void appRenameHDDFiles(const TCHAR* From, const TCHAR* To);
			appRenameHDDFiles(TempFilename, Dest);
		}
#else
