...
UBOOL FTerrainMaterialResource::RequiresSimpleStaticLighting(EMaterialShaderPlatform MatPlatform) const
{ 
	// force simple static lighting for terrain materials
	return( MatPlatform == MSP_SM2 );
}
		...
		for( INT MatIdx=0; MatIdx < CachedMaterials.Num(); MatIdx++ )
		{
			...
			if( CachedMat )
			{
				...
				if (bForceAllPlatforms || PlatformIndex == RequestedMaterialPlatform)
				{
					...
					{
						CachedMat->CacheShaders(CurrentShaderPlatform, bIsSM2Platform);
					}
					else
					{
						CachedMat->InitShaderMap(CurrentShaderPlatform, bIsSM2Platform);
					}
				}
			}
		}
...
FMaterialRenderProxy* ATerrain::GetCachedMaterial(const FTerrainMaterialMask& Mask, UBOOL& bIsTerrainResource)
{
	...
	if( !Result )
	{
		...
		if( Result )
		{
			...
			Result->CacheShaders(GRHIShaderPlatform,MatPlatform == MSP_SM2);
		}
...
public:
	...
	virtual void InitRHI()
	{
		...
		if (MaxSize > 0)
		{
			...
			IndexBufferRHI = RHICreateIndexBuffer(Stride, MaxSize, NULL, RUF_Static);
	...
	virtual void InitDynamicRHI()
	{
		...
		{
			INT Stride = sizeof(WORD);
			IndexBufferRHI = RHICreateIndexBuffer(Stride, MaxSize, NULL, RUF_Dynamic);
			if (bDeferredFillData == FALSE)
			{
				PrimeBuffer();
				FillData();
	...
	, SmoothIndexBuffer( NULL )
{
	...
	//@todo decal - check decal frustum against the heightmap as well
...
FDecalRenderData* FTerrainObject::GenerateDecalRenderData(class FDecalState* Decal) const
{
...
void FTerrainObject::ReleaseResources()
{
	...
	if (CollisionIndexBuffer)
	{
		BeginReleaseResource(CollisionIndexBuffer);
	}
}
...
void FTerrainIndexBuffer::InitRHI()
{
	...
	IndexBufferRHI = RHICreateIndexBuffer(Stride, Size, NULL, RUF_Static);
	...
	for(INT MaterialIndex = 0;MaterialIndex < Terrain->WeightedMaterials.Num();MaterialIndex++)
	{
		...
		for(INT MeshIndex = 0;MeshIndex < TerrainMaterial->FoliageMeshes.Num();MeshIndex++)
		{
			...
			if(Mesh->StaticMesh)
			{
				...
				if(!Material || !Material->CheckMaterialUsage(MATUSAGE_Foliage) || !Material->CheckMaterialUsage(MATUSAGE_StaticLighting))
				{
				...
				// Set the foliage mesh's bUsedWithInstancing flag if it isn't set yet, and this is in the editor.
				if(!Mesh->StaticMesh->bUsedForInstancing && GIsEditor)
				{
					Mesh->StaticMesh->PreEditChange(NULL);
					Mesh->StaticMesh->bUsedForInstancing = TRUE;
					Mesh->StaticMesh->PostEditChange(NULL);
					Mesh->StaticMesh->MarkPackageDirty();
				}
			}
		}
	}
...
void FTerrainComponentSceneProxy::DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex,DWORD Flags)
{
	...
	if ((View->Family->ShowFlags & SHOW_TerrainCollision) && (DPGIndex == SDPG_Foreground))
	{
		if (TerrainObject->CollisionWireframeMaterialInstance == NULL)
		{
			TerrainObject->CollisionWireframeMaterialInstance = ::new FColoredMaterialRenderProxy(
				GEngine->WireframeMaterial->GetRenderProxy(FALSE), 
				GetSelectionColor(FLinearColor(1.0f, 0.2f, 0.2f),TRUE)
				);
			check(TerrainObject->CollisionWireframeMaterialInstance);
		}
		if (TerrainObject->CollisionVertexBuffer == NULL)
		{
			TerrainObject->CollisionVertexBuffer = ::new FTerrainWireVertexBuffer(TerrainObject->TerrainComponent);
			check(TerrainObject->CollisionVertexBuffer);
			TerrainObject->CollisionVertexBuffer->InitResource();
		}

		if (TerrainObject->CollisionVertexBuffer->GetNumVertices())
		{
			if (TerrainObject->CollisionIndexBuffer == NULL)
			{
				TerrainObject->CollisionIndexBuffer = ::new FTerrainWireIndexBuffer(TerrainObject->TerrainComponent);
				check(TerrainObject->CollisionIndexBuffer);
				TerrainObject->CollisionIndexBuffer->InitResource();
			}

			if (TerrainObject->CollisionVertexFactory == NULL)
			{
				TerrainObject->CollisionVertexFactory = ::new FLocalVertexFactory();
				check(TerrainObject->CollisionVertexFactory);

				FLocalVertexFactory::DataType TheData;
				TheData.PositionComponent = STRUCTMEMBER_VERTEXSTREAMCOMPONENT(TerrainObject->CollisionVertexBuffer,FTerrainWireVertex,Position,VET_Float3);
				TheData.TextureCoordinates.AddItem(
					FVertexStreamComponent(TerrainObject->CollisionVertexBuffer,STRUCT_OFFSET(FTerrainWireVertex,UV),sizeof(FTerrainWireVertex),VET_Float2)
					);
				TheData.TangentBasisComponents[0] = STRUCTMEMBER_VERTEXSTREAMCOMPONENT(TerrainObject->CollisionVertexBuffer,FTerrainWireVertex,TangentX,VET_PackedNormal);
				TheData.TangentBasisComponents[1] = STRUCTMEMBER_VERTEXSTREAMCOMPONENT(TerrainObject->CollisionVertexBuffer,FTerrainWireVertex,TangentZ,VET_PackedNormal);
				TerrainObject->CollisionVertexFactory->SetData(TheData);

				TerrainObject->CollisionVertexFactory->InitResource();
			}

			FMeshElement Mesh;
			Mesh.NumPrimitives = TerrainObject->CollisionIndexBuffer->GetNumTriangles();
			if (Mesh.NumPrimitives != 0)
			{
				Mesh.IndexBuffer = TerrainObject->CollisionIndexBuffer;
				...
				Mesh.MaterialRenderProxy = TerrainObject->CollisionWireframeMaterialInstance;
				Mesh.LCI = NULL;
				// Collision vertices are stored in world space.
				Mesh.LocalToWorld = FMatrix::Identity;
				Mesh.WorldToLocal = FMatrix::Identity;
				Mesh.FirstIndex = 0;
				Mesh.MinVertexIndex = 0;
				Mesh.MaxVertexIndex = TerrainObject->CollisionVertexBuffer->GetNumVertices() - 1;
				Mesh.UseDynamicData = FALSE;
				Mesh.ReverseCulling = LocalToWorldDeterminant < 0.0f ? TRUE : FALSE;
				...
				DrawRichMesh(PDI,Mesh,FLinearColor(1.0f, 0.2f, 0.2f),LinearLevelColor,FLinearColor(1.0f, 0.2f, 0.2f),PrimitiveSceneInfo,bSelected);
			}
		}
	}
}
	...
	if (TerrainObject != NULL)
	{
		if(ShowFlags & SHOW_Terrain)
		{
			if (IsShown(View))
			{
				...
				if (ShowFlags & SHOW_TerrainCollision)
				{
...
*/
void FTerrainComponentSceneProxy::GetLightRelevance(FLightSceneInfo* LightSceneInfo, UBOOL& bDynamic, UBOOL& bRelevant, UBOOL& bLightMapped)
{
...
void FTerrainComponentSceneProxy::PreRenderView(const FSceneViewFamily* ViewFamily, const TBitArray<FDefaultBitArrayAllocator>& VisibilityMap, INT FrameNumber)
{
	...
	if (PrimitiveSceneInfo->LastRenderTime == 0.0f)
	{
	...
	if (GWorld && GWorld->GetWorldInfo() && ((GWorld->GetWorldInfo()->TimeSeconds - PrimitiveSceneInfo->LastRenderTime) > 0.25f))
	{
		// Assume this is the first time rendered in a while, so force a tessellation check
		bForceRepack = TRUE;
 		bCheckTessellation = TRUE;
	}
	else
