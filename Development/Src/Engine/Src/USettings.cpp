...
void USettings::SetStringSettingValue(INT StringSettingId,INT ValueIndex,UBOOL bShouldAutoAdd)
{
	...
	{
		delegateNotifySettingValueUpdated(GetStringSettingName(StringSettingId));
	}
}
...
UBOOL USettings::IncrementStringSettingValue(INT StringSettingId,INT Direction,UBOOL bShouldWrap)
{
	...
	for (INT Index = 0; Index < LocalizedSettings.Num(); Index++)
	{
		if (LocalizedSettings(Index).Id == StringSettingId)
		{
			...
			INT NewValueIndex = LocalizedSettings(Index).ValueIndex + Direction;
			INT Max = 0;
				...
				if (LocalizedSettingsMappings(Index).ValueMappings(Index2).Name == ValueName)
				{
					...
					for (INT Index3 = 0; Index3 < LocalizedSettings.Num(); Index3++)
					{
						if (LocalizedSettings(Index3).Id == LocalizedSettingsMappings(Index).Id)
						{
							...
							if ( bValueChanged && DELEGATE_IS_SET(NotifySettingValueUpdated) )
							{
								delegateNotifySettingValueUpdated(StringSettingName);
							}
							return TRUE;
						}
					}
	...
	if (GetPropertyId(PropertyName,PropertyId))
	{
		...
		for (INT Index = 0; Index < Properties.Num(); Index++)
		{
			if (Properties(Index).PropertyId == PropertyId)
			{
				...
				{
					if ( bValueChanged && DELEGATE_IS_SET(NotifyPropertyValueUpdated) )
					{
						delegateNotifyPropertyValueUpdated(PropertyName);
					}
...
void USettings::SetFloatProperty(INT PropertyId,FLOAT Value)
{
	...
	{
		// Set the value
		//@todo ronp - check that value is actually changed before firing notification
		Property->Data.SetData(Value);
		if ( DELEGATE_IS_SET(NotifyPropertyValueUpdated) )
		{
			delegateNotifyPropertyValueUpdated(GetPropertyName(PropertyId));
		}
	}
}
			...
			if ( MetaData->MappingType == PVMT_IdMapped )
			{
				for ( INT ValueIndex = 0; ValueIndex < MetaData->ValueMappings.Num(); ValueIndex++ )
				{
					...
					if ( Mapping.Id == ValueId )
					{
						...
						if ( DELEGATE_IS_SET(NotifyPropertyValueUpdated) )
						{
							delegateNotifyPropertyValueUpdated(Mapping.Name);
						}
						bResult = TRUE;
						break;
					}
	...
	UBOOL bShouldAddIfMissing)
{
	...
	for (INT Index = 0; Index < Settings.Num(); Index++)
	{
		...
		if (Setting != NULL)
		{
			...
			const UBOOL bValueChanged
				=	SourceSetting.Id != Setting->Id
				||	SourceSetting.ValueIndex != Setting->ValueIndex
				||	SourceSetting.AdvertisementType != Setting->AdvertisementType;

			*Setting = SourceSetting;
			if ( bValueChanged && DELEGATE_IS_SET(NotifySettingValueUpdated) )
			{
				delegateNotifySettingValueUpdated(GetStringSettingName(SourceSetting.Id));
			}
		}
	...
	for (INT Index = 0; Index < Props.Num(); Index++)
	{
		...
		if (DestProp != NULL)
		{
			...
			*DestProp = SourceProp;
			...
			if ( DELEGATE_IS_SET(NotifyPropertyValueUpdated) )
			{
				delegateNotifyPropertyValueUpdated(GetPropertyName(SourceProp.PropertyId));
			}
		}
		else if (bShouldAddIfMissing)
		{
