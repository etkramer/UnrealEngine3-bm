...
const VectorRegister GVectorHalf255_W0 = MakeVectorRegister( 127.5f, 127.5f, 127.5f, 0.f );
...
#endif
	VectorToUnpack						= VectorMultiplyAdd( VectorToUnpack, GVectorOneOverHalf255, VectorNegate( VectorOne() ) );
...
FLOAT GMipLevelFadingInRate = 0.3f;

/** How many seconds to fade out one mip-level. */
FLOAT GMipLevelFadingOutRate = 0.1f;
 ...
 */
void FMipBiasFade::SetNewMipCount( FLOAT ActualMipCount, FLOAT TargetMipCount, DOUBLE LastRenderTime )
{
	...
	if ( TotalMipCount == 0 || GIsGame == FALSE || TimeSinceLastRendered >= GMipLevelFadingAgeThreshold )
	{
		// No fading.
	...
	else
	{
		...
		MipCountFadingRate = (MipCountDelta > 0.0f) ? 1.0f/(GMipLevelFadingInRate*MipCountDelta) : -1.0f/(GMipLevelFadingOutRate*MipCountDelta);
	}
}
