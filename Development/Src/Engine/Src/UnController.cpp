...
void AActor::execSuggestTossVelocity( FFrame& Stack, RESULT_DECL )
{
	...
	*(DWORD*)Result = SuggestTossVelocity(&TossVelocity, Destination, Start, TossSpeed, BaseTossZ, DesiredZPct, CollisionSize, TerminalVelocity, OverrideGravityZ);
}
...
void APawn::SetBasedPosition( FBasedPosition& BP, FVector Pos, AActor* ForcedBase )
{
	BP.Set( ((ForcedBase != NULL) ? ForcedBase : Base), Pos );
}

FVector APawn::GetBasedPosition( FBasedPosition BP )
{
	return *BP;
}

void AController::SetBasedPosition( FBasedPosition& BP, FVector Pos, AActor* ForcedBase )
{
	BP.Set( ((ForcedBase != NULL) ? ForcedBase : ((Pawn != NULL) ? Pawn->Base : NULL)), Pos );
}
FVector AController::GetBasedPosition( FBasedPosition BP )
{
	return *BP;
}
...
void AController::execMoveToward(FFrame& Stack, RESULT_DECL)
{
	...
	SetAdjustLocation( GetDestinationPosition(), FALSE );
	Pawn->ClearSerpentine();
	bAdvancedTactics = bStrafe && bAllowAdvancedTactics && ( (PlayerReplicationInfo && PlayerReplicationInfo->bHasFlag) || bForceStrafe || (GWorld->GetNetMode() != NM_Standalone) || (GWorld->GetTimeSeconds() - Pawn->LastRenderTime < 5.f) || bSoaking );
...
void APlayerController::UpdateViewTarget(AActor* NewViewTarget)
{
	...
	if (!LocalPlayerController() && WorldInfo->NetMode != NM_Client)
	{
 ...
 * is returned.
 */
FString APlayerReplicationInfo::GetPlayerAlias()
{
	if ( PlayerAlias != TEXT("") )
	{
		return PlayerAlias;
...
{
	return (PlayerReplicationInfo && PlayerReplicationInfo->Team) ? PlayerReplicationInfo->Team->TeamIndex : 255;
}
