...
void URB_BodyInstance::InitBody(URB_BodySetup* setup, const FMatrix& transform, const FVector& Scale3D, UBOOL bFixed, UPrimitiveComponent* PrimComp, FRBPhysScene* InRBScene)
{
	...
	UBOOL bStatic = (Owner != NULL && (Owner->bStatic || !Owner->bMovable));
		...
		if( NovodexScene )
		{
			...
			if( nActor )
			{
				...
				else
				{
					...
					INT NumShapes = nActor->getNbShapes();
					debugf(TEXT("RELEASE %s"), OwnerComponent ? *OwnerComponent->GetPathName() : TEXT("NONE"));
					UStaticMeshComponent* SMComp = Cast<UStaticMeshComponent>(OwnerComponent);
					if(SMComp && SMComp->StaticMesh)
					{
						debugf(TEXT(" MESH: %s"), *SMComp->StaticMesh->GetPathName());
					}
					for(INT ShapeIdx=0; ShapeIdx<NumShapes; ShapeIdx++)
					{
						NxBounds3 Bounds;
						nShapes[ShapeIdx]->getWorldBounds(Bounds);

						NxVec3 Dim;
						Bounds.getDimensions(Dim);
						Dim *= P2UScale;
						debugf(TEXT(" %d  (%6.2f %6.2f %6.2f)"), ShapeIdx, Dim.x, Dim.y, Dim.z);
					}
#endif
...
FPhysEffectInfo UPhysicalMaterial::FindPhysEffectInfo(BYTE Type)
{
	...
	while( (!Info.Effect || !Info.Sound || Info.Threshold == 0.f || Info.ReFireDelay == 0.f) && TestMat )
	{
		...
		if(!Info.Effect)
		{
			Info.Effect = (Type == EPMET_Impact) ? ImpactEffect : SlideEffect;
		}

		if(!Info.Sound)
		{
			Info.Sound = (Type == EPMET_Impact) ? ImpactSound : SlideSound;
		}

		if(Info.Threshold == 0.f)
		{
			Info.Threshold = (Type == EPMET_Impact) ? ImpactThreshold : SlideThreshold;
		}

		if(Info.ReFireDelay == 0.f)
		{
			Info.ReFireDelay = (Type == EPMET_Impact) ? ImpactReFireDelay : SlideReFireDelay;
		}
