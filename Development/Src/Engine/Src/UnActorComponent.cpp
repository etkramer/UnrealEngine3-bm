...
#define LOG_DETAILED_DO_EVERY_FRAME 0
#define LOG_DETAILED_COMPONENT_UPDATE_STATS !FINAL_RELEASE && (LOOKING_FOR_PERF_ISSUES || 0)
#define LOG_DETAILED_ACTOR_UPDATE_STATS		!FINAL_RELEASE && (LOOKING_FOR_PERF_ISSUES || 0)

/** Global boolean to toggle the log of detailed tick stats. */
/** Needs LOG_DETAILED_COMPONENT_UPDATE_STATS to be 1 **/
UBOOL GLogDetailedComponentUpdateStats = TRUE; 

/** Global boolean to toggle the log of detailed tick stats. */
/** Needs LOG_DETAILED_ACTOR_UPDATE_STATS to be 1 **/
...
#define TRACK_DETAILED_COMPONENT_UPDATE_STATS(Object)	FScopedDetailTickStats DetailedTickStats(GDetailedComponentUpdateStats,Object);
#else
#define TRACK_DETAILED_COMPONENT_UPDATE_STATS(Object)
#endif
...
#define ENABLE_FRAME_COMPUPDATE_DUMP LOOKING_FOR_PERF_ISSUES || (0) 
...
void AActor::UpdateComponentsInternal(UBOOL bCollisionUpdate)
{
	...
	else
	{
		...
		const INT ComponentsNum = Components.Num();
		for(INT ComponentIndex = 0;ComponentIndex < ComponentsNum;ComponentIndex++)
		{
			UActorComponent* Component = Components(ComponentIndex); 
			if( ComponentIndex+2 < ComponentsNum )
			{
...
void UActorComponent::DetachFromAny()
{
	if (IsAttached())
	{
		...
		if (IsAttached())
		{
			...
			appErrorfSlow(NAME_Error, TEXT("ActorComponent::DetachFromAny() failed to detach %s (%s) from %s - missing from AllComponents array and/or SkeletalMeshComponent's Attachments array?"), *GetName(), *GetDetailedInfo(), *Owner->GetName());
		}
	}
}

#if !FINAL_RELEASE && ENABLE_FRAME_COMPUPDATE_DUMP
