...
public:
	...
	*/
	virtual UBOOL RequiresOcclusion(const FSceneView* View) const
	{
		...
		return View->Family->ShowFlags & SHOW_DecalInfo;
	}
		...
		{	
			...
			if ( (View->Family->ShowFlags & SHOW_DecalInfo) && ((View->Family->ShowFlags & SHOW_Game) || !Owner || bSelected)  )
			{
				//if ( DPGIndex == SDPG_World )
				{
					...
					// Upper box.
					PDI->DrawLine( DecalState.FrustumVerts[0], DecalState.FrustumVerts[1], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[1], DecalState.FrustumVerts[2], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[2], DecalState.FrustumVerts[3], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[3], DecalState.FrustumVerts[0], White, SDPG_Foreground );

					// Lower box.
					PDI->DrawLine( DecalState.FrustumVerts[4], DecalState.FrustumVerts[5], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[5], DecalState.FrustumVerts[6], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[6], DecalState.FrustumVerts[7], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[7], DecalState.FrustumVerts[4], White, SDPG_Foreground );

					// Vertical box pieces.
					PDI->DrawLine( DecalState.FrustumVerts[0], DecalState.FrustumVerts[4], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[1], DecalState.FrustumVerts[5], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[2], DecalState.FrustumVerts[6], White, SDPG_Foreground );
					PDI->DrawLine( DecalState.FrustumVerts[3], DecalState.FrustumVerts[7], White, SDPG_Foreground );
...
{
	const UBOOL bShowInEditor = (!HiddenEditor && (!Owner || !Owner->bHiddenEd)) || bForceEnableForSave;
...
void UDecalComponent::Attach()
{
	...
	if ( !GIsUCC && bDetailModeAllowsRendering )
	{
		DetachFromReceivers();		
...
void UDecalComponent::SetParentToWorld(const FMatrix& ParentToWorld)
{
	...
	// for placed decal actors don't keep track of relative decal orientation
	ADecalActorBase* DecalActor = Cast<ADecalActorBase>(Owner);
	if( DecalActor )
	{
		Location = DecalActor->Location;
		Orientation = DecalActor->Rotation;
	}
	else
	{
	...
	if ( DecalMaterial ) 
	{
		...
		for ( INT ReceiverIndex = 0 ; ReceiverIndex < StaticReceivers.Num() ; ++ReceiverIndex )
		{
			...
			if ( Receiver && Receiver->IsAttached() && Receiver->GetScene() == GetScene() )
			{
				...
				// @todo decal: Fully attach lit decals on static meshes until 1D lightmap serialization issue is fixed.
				const UBOOL bRecomputeDecal = 
					(bLitDecalMaterial && Receiver->IsA(UStaticMeshComponent::StaticClass())) ||
					Receiver->IsA(UTerrainComponent::StaticClass());
				...
				if( bRecomputeDecal ) 
				{
					...
					if( DecalRenderData )
					{
						if( DecalRenderData->NumTriangles > 0 )
						{
		...
		else
		{
			...
			if( bProjectOnNonBSP )
			{
				...
				for( FCheckResult* HitResult = ActorResult ; HitResult ; HitResult = HitResult->GetNext() )
				{
					...
					if( HitComponent && 
						(HitComponent->DetailMode <= GSystemSettings.DetailMode) &&
						((HitComponent->bAcceptsStaticDecals && (bStaticDecal || bMovableDecal)) || (HitComponent->bAcceptsDynamicDecals && bHasBegunPlay && !bStaticDecal)) )
					{
...
void UDecalComponent::AttachReceiver(UPrimitiveComponent* Receiver)
{
	...
	const UBOOL bHasBegunPlay = GWorld->HasBegunPlay();
	if( Receiver && 
		((Receiver->bAcceptsStaticDecals && (bStaticDecal || bMovableDecal)) || (Receiver->bAcceptsDynamicDecals && bHasBegunPlay && !bStaticDecal)) &&
		...
		if( OwnerLevel )
		{
			...
			for ( INT ReceiverIndex = 0 ; ReceiverIndex < DecalReceivers.Num(); ++ReceiverIndex )
			{
					...
					DecalReceiver.RenderData->NumTriangles > 0 )
				{
					...
					CopyDecalRenderDataToStaticReceiverData( *NewStaticReceiver, *DecalReceiver.RenderData, DecalInteraction );
...
void UDecalComponent::CaptureDecalState(FDecalState* DecalState) const
{
	...
	{
		warnf( TEXT("Decal (%s) was not correct and DefaultDecalMaterial was used!"), *this->GetFullName() );
	...
	{
		warnf( TEXT("Decal (%s) was not correct and DefaultDecalMaterial was used!"), *DecalState->DecalMaterial->GetFullName() );
		DecalState->DecalMaterial = GEngine->DefaultDecalMaterial;
	}
