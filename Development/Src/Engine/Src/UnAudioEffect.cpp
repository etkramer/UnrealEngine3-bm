...
LONG FAudioEffectsManager::VolumeToMilliBels( FLOAT Volume )
{
	...
	if( Volume > 0.0f )
	{
		MilliBels = Clamp<LONG>( ( LONG )( 2000.0f * log10f( Volume ) ), -10000, 0 );
	}
...
void FAudioEffectsManager::Interpolate( FAudioEQEffect& Current, FAudioEQEffect& Start, FAudioEQEffect& End )
{
	...
	{
		InterpValue = ( FLOAT )( ( appSeconds() - Start.RootTime ) / ( End.RootTime - Start.RootTime ) );
	}
...
FAudioEffectsManager::FAudioEffectsManager( UAudioDevice* InDevice )
{
	...
	CurrentMode = NULL;
	USoundMode* Mode = AudioDevice->SoundModes.FindRef( FName( TEXT( "Default" ) ) );
...
void FAudioEffectsManager::SetModeSettings( USoundMode* NewMode )
{
	if( NewMode && NewMode != CurrentMode )
	{
		...
		SourceEQEffect.RootTime = appSeconds();
		...
		{
			USoundMode* DefaultMode = AudioDevice->SoundModes.FindRef( FName( TEXT( "Default" ) ) );
			DestinationEQEffect = DefaultMode->EQSettings;
		}

		DestinationEQEffect.RootTime = SourceEQEffect.RootTime + NewMode->FadeInTime;
