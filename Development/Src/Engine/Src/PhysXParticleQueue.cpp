...
BOOL FPhysXParticleQueue::RemoveParticleById(WORD Id, BYTE* InIndexBase, UINT InIndexStride)
{
...
BOOL FPhysXParticleQueue::RemoveParticleById(WORD Id, QueueParticle& Removed, BYTE* InIndexBase, UINT InIndexStride)
{
	WORD Index = FindHeapIndex(Id);
	if (Index == 0)
		return FALSE;
...
BOOL FPhysXParticleQueue::RemoveParticle(WORD HeapIndex, QueueParticle& Removed, BYTE* InIndexBase, UINT InIndexStride)
{
	check(HeapSize > 1);
...
BOOL FPhysXParticleQueue::RemoveParticle(QueueParticle& Removed, BYTE* InIndexBase, UINT InIndexStride)
{
	if(HeapSize <= 1)
		return FALSE;
...
BOOL FPhysXParticleQueue::Front(QueueParticle& Removed)
{
	if(HeapSize <= 1)
		return FALSE;
