...
static UBOOL GenerateConstrainedPath( APawn *Pawn, ANavigationPoint *Start )
 {
	...
	{
		debugf(NAME_Warning,TEXT("[%f] GenerateConstrainedPath... Initial abort! %s goal %s"),Pawn->WorldInfo->TimeSeconds, *Pawn->GetName(),*Pawn->PathGoalList->GetName());
...
ANavigationPoint* APawn::FindAnchor(AActor* TestActor, const FVector& TestLocation, UBOOL bStartPoint, UBOOL bOnlyCheckVisible, FLOAT& Dist)
{
	...
	for (INT i = 0; i < Objects.Num(); i++)
	{
		...
		if (Nav != NULL 
			...
			&& Abs<FLOAT>(Nav->Location.Z - Location.Z) < MaxStepHeight)
		{
			Dist = 0.f;
			return Nav;
		}
		else
		{
				...
				Spec->supports(Radius, Height, MoveFlags, iMaxFallSpeed))
			{
				...
				if (StartDistSquared < EndDistSquared && Spec->Start->IsUsableAnchorFor(this) && HasReachSpecForAnchoring(Spec->End.Nav(), Spec->Start) && Abs<FLOAT>(Spec->Start->Location.Z - Location.Z) < MaxStepHeight) // only consider Start if there's a reverse ReachSpec
				{
				...
				else if (EndDistSquared < BestReachSpecPointDistSquared && Spec->End.Nav()->IsUsableAnchorFor(this) && Abs<FLOAT>(Spec->End.Nav()->Location.Z - Location.Z) < MaxStepHeight)
				{
...
UBOOL APawn::GeneratePath()
{
	...
	FVector RealLocation = Location;
	FLOAT	StartDist = 0.f;
	UBOOL	bOnlyCheckVisible = (Physics == PHYS_RigidBody);
	FLOAT	EndDist = 0.f;
...
UBOOL UGoal_AtActor::InitialAbortCheck( ANavigationPoint* Start, APawn* Pawn )
{
	...
	if( Start != NULL && Start->IsOnDifferentNetwork( GoalNav ) )
	{
		...
		debugf(TEXT("Attempt to find path from %s to %s failed because they are on unconnected networks (%i/%i)"),*Start->GetFullName(),*GoalNav->GetFullName(),Start->NetworkID,GoalNav->NetworkID);
