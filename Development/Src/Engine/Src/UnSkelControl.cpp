...
FORCEINLINE FLOAT USkelControlBase::GetBlendControlAlpha(FLOAT ControlAlpha)
{
	switch( BlendType )
	{
	case ABT_Sinusoidal			: return Clamp<FLOAT>((appSin(ControlAlpha * PI - HALF_PI) + 1.f) / 2.f, 0.f, 1.f); 
	case ABT_Cubic				: return Clamp<FLOAT>(CubicInterp<FLOAT>(0.f, 0.f, 1.f, 0.f, ControlAlpha), 0.f, 1.f); 
	case ABT_EaseInOutExponent2	: return Clamp<FLOAT>(FInterpEaseInOut(0.f, 1.f, ControlAlpha, 2), 0.f, 1.f); 
	case ABT_EaseInOutExponent3	: return Clamp<FLOAT>(FInterpEaseInOut(0.f, 1.f, ControlAlpha, 3), 0.f, 1.f); 
	case ABT_EaseInOutExponent4	: return Clamp<FLOAT>(FInterpEaseInOut(0.f, 1.f, ControlAlpha, 4), 0.f, 1.f); 
	case ABT_EaseInOutExponent5	: return Clamp<FLOAT>(FInterpEaseInOut(0.f, 1.f, ControlAlpha, 5), 0.f, 1.f); 
	}
...
UBOOL USkelControlLookAt::CanLookAtPoint(FVector PointLoc, UBOOL bDrawDebugInfo, UBOOL bDebugUsePersistentLines, UBOOL bDebugFlushLinesFirst)
{
	...
	if( (GWorld->GetWorldInfo()->TimeSeconds - LastCalcTime) < 1.0f )
	{
		if( bLimitBasedOnRefPose )
		{
			...
			const FMatrix BoneRefPose	= FQuatRotationTranslationMatrix(SkelComponent->SkeletalMesh->RefSkeleton(ControlBoneIndex).BonePos.Orientation, SkelComponent->SkeletalMesh->RefSkeleton(ControlBoneIndex).BonePos.Position) * SkelComponent->SpaceBases(ParentIndex);
...
//	CanLookAtPoint(DesiredTargetLocation, TRUE, TRUE, TRUE);
	...
	if( !bAllowRotationX || !bAllowRotationY || !bAllowRotationZ )
	{
		FMatrix ComponentToAllowFrame = SkelComp->CalcComponentToFrameMatrix(BoneIndex, AllowRotationSpace, AllowRotationOtherBoneName);
		ComponentToAllowFrame.RemoveScaling();
		FQuat CompToFrameQuat(ComponentToAllowFrame);
...
FMatrix USkelControlLookAt::GetWidgetTM(INT WidgetIndex, USkeletalMeshComponent* SkelComp, INT BoneIndex)
{
	...
	FVector WorldLookTarget = SkelComp->LocalToWorld.TransformFVector( ComponentToFrame.InverseSafe().TransformFVector(TargetLocation) );
...
void USkelControlLimb::CalculateNewBoneTransforms(INT BoneIndex, USkeletalMeshComponent* SkelComp, TArray<FMatrix>& OutBoneTransforms)
{
	...
	const FVector	JointTargetPos				= JointTargetComponentToFrame.InverseSafe().TransformFVector(JointTargetLocation);
		...
		if( ScaleRange > KINDA_SMALL_NUMBER && MaxLimbLength > KINDA_SMALL_NUMBER )
		{
			...
			if( ScalingFactor > KINDA_SMALL_NUMBER )
			{
				...
				{	
					...
					{
						SkelComp->LocalAtoms(RollBoneIndex).Translation *= (1.f + ScalingFactor * ControlStrength);
					}
				}
			}
		}
	...
	if( bTakeRotationFromEffectorSpace )
	{
		...
		FMatrix FrameToComponent = DesiredComponentToFrame.InverseSafe();
...
void USkelControlFootPlacement::CalculateNewBoneTransforms(INT BoneIndex, USkeletalMeshComponent* SkelComp, TArray<FMatrix>& OutBoneTransforms)
{
	...
	if(bOrientFootToGround && bHit && ((FootPos - DesiredPos).Size() < 1.0f) && !HitNormal.IsZero())
	{
		...
		FQuatRotationTranslationMatrix TempFootTM( DeltaFootQuat, FVector(0.f) );
...
void USkelControlWheel::CalculateNewBoneTransforms(INT BoneIndex, USkeletalMeshComponent* SkelComp, TArray<FMatrix>& OutBoneTransforms)
{
	...
	BoneRotation = FRotationMatrix(TotalRot).Rotator();
...
void USkelControlTrail::CalculateNewBoneTransforms(INT BoneIndex, USkeletalMeshComponent* SkelComp, TArray<FMatrix>& OutBoneTransforms)
{
	...
	if(!FakeVelocity.IsZero())
	{
		...
		{
			const FMatrix ActorToWorld = FRotationTranslationMatrix(SkelComp->GetOwner()->Rotation, SkelComp->GetOwner()->Location);
