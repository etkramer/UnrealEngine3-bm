...
void USkeletalMeshComponent::UpdateFixedClothVerts()
{
	...
	for(INT i=SkeletalMesh->NumFreeClothVerts; i<SkeletalMesh->ClothToGraphicsVertMap.Num(); i++)
	{
		...
		else
		{
			...
			const BYTE&			BoneIndex = Chunk.BoneMap(SrcRigidVertex->InfluenceBones[RIGID_INFLUENCE_INDEX]);
			const FMatrix		RefToLocal = SkeletalMesh->RefBasesInvMatrix(BoneIndex) * SpaceBases(BoneIndex);
		...
		{
			ClothVertIdx = SkeletalMesh->ClothWeldingMap(ClothVertIdx);
		}

		// Update cloth vertex location
		nCloth->attachVertexToGlobalPosition(ClothVertIdx, U2NPosition(WorldPos)); 
	}
...
{
	{
		...
		ClothValidBounds.set(U2NPosition(ClothValidBoundsMin),U2NPosition(ClothValidBoundsMax));
...
{
	// When unfreezing (untorn) cloth, first teleport vertices to ref pose
	if(bClothFrozen && !bNewFrozen && SkeletalMesh && !SkeletalMesh->bEnableClothTearing)
	{
		ResetClothVertsToRefPose();
	}
 ...
 */
void USkeletalMeshComponent::HideBone( INT BoneIndex, UBOOL bTermPhysicsBelow )
{
	if ( BoneIndex != INDEX_NONE )
	{
		LocalAtoms( BoneIndex ).Scale = 0.0f;
		BoneVisibility( BoneIndex ) = 0;

		if(bTermPhysicsBelow && PhysicsAssetInstance)
		{
...
void USkeletalMeshComponent::UnHideBone( INT BoneIndex )
{
	if ( BoneIndex != INDEX_NONE )
	{
		LocalAtoms( BoneIndex ).Scale = 1.0f;
...
void USkeletalMeshComponent::UpdateMeshForBrokenConstraints()
{
	...
	DEBUGBROKENCONSTRAINTUPDATE(debugf(TEXT("%3.3f UGearAnim_BaseBlendNode::InitAnim"), GWorld->GetTimeSeconds());)
...
void UStaticMeshComponent::DisableRBCollisionWithSMC(UStaticMeshComponent* OtherSMC, UBOOL bDisabled)
{
...
{
	P_GET_OBJECT(UStaticMeshComponent, OtherSMC);
