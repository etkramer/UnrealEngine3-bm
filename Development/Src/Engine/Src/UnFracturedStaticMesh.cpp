...
void UFracturedStaticMesh::PostLoad()
{
	...
	if (!GIsEditor || GCookingTarget & UE3::PLATFORM_Console)
	{
...
void UFracturedStaticMesh::PostEditChange(UProperty* PropertyThatChanged)
{
...
void UFracturedBaseComponent::PostEditChange(UProperty* PropertyThatChanged)
{	
...
typedef TkDOPTree<class FFracturedStaticMeshCollisionDataProvider,WORD> kDOPTreeTypeFractured;
...
UBOOL UFracturedStaticMeshComponent::LineCheck(FCheckResult& Result,const FVector& End,const FVector& Start,const FVector& Extent,DWORD TraceFlags)
{
	...
	else if (StaticMesh->kDOPTree.Nodes.Num())
	{
		...
		if (bZeroExtent)
		{
			...
			TkDOPLineCollisionCheck<FFracturedStaticMeshCollisionDataProvider,WORD> kDOPCheck(Start,End,TraceFlags,Provider,&Result);
			// Do the line check
		...
		{
			// Create the check structure with all the local space fun
			TkDOPBoxCollisionCheck<FFracturedStaticMeshCollisionDataProvider,WORD> kDOPCheck(Start,End,Extent,TraceFlags,Provider,&Result);
...
UBOOL UFracturedStaticMeshComponent::PointCheck(FCheckResult& Result,const FVector& Location,const FVector& Extent,DWORD TraceFlags)
{
	...
	{ 
		...
		// Create the check structure with all the local space fun
		TkDOPPointCollisionCheck<FFracturedStaticMeshCollisionDataProvider,WORD> kDOPCheck(Location,Extent,Provider,&Result);
...
FDecalRenderData* UFracturedStaticMeshComponent::GenerateDecalRenderData(class FDecalState* Decal) const
{
	...
	TArray<WORD> Leaves;
	TkDOPFrustumQuery<FFracturedStaticMeshCollisionDataProvider,WORD> kDOPQuery( 
		static_cast<FPlane*>( Decal->Planes.GetData() ),
		Decal->Planes.Num(),
		Leaves,
		MeshData );	
	...
	{
		const kDOPTreeTypeFractured::NodeType& Node		= kDOPQuery.Nodes(Leaves(LeafIndex));
...
void UFracturedSkinnedMeshComponent::SetFragmentTransform(INT FragmentIndex, const FMatrix& LocalToWorld)
{
	...
	check(FragmentIndex >= 0 && FragmentIndex < VisibleFragments.Num());
	FragmentTransforms(FragmentIndex) = LocalToWorld;
...
void UFracturedSkinnedMeshComponent::RemoveDependentComponent(UFracturedStaticMeshComponent* InComponent)
{
	...
	{
		if (Owner && !CastChecked<AFracturedStaticMeshActor>(Owner)->bBreakChunksOnPawnTouch)
		{
	...
	if (StaticMesh)
	{
		...
		if (bBecameVisible)
		{
				...
				&& BaseComponent->IsAttached())
			{
				...
				DynamicLightEnv->bOverrideOwnerBounds = TRUE;
...
FLightRayIntersection FFracturedStaticLightingMesh::IntersectLightRay(const FVector& Start,const FVector& End,UBOOL bFindNearestIntersection) const
{
	...
	TkDOPLineCollisionCheck<FFracturedStaticMeshCollisionDataProvider,WORD> kDOPCheck(Start,End,!bFindNearestIntersection ? TRACE_StopAtAnyHit : 0,Provider,&Result);
...
void AFracturedStaticMeshActor::BreakOffPartsInRadius(FVector Origin, FLOAT Radius, FLOAT RBStrength, UBOOL bWantPhysChunksAndParticles)
{
	...
	{
		FracturedStaticMeshComponent->SetMaterial(FracMesh->OutsideMaterialIndex, FracMesh->LoseChunkOutsideMaterial);
	}	
...
AFracturedStaticMeshPart* AFracturedStaticMeshActor::SpawnPartMulti(const TArray<INT>& ChunkIndices, FVector InitialVel, FVector InitialAngVel, FLOAT RelativeScale, UBOOL bExplosion)
{
	...
	// Propagate bBreakChunksOnPawnTouch option if this is a composite chunk
	if(bBreakChunksOnPawnTouch && ChunkIndices.Num() > 1)
	{
		NewPart->bBreakChunksOnPawnTouch = TRUE;
	}
...
void AFracturedStaticMeshActor::RecreatePhysState()
{
	...
	if(FracturedStaticMeshComponent->BodyInstance)
	{
		// Backup angular vel
		if(FracturedStaticMeshComponent->BodyInstance->IsValidBodyInstance())
		{
			AngVel = FracturedStaticMeshComponent->BodyInstance->GetUnrealWorldAngularVelocity();
		}
	...
	FracturedStaticMeshComponent->InitComponentRBPhys(Physics != PHYS_RigidBody);
	// Restore ang vel
	FracturedStaticMeshComponent->SetRBAngularVelocity(AngVel, FALSE);
	// Wake up
	FracturedStaticMeshComponent->WakeRigidBody();
}
...
void AFracturedStaticMeshActor::TickSpecial(FLOAT DeltaSeconds)
{
	...
	if(bBreakChunksOnPawnTouch)
	{
		...
		for(INT TouchIdx=0; TouchIdx<Touching.Num(); TouchIdx++)
		{
			...
			{
				APawn* P = Touching(TouchIdx)->GetAPawn();
				if(P)
				{
					...
					for(INT ChunkIdx=0; ChunkIdx<FragmentVis.Num(); ChunkIdx++)
					{
						if((FragmentVis(ChunkIdx) != 0) && (ChunkIdx != FracturedStaticMeshComponent->GetCoreFragmentIndex()))
						{
							...
							UBOOL bHit = !P->CollisionComponent->PointCheck(Result, FracCenter, FracBox.GetExtent(), TRACE_Pawns);
							if(bHit)
							{
								FVector AngAxis = -1.f * ((FracCenter - P->Location) ^ (P->Velocity)).SafeNormal();

								FVector PartVel = (1.f + (appFrand() * 1.f)) * P->Velocity; // Between 1 and 2 times pawn velocity
								FLOAT PartScale = FracMesh->NormalPhysicsChunkScaleMin + appFrand() * (FracMesh->NormalPhysicsChunkScaleMax - FracMesh->NormalPhysicsChunkScaleMin);
...
{
	return !bBreakChunksOnPawnTouch;
}

void AFracturedStaticMeshActor::PostEditChange(UProperty* PropertyThatChanged)
{
...
UPhysicalMaterial* AFracturedStaticMeshActor::GetFracturedMeshPhysMaterial()
{
	FCheckResult Check(0.f);
	Check.Actor = this;
	Check.Component = FracturedStaticMeshComponent;
...
UBOOL AFracturedStaticMeshPart::InStasis()
{
	return bHasBeenRecycled || AActor::InStasis();
}
...
void AFracturedStaticMeshPart::OnRigidBodyCollision(const FRigidBodyCollisionInfo& Info0, const FRigidBodyCollisionInfo& Info1, const FCollisionImpactData& RigidCollisionData)
{
...
void AFractureManager::CreateFSMParts()
{
	...
	FVector SpawnLocation = FVector(250000.f, 250000.f, 250000.f);
