...
*/
FRotator AActor::FindSlopeRotation(FVector const& FloorNormal, FRotator const& NewRotation)
{
...
void APawn::physWalking(FLOAT deltaTime, INT Iterations)
{
	...
	{
		DesiredMove += PhysicsVolume->ZoneVelocity * 25.f * deltaTime;
	}
	DesiredMove.Z = 0.f;
	...
	while ( (remainingTime > 0.f) && (Iterations < 8) && (Controller || bRunPhysicsWithNoController) )
	{
		...
		if ( NewBase )
		{
			...
			FVector ColLocation = CollisionComponent ? CollisionComponent->Bounds.Origin : Location;
		...
		else
		{
				  ...
				  (!bCanWalkOffLedges && (bIsWalking || bIsCrouched) ) ) ) )
			{
				...
				if (OldBase != NULL && (OldBase->bStatic || OldBase->bWorldGeometry || !OldBase->bMovable || (OldBase->IsEncroacher() && OldBase->Location == OldBaseLocation)))
				{
...
{
	AActor* const HitActor = WallHit.Actor;
	if ( !HitActor )
	{
		return;
	}
...
UBOOL AActor::MoveWithInterpMoveTrack(UInterpTrackMove* MoveTrack, UInterpTrackInstMove* MoveInst, FLOAT CurInterpTime, FLOAT DeltaTime)
{
	...
	UBOOL bIsTrackEnabled = !MoveTrack->bDisableTrack;
	...
	AdjustInterpTrackMove(NewPos, NewRot, DeltaTime);
