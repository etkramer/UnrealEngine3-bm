...
void ANxRadialForceField::PostEditChange(UProperty* PropertyThatChanged)
{
	if(RenderComponent)
	{
...
/** Update the render component to match the force radius. */
void ANxCylindricalForceFieldCapsule::PostEditChange(UProperty* PropertyThatChanged)
{
	if(RenderComponent)
	{
		{
...
//#define ConvexMeshReferenceBugTest
			...
			assert(beforeForceFieldDestroy - 1  == afterForceFieldDestroy);
#else
			// (2)
			ForceField->Destroy();
#endif
...
void ANxGenericForceFieldBox::InitRBPhys()
{
	Super::InitRBPhys();
}

void ANxGenericForceFieldBox::TermRBPhys(FRBPhysScene* Scene) 
{
	Super::TermRBPhys(Scene);
}

void ANxGenericForceFieldBox::TickSpecial(FLOAT DeltaSeconds)
{
	Super::TickSpecial(DeltaSeconds);
}
...
/** Update the render component to match the force radius. */
void ANxGenericForceFieldBox::PostEditChange(UProperty* PropertyThatChanged)
{
	if(RenderComponent)
	{
		FComponentReattachContext ReattachContext(RenderComponent);
...
void ANxGenericForceFieldCapsule::InitRBPhys()
{
	Super::InitRBPhys();
}

void ANxGenericForceFieldCapsule::TermRBPhys(FRBPhysScene* Scene) 
{
	Super::TermRBPhys(Scene);
}

void ANxGenericForceFieldCapsule::TickSpecial(FLOAT DeltaSeconds)
{
	Super::TickSpecial(DeltaSeconds);
}
...
/** Update the render component to match the force radius. */
void ANxGenericForceFieldCapsule::PostEditChange(UProperty* PropertyThatChanged)
{
	if(RenderComponent)
	{
		FComponentReattachContext ReattachContext(RenderComponent);
