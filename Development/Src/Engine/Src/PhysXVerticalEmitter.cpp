...
void FPhysXVerticalEmitter::Tick(FLOAT DeltaTime)
{
	...
	else
	{
		...
		AWorldInfo* Info = (AWorldInfo*)(AWorldInfo::StaticClass()->GetDefaultObject());
		FLOAT MaxDeltaTime = Info->MaxPhysicsDeltaTime;
		FLOAT PhysXDT = ::Min(DeltaTime, MaxDeltaTime);
		TickRBPhysScene(Ps.CascadeScene, PhysXDT, Info->PhysicsProperties.PrimaryScene.TimeStep, Info->PhysicsProperties.PrimaryScene.bFixedTimeStep);
