...
INT UMantleReachSpec::CostFor(APawn* P)
{
	...
	//@FIXME: StartMarker should be cached
	ACoverSlotMarker* StartMarker = Cast<ACoverSlotMarker>(Start);
	if (StartMarker == NULL)
	{
	//	warnf(TEXT("WARNING! MantleReachSpec's start node is not a coverslot marker?!? wtf mate (start was %s) for %s"),(Start) ? *Start->GetName() : TEXT("NULL"), *P->GetName());
	}
	// if the slot is disabled, we can't get in cover and therefore can't mantle
	else if ( StartMarker->OwningSlot.Link != NULL &&
			(!StartMarker->OwningSlot.Link->IsEnabled() || !StartMarker->OwningSlot.Link->Slots(StartMarker->OwningSlot.SlotIdx).bEnabled) )
	{
		return UCONST_BLOCKEDPATHCOST;
	}
...
INT UCoverSlipReachSpec::CostFor(APawn *P)
{
	...
	else
	{
		...
		if(StartMarker == NULL)
		{
			warnf(TEXT("WARNING! CoverSlipReachSpec's start node is not a coverslot marker?!? wtf mate (start was %s)"),(Start) ? *Start->GetName() : TEXT("NULL"));
		}
		
		// only let guys cover slip from links they are taking cover in (doesn't make sense to get into cover just to cover slip)
		if (StartMarker != NULL && StartMarker->OwningSlot.Link != NULL && !StartMarker->OwningSlot.Link->Claims.ContainsItem(P))
		{
			return UCONST_BLOCKEDPATHCOST;
		}
...
UBOOL UReachSpec::PrepareForMove(AController* C)
{
	...
	if (bCheckForObstructions && C != NULL && C->Pawn != NULL && C->Pawn->CylinderComponent != NULL)
	{
		...
		for (FCheckResult* CheckHit = Hits; CheckHit != NULL && !bResult; CheckHit = CheckHit->GetNext())
		{
			if ( CheckHit->Actor != NULL && !CheckHit->Actor->bStatic && !C->ShouldIgnoreNavigationBlockingFor(CheckHit->Actor) && CheckHit->Actor != Start && CheckHit->Actor != End &&
				(Dir | (CheckHit->Actor->Location - Start->Location).SafeNormal()) > KINDA_SMALL_NUMBER )
			{
...
{
	if(Start && Start->bCrowdPath && End.Nav() && End.Nav()->bCrowdPath)
	{
		return FPlane(1.0f, 1.0f, 1.0f, 0.f);
	}
...
UBOOL UReachSpec::ShouldPruneAgainst( UReachSpec* Spec )
{
	if( !bPruned && !bSkipPrune && *End != NULL )
	{
		...
		// if either end is a coverslotmarker, don't check width first.. just do it!
		if((*End && End->IsA(ACoverSlotMarker::StaticClass())))
		{
			return TRUE;
		}
		else
...
FVector UMantleReachSpec::GetForcedPathSize( ANavigationPoint* Start, ANavigationPoint* End, AScout* Scout )
{
	...
	ACoverSlotMarker* StartCov = Cast<ACoverSlotMarker>(Start);
	ACoverSlotMarker* EndCov = Cast<ACoverSlotMarker>(End);

	if(StartCov == NULL || EndCov == NULL)
	{
		return Super::GetForcedPathSize(Start,End,Scout);
	}

	FVector SrcLocation = StartCov->GetSlotLocation();
	FVector DestLocation = EndCov->GetSlotLocation();

	FVector BiggestToPass = FVector(-1.f);
	INT StartIdx = Scout->PathSizes.Num() - 1;
	if( appStricmp( appGetGameName(), TEXT("Gear") ) == 0 )
	{
		StartIdx = 2;
	}	
	...
	{
		FPathSizeInfo& CurrentSizeInfo = Scout->PathSizes(Idx);
		FCheckResult Hit;
		FVector Extent(CurrentSizeInfo.Radius);
		FVector Start = SrcLocation + FVector(0,0,1) * (CurrentSizeInfo.Radius + StartCov->OwningSlot.Link->MidHeight);
		FVector	End	  = FVector(DestLocation.X, DestLocation.Y, Start.Z);

		if( GWorld->SingleLineCheck( Hit, StartCov, Start, End, TRACE_World|TRACE_StopAtAnyHit, Extent ) )
		{
			if(!Hit.bStartPenetrating)
			{
				BiggestToPass.X = Max<FLOAT>(BiggestToPass.X, CurrentSizeInfo.Radius);
				BiggestToPass.Y = Max<FLOAT>(BiggestToPass.Y, CurrentSizeInfo.Height);
			}
		}
	}

	if(BiggestToPass.X < 0.f)
	{
...
INT USlotToSlotReachSpec::defineFor( ANavigationPoint* begin, ANavigationPoint* dest, APawn* Scout )
{
	...
	ACoverLink*		  Link		  = BeginMarker->OwningSlot.Link;
	...
	if( Link->bLooped && BeginIdx == 0 && DestIdx == LastIdx )
	{
		SpecDirection = CD_Left;
	}
	else
	// If spec is looping from last to first slot
	if( Link->bLooped && BeginIdx == LastIdx && DestIdx == 0 )
	{
...
INT USwatTurnReachSpec::defineFor( ANavigationPoint* begin, ANavigationPoint* dest, APawn* Scout )
{
	...
	if( Result )
	{
		...
		if( StartMarker != NULL )
		{
			SpecDirection = ((FRotationMatrix(StartMarker->GetSlotRotation()).GetAxis(1) | GetDirection()) > 0) ? CD_Right : CD_Left;
		}
	}
