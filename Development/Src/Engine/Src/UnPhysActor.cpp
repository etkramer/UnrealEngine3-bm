...
void AActor::SyncActorToRBPhysics()
{
	...
	else
	{
		...
		else
		{
			...
			debugf( TEXT("AActor::SyncActorToRBPhysics: Invalid or Missing BodyInstance in CollisionComponent. (check to make certain your object actually has a collision volume) for: %s  CollisionComp: %s  DetailedInfo: %s"), *GetFullName(), *SpecificName, *GetDetailedInfo() );
		}
#endif //!FINAL_RELEASE
	}
...
void AKActor::TickSpecial(FLOAT DeltaSeconds)
{
	...
	if(bEnableStayUprightSpring)
	{
		...
		if(CrossMag > KINDA_SMALL_NUMBER)
		{
			...
			// Scale and clamp
			FLOAT UprightTorqueMag = ::Clamp(StayUprightTorqueFactor * LevelAngle, -StayUprightMaxTorque, StayUprightMaxTorque);
...
void AKActor::physRigidBody(FLOAT DeltaTime)
{
	...
	else
	{
		...
		if (RBState.bNewData & UCONST_RB_Sleeping)
		{
#if WITH_NOVODEX
			if (CollisionComponent != NULL)
			{
				NxActor* nActor = CollisionComponent->GetNxActor();
				if (nActor != NULL && !nActor->isSleeping())
				{
					RBState.bNewData |= UCONST_RB_NeedsUpdate;
				}
			}
	...
	if(bLimitMaxPhysicsVelocity)
	{
		...
		{
			FVector NewVel = (Velocity/VMag) * MaxPhysicsVelocity;
...
void AActor::OnRigidBodyCollision(const FRigidBodyCollisionInfo& Info0, const FRigidBodyCollisionInfo& Info1, const FCollisionImpactData& RigidCollisionData)
{
	...
	if(TRUE)
	{
		FString Actor0Name = Info0.Component ? FString(*Info0.Component->GetPathName()) : FString(TEXT(""));
		FString Actor1Name = Info1.Component ? FString(*Info1.Component->GetPathName()) : FString(TEXT(""));
