...
#else
,	YFirstIndexBuffer( InSimulationQuadsX * InSimulationQuadsY * 6, FALSE, sizeof(DWORD) )
,	XFirstIndexBuffer( InSimulationQuadsX * InSimulationQuadsY * 6, FALSE, sizeof(DWORD) )
#endif
,	SimulationIndex( 0 )
...
/** Called from the worker thread, the last call before NULL:ing out the work pointer. */
void FFluidSimulation::Dispose()
{
...
void FFluidSimulation::RenderThreadTick( FLOAT InDeltaTime )
{
	...
	// Render the full simulation geometry or a simple flat quad.
...
void FFluidSimulation::InitIndexBufferX()
{
	...
	DWORD* Indices = (DWORD*) YFirstIndexBuffer.Lock();
	DWORD* CurrentIndices = Indices;
	DWORD IndexOrigin	= 0;
...
{
	// Generate the indices on the grid, iterating over the grid cells.
	DWORD* Indices = (DWORD*) XFirstIndexBuffer.Lock();
	DWORD* CurrentIndices = Indices;
	INT NumX = GetNumCellsX();
	INT NumY = GetNumCellsY();
	DWORD IndexOrigin	= 0;
...
FTexture2DResourceMem* FFluidSimulation::CreateTextureResourceMemory()
{
	...
	FXeTexture2DResourceMem* XeResourceMem = new FXeTexture2DResourceMem(SizeX, SizeY, 1, PF_FloatRGBA, TexCreate_Dynamic|TexCreate_NoTiling);

	// Did it fail?
	if ( XeResourceMem->IsValid() == FALSE )
	{
		delete XeResourceMem;
		XeResourceMem = NULL;
	}
