...
void UDebuggerCore::RefreshWatch( const FStackNode* CNode )
{
	...
	TFieldIterator<UProperty,CASTCLASS_UProperty> PropertyIt(ContextObject->GetClass());
	...
	case DI_PrevStackState:
		{
			...
			{
				// check if the specified frame is actually in the stack
				// if not, just ignore this command
				for (INT i = 0; i < Stack.Num(); i++)
				{
					if (AreStackFramesIdentical(Stack(0).StackNode, CurrentFrame))
					{
						FStackNode& CurrentTop = Stack.Last();
