...
UBOOL UOnlineNewsInterfaceMcp::ReadNews(BYTE LocalUserNum,BYTE NewsType)
{
		...
		NewsCacheEntry->NewsUrl.Len())
	{
		...
		if (NewsCacheEntry->ReadState == OERS_NotStarted)
		{
			...
			const FString Options = FString::Printf(TEXT("TitleID=%d&Localization=%s&PlatformID=%d"),
				appGetTitleId(),
				*appGetLanguageExt(),
				(DWORD)appGetPlatformType());
			// Create the object that will download the data
			NewsCacheEntry->HttpDownloader = new FHttpDownloadString(FALSE,NewsCacheEntry->TimeOut,Options,ResolveInfo);
