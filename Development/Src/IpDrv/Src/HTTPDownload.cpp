...
void FHttpDownload::DownloadUrl(FURL& Url,FArchive* Ar,INT HttpPort)
{
	...
	{
		debugf(NAME_DevNet,TEXT("FHttpDownload::DownloadUrl: Can't write to a NULL archive"));
...
void FHttpDownload::Tick(FLOAT DeltaTime)
{
	...
	{
		debugf(NAME_DevNet,TEXT("FHttpDownload::Tick: Timeout processing request"));
		HttpState = HTTP_Error;
	}
}
...
void FHttpDownload::StateResolving(void)
{
	...
	else if (ResolveInfo->GetErrorCode() != SE_NO_ERROR)
	{
		...
		debugf(TEXT("Failed to resolve hostname for HTTP download"));
		HttpState = HTTP_Error;
		delete ResolveInfo;
		ResolveInfo = NULL;
	}
	...
	{
		debugf(TEXT("FHttpDownload resolve complete"));
...
void FHttpDownload::ResolveHostIp(void)
{
	...
	else
	{
		...
		debugf(NAME_DevNet,TEXT("Performing DNS lookup for %s"),*RequestUrl.Host);
...
void FHttpDownload::StateConnecting(void)
{
	...
	switch (ServerSocket->GetConnectionState())
	{
		...
		case SCS_NotConnected:
		{
			...
			{
				debugf(NAME_DevNet,TEXT("FHttpDownload::StateConnecting() timed out") );
				HttpState = HTTP_Error;
			}
			break;
		}
		...
		default:
		{
			debugf(NAME_DevNet,TEXT("FHttpDownload::StateConnecting() connection failed or refused"));
			HttpState = HTTP_Error;
			break;
		}
	}
...
void FHttpDownload::SendHttpRequest(void)
{
	...
	{
		debugf(NAME_DevNet,TEXT("FHttpDownload::SendHttpRequest() failed"));
		HttpState = HTTP_Error;
	}
}
...
void FHttpDownload::StateReceivingHeader(void)
{
	...
	{
		debugf(NAME_DevNet,TEXT("FHttpDownload::StateReceivingHeader: Connection to HTTP server failed"));
		HttpState = HTTP_Error;
	}
}

/**
 * Handle parsign the header after having read the them
 */
void FHttpDownload::StateParsingHeader(void)
{
...
#endif
	{
		...
		if (bWasFound == FALSE)
		{
			debugf(NAME_DevNet,TEXT("FHttpDownload::StateParsingHeader: Failed to find file size"));
			HttpState = HTTP_Error;
		}
	}
		...
		&& RemainingRedirects)
	{
		...
		if (bWasFound == FALSE)
		{
			debugf(NAME_DevNet,TEXT("FHttpDownload::StateParsingHeader: Failed to find file redirect path"));
			HttpState = HTTP_Error;
		}
	}
	// Handle not found
	...
	{
		debugf(NAME_Error,TEXT("404: Failed to GET the requested file"));
		HttpState = HTTP_Error;
	}
...
//		HttpState = HTTP_PostPayload;
	...
	{
		debugf(NAME_DevNet,
			TEXT("FHttpDownload::StateParsingHeader: Got unknown status code %s"),
			*Code);
		HttpState = HTTP_Error;
	}
}
...
void FHttpDownload::StateReceivingData(void)
{
	...
	{
		debugf(NAME_DevNet,TEXT("FHttpDownload::StateReceivingData: Connection to HTTP server failed"));
		HttpState = HTTP_Error;
	}
}
...
void FHttpDownload::StatePostPayload(void)
{
	...
	{
#if _DEBUG
		debugf(NAME_DevNet,TEXT("Continuing POST with %d bytes"),SendSize);
#endif
		INT BytesSent = 0;
		...
		else
		{
			...
			{
				debugf(NAME_DevNet,TEXT("FHttpDownload::StatePostPayload() failed"));
				HttpState = HTTP_Error;
			}
		}
	}
	...
	{
		debugf(NAME_DevNet,TEXT("HTTP POST payload sent, waiting for response"));
		HttpState = HTTP_ReceivingHeader;
	}
}
