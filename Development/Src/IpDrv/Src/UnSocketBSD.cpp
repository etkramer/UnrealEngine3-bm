...
{
	const INT rc = connect(Socket,Addr,sizeof(struct sockaddr_in));
	INT Return = 0;
	switch (rc)
	{
		case 0:
 ...
 */
FSocket* FSocketBSD::Accept()
{
	SOCKET NewSocket = accept(Socket,NULL,NULL);
	if ( NewSocket != INVALID_SOCKET )
	{
		return new FSocketBSD( NewSocket, SocketType );
	}
	return NULL;
}
 ...
 * @return		The new (heap-allocated) socket, or NULL if unsuccessful.
 */
FSocket* FSocketBSD::Accept(FInternetIpAddr& OutAddr)
{
...
ESocketConnectionState FSocketBSD::GetConnectionState(void)
{
	...
	if (SelectStatus == 0)
	{
		...
		SelectStatus = select(0,NULL,&SocketSet,NULL,&Time2);
		if (SelectStatus > 0)
		{
...
{
	int iSize = NewSize;
	UBOOL bOk = setsockopt(Socket,SOL_SOCKET,SO_SNDBUF,&iSize,sizeof(iSize)) == 0;
 ...
 */
FSocket* FSocketSubsystemBSD::CreateDGramSocket(UBOOL)
{
	SOCKET Socket = socket(AF_INET,SOCK_DGRAM,IPPROTO_UDP);
	return Socket != INVALID_SOCKET ? new FSocketBSD(Socket,SOCKTYPE_Datagram) : NULL;
}
 ...
 * @return the new socket or NULL if failed
 */
FSocket* FSocketSubsystemBSD::CreateStreamSocket(void)
{
	SOCKET Socket = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	return Socket != INVALID_SOCKET ? new FSocketBSD(Socket,SOCKTYPE_Streaming) : NULL;
}
...
#if !WITH_UE3_NETWORKING || !PLATFORM_UNIX
// Suppress linker warning "warning LNK4221: no public symbols found; archive member will be inaccessible"
INT UnSocketBSDLinkerHelper;
#endif // !WITH_UE3_NETWORKING || !PLATFORM_UNIX
