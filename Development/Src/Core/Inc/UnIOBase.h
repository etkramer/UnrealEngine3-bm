...
protected:
	...
	struct FAsyncIORequest
	{
		...
		{
			return FString::Printf(TEXT("%11.1f, 0x%08x, %10d, %10d, %10d, %10d, %10d, 0x%08x, 0x%08x, 0x%08x, %d, %s"),
				(DOUBLE)RequestIndex, (DWORD)FileHandle.Handle, FileHandle.StatsHandle,
				FileStartSector, Offset, Size, UncompressedSize, (DWORD)Dest, (DWORD)CompressionFlags,
...
#if PLATFORM_UNIX
/**
 * Unix specific implementation of async IO system. @todo move to PlatformWindows project
 */
struct FAsyncIOSystemUnix : public FAsyncIOSystemBase
{
	/** 
	 * Reads passed in number of bytes from passed in file handle.
	 *
