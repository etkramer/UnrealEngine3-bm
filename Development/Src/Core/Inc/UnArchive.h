...
public:
	...
	UBOOL Close()
	{
		...
		if (ReaderData)
		{
			...
			GThreadPool->AddQueuedWork(new FAsyncSHAVerify(ReaderData, ReaderSize, bFreeOnClose, *SourcePathname, bIsUnfoundHashAnError));
			ReaderData = NULL;
		}
...
struct FTraceRouteRecord
{
	...
	, ReferencerProperty(InReferencerProperty)
	{}
};
