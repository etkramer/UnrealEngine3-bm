...
enum EStats
{
	...
	// Script stat IDs are allocated on the fly
	STAT_FirstScriptFunctionStat		= 40000,
	STAT_LastScriptFunctionStat			= 59999,
...
class FStatManager
{
	...
	/** Maps script functions to unique stat IDs */
	TMap< UFunction*, DWORD > ScriptFunctionToStatIDMap;
	
	/** List of stat factories for script functions */
	TArray< FStatFactory* > ScriptFunctionStatFactories;
...
public:
	...
	/**
	 * Returns the stat ID for the specified script function and creates new stat factories
	 * for script functions that are not yet associated with a stat
	 *
	 * @param	InScriptFunction	The script function pointer
	 *
	 * @return	Stat ID for this script function
	 */
	DWORD FindStatIDForScriptFunction( UFunction* InScriptFunction )
	{
		DWORD* StatIDPtr = ScriptFunctionToStatIDMap.Find( InScriptFunction );
		if( StatIDPtr != NULL )
		{
			return *StatIDPtr;
		}
		...
		{
			// Map UFunction to a stat ID and create stat a factory for this function
			const DWORD FirstScriptFunctionStatID = STAT_FirstScriptFunctionStat;
			static DWORD CurScriptFunctionStatID = FirstScriptFunctionStatID;

			const DWORD MyStatID = CurScriptFunctionStatID;
			check( MyStatID <= STAT_LastScriptFunctionStat );

			// Generate a name string (we'll free this when the stats manager is destroyed)
			FString* ScriptFunctionNameFStringPtr = new FString( InScriptFunction->GetPathName() );
			const TCHAR* ScriptFunctionName = *( *ScriptFunctionNameFStringPtr );

			// Create stat factory for this stat ID
			FStatFactory* NewStatFactory =
				new FStatFactory(
					ScriptFunctionName,
					MyStatID,
					STATGROUP_Script,
			...
			ScriptFunctionStatFactories.AddItem( NewStatFactory );

			// Add to global script function stat ID map
			ScriptFunctionToStatIDMap.Set( InScriptFunction, MyStatID );

			// Advance stat ID
			++CurScriptFunctionStatID;
