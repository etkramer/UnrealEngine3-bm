...
const TCHAR* ReadToken( const TCHAR* Buffer, FString& String, UBOOL DottedNames=0 )
{
	...
	{
		// Get identifier.
		while( (appIsAlnum(*Buffer) || *Buffer=='_' || *Buffer=='-' || (DottedNames && (*Buffer=='.' || *Buffer==SUBOBJECT_DELIMITER_CHAR) )) )
			String += *Buffer++;
	}
	else
	{
...
void UProperty::ExportCppDeclaration( FOutputDevice& Out, UBOOL IsMember, UBOOL IsParm, UBOOL bImportsDefaults ) const
{
	...
	if( IsA(UBoolProperty::StaticClass()) )
	{
		...
		else if( IsParm && HasAnyPropertyFlags(CPF_OutParm) )
		{
			...
			Out.Logf( TEXT("%s%s%s %s%s"), *TypeText, *ExtendedTypeText, HasAnyPropertyFlags(CPF_OptionalParm) ? TEXT("*") : TEXT("&"), *GetName(), ArrayStr );
		}
	...
	else if ( RequiresInit() )
	{
		if ( IsParm )
		{
			if ( ArrayDim > 1 )
			{
				// export as a pointer
				//@fixme ronp
				Out.Logf( TEXT("%s%s* %s"), *TypeText, *ExtendedTypeText, *GetName() );
			}
			else
			{
	...
	else if ( IsParm )
	{
		...
		else
		{
			...
			Out.Logf( TEXT("%s%s%s %s%s"), *TypeText, *ExtendedTypeText, ModifierString, *GetName(), ArrayStr );
		}
	}
	else
	{
...
const TCHAR* UDelegateProperty::ImportText( const TCHAR* Buffer, BYTE* PropertyValue, INT PortFlags, UObject* Parent, FOutputDevice* ErrorText ) const
{
	...
	if( Object )
	{
		...
		if( Func )
		{
			...
			{
				INT Count=0;
				for( TFieldIterator<UProperty,CASTCLASS_UProperty> It1(Func),It2(Delegate); Count<Delegate->NumParms; ++It1,++It2,++Count )
				{
...
static void InitializeFromInstance( UComponent* NewTemplate, UComponent* InstanceComponent, UObject* SubobjectRoot )
{
	...
	for ( TFieldIterator<UProperty,CASTCLASS_UProperty> It(NewTemplate->GetClass()); It; ++It )
	{
		UProperty* Prop = *It;
...
const TCHAR* UArrayProperty::ImportText( const TCHAR* Buffer, BYTE* Data, INT PortFlags, UObject* Parent, FOutputDevice* ErrorText ) const
{
	...
	if ( !ValidateImportFlags(PortFlags,ErrorText) || Buffer == NULL || *Buffer++ != TEXT('(') )
		return NULL;
...
void UStructProperty_ExportTextItem(class UScriptStruct* InStruct, FString& ValueStr, const BYTE* PropertyValue, const BYTE* DefaultValue, UObject* Parent, INT PortFlags)
{
	...
	for (TFieldIterator<UProperty,CASTCLASS_UProperty> It(InStruct); It; ++It)
	{
