...
// This isn't in Core/Src/UnUnix for UE3...  --ryan.
#if PLATFORM_UNIX
#include <sys/socket.h>
#include <netdb.h>
static unsigned32 appGetLocalIP(void)
{
	static bool lookedUp = false;
	static unsigned32 LocalIP = 0;
	...
	if( !lookedUp )
	{
		...
		if (gethostname( Hostname, sizeof(Hostname) ) == 0)
		{
			...
			if (getaddrinfo(Hostname, NULL, NULL, &addr) == 0)
			{
				for (struct addrinfo *i = addr; i != NULL; i = i->ai_next)
				{
					if (i->ai_family == AF_INET)
					{
						LocalIP = ((sockaddr_in *) i->ai_addr)->sin_addr.s_addr;
						if (LocalIP != 0)
							break;
					}
				}
