...
#define WITH_MULTI_THREADED_COMPRESSION (!CONSOLE)
		...
		struct FAsyncCompressionChunks 
		{
			/** Thread safe counter indicating whether an task is still outstanding.	*/
			...
			/** Index of this chunk in CompressionChunks array.							*/
			INT ChunkIndex;
		};

		// Don't scale more than 8x to avoid going overboard wrt temporary memory.
		FAsyncCompressionChunks AsyncChunks[8];
		// Zero initialize struct. ChunkIndex is always going to be > 0 so all values are actually "invalid" this way.
		appMemzero( AsyncChunks, sizeof(FAsyncCompressionChunks) * ARRAY_COUNT(AsyncChunks) );

		// Maximum number of concurrent async tasks we're going to kick off. This is based on the number of processors
		// available in the system.
		INT MaxConcurrentAsyncChunks = Clamp<INT>( GNumHardwareThreads - GNumUnusedThreads_SerializeCompressed, 1, ARRAY_COUNT(AsyncChunks) );
		...
		{
			// If TRUE we are waiting for async tasks to complete and should give up our timeslice
			...
			if( NumChunksLeftToKickOff )
			{
				...
				{
					// UncompressedSize == NULL means that this chunk is available.
					if( AsyncChunks[i].UncompressedSize == 0 )
					{
						FreeIndex = i;
						break;
					}
				...
				if( FreeIndex != INDEX_NONE )
				{
					...
					AsyncChunks[FreeIndex].CompressedSize	= 2 * GSavingCompressionChunkSize;
					// Allocate compressed buffer placeholder on first use.
					if( AsyncChunks[FreeIndex].CompressedBuffer == NULL )
					{
						AsyncChunks[FreeIndex].CompressedBuffer = appMalloc( AsyncChunks[FreeIndex].CompressedSize	);
					}

					// We need to pre-increment as the async work task will only decrement.
					check( AsyncChunks[FreeIndex].Counter.GetValue() == 0 );
					AsyncChunks[FreeIndex].Counter.Increment();

					// By default everything is chunked up into GSavingCompressionChunkSize chunks.
					AsyncChunks[FreeIndex].UncompressedSize	= Min( BytesRemainingToKickOff, GSavingCompressionChunkSize );
					check(AsyncChunks[FreeIndex].UncompressedSize>0);
					...
					if( bTreatBufferAsFileReader )
					{
						...
						if( !AsyncChunks[FreeIndex].UncompressedBuffer )
						{
							AsyncChunks[FreeIndex].UncompressedBuffer = appMalloc(GSavingCompressionChunkSize);
						}
						((FArchive*)V)->Serialize(AsyncChunks[FreeIndex].UncompressedBuffer, AsyncChunks[FreeIndex].UncompressedSize);
					}
					// Advance src pointer by amount to be compressed.
					else
					{
						AsyncChunks[FreeIndex].UncompressedBuffer = SrcBuffer;
						SrcBuffer += AsyncChunks[FreeIndex].UncompressedSize;
					}

					// Update status variables for tracking how much work is left, what to do next.
					BytesRemainingToKickOff -= AsyncChunks[FreeIndex].UncompressedSize;
					AsyncChunks[FreeIndex].ChunkIndex = CurrentChunkIndex++;
					NumChunksLeftToKickOff--;

					// Create async work and add it to queued work pool.
					GThreadPool->AddQueuedWork( 
						new	FAsyncCompress( 
							Flags, 
							AsyncChunks[FreeIndex].CompressedBuffer, 
							AsyncChunks[FreeIndex].CompressedSize, 
							AsyncChunks[FreeIndex].UncompressedBuffer, 
							AsyncChunks[FreeIndex].UncompressedSize,
							&AsyncChunks[FreeIndex].Counter	) );
				}
				// No chunks were available to use, set variable to give up time slice.
				else
				{
			...
			// Finalize async decompression tasks (if available)
			do
			{
				...
				for( INT i=0; i<MaxConcurrentAsyncChunks; i++ )
				{
					// UncompressedSize == NULL means that this chunk is available.
					if( AsyncChunks[i].UncompressedSize != 0 
					// 0 is reserved, so this is a real chunk
					&&	AsyncChunks[i].ChunkIndex != 0 )
					{
						// Either 'first' chunk
						if( OldestAsyncChunkIndex == INDEX_NONE 
						// or oldest chunk.
						||	AsyncChunks[i].ChunkIndex < AsyncChunks[OldestAsyncChunkIndex].ChunkIndex )
						{
				...
				// Are there are chunks at all outstanding?
				if( OldestAsyncChunkIndex != INDEX_NONE
				// This guarantees that the async work has finished.
				&&	AsyncChunks[OldestAsyncChunkIndex].Counter.GetValue() == 0 )
				{
					// Serialize the data via archive.
					Serialize( AsyncChunks[OldestAsyncChunkIndex].CompressedBuffer, AsyncChunks[OldestAsyncChunkIndex].CompressedSize );

					// Update associated chunk.
					INT CompressionChunkIndex = AsyncChunks[OldestAsyncChunkIndex].ChunkIndex;
					check(CompressionChunkIndex<TotalChunkCount);
					CompressionChunks[CompressionChunkIndex].CompressedSize		= AsyncChunks[OldestAsyncChunkIndex].CompressedSize;
					CompressionChunks[CompressionChunkIndex].UncompressedSize	= AsyncChunks[OldestAsyncChunkIndex].UncompressedSize;

					// Keep track of total compressed size, stored in first chunk.
					CompressionChunks[0].CompressedSize	+= AsyncChunks[OldestAsyncChunkIndex].CompressedSize;

					// Clean up chunk. Src and dst buffer are not touched as the contain allocations we keep till the end.
					AsyncChunks[OldestAsyncChunkIndex].ChunkIndex		= 0;
					AsyncChunks[OldestAsyncChunkIndex].CompressedSize	= 0;
					AsyncChunks[OldestAsyncChunkIndex].UncompressedSize = 0;
					check(AsyncChunks[OldestAsyncChunkIndex].Counter.GetValue() == 0);
				...
				// Give up timeslice if requested.
				if( bNeedToWaitForAsyncTask )
				{
					appSleep(0);
				}
			}
			// Iterate till there are free async chunks/ tasks available if needed.
			while( bNeedToWaitForAsyncTask );
		}
		...
		for( INT i=0; i<MaxConcurrentAsyncChunks; i++ )
		{
			...
			appFree( AsyncChunks[i].CompressedBuffer );
			AsyncChunks[i].CompressedBuffer = NULL;
			// Free temporary uncompressed buffer storage if data was serialized in.
			if( bTreatBufferAsFileReader )
			{
				appFree( AsyncChunks[i].UncompressedBuffer );
				AsyncChunks[i].UncompressedBuffer = NULL;
			}
		}
...
TMap<UObject*,UProperty*> FArchiveTraceRoute::FindShortestRootPath( UObject* Obj, UBOOL bIncludeTransients, EObjectFlags KeepFlags )
{
	...
	if ( Routes.Num() > 0 || Obj->HasAnyFlags(KeepFlags) )
	{
		...
		for ( INT RecordIndex = Records.Num() - 1; RecordIndex >= 0; RecordIndex-- )
		{
			...
			Result.Set( Record.GraphNode->NodeObject, Record.ReferencerProperty );
		}
	}
...
//	 	TArray<TDoubleLinkedList<FObjectGraphNode*> > ShortestRoutes;
		...
		for (TDoubleLinkedList<FObjectGraphNode*>::TIterator It(ShortestRoute.GetHead()); It; ++It )
		{
			...
			InRoutes.Set(CurrentNode->NodeObject, FTraceRouteRecord(CurrentNode, CurrentNode->ReferencerProperty));
		}
	}
}
...
FArchive& FArchiveTraceRoute::operator<<( class UObject*& Obj )
{
	...
	&&	(bIncludeTransients || Obj->HasAnyFlags(RF_Transient)) )
	{
		...
		if ( Obj != CurrentReferencer )
		{
			...
			if ( !ReferencerNode->ReferencedObjects.HasKey(Obj) )
			{
				ReferencerNode->ReferencedObjects.Set(Obj, FTraceRouteRecord(CurrentObjectNode));
			}
			if ( !CurrentObjectNode->ReferencerRecords.HasKey(CurrentReferencer) )
			{
...
void FArchiveTraceRoute::CalculateReferenceDepthsForNode( FObjectGraphNode* ObjectNode )
{
	...
	for ( TMap<UObject*, FTraceRouteRecord>::TIterator It(ObjectNode->ReferencerRecords); It; ++It )
	{
		...
		if ( Record.GraphNode->ReferenceDepth > Depth )
		{
			...
			Record.GraphNode->ReferencerProperty = Record.ReferencerProperty;
...
void FArchiveSaveCompressedProxy::Serialize( void* InData, INT Count )
{
	...
	else
	{
		while( Count )
		{
			INT BytesToCopy = Min( Count, TmpDataEnd - TmpData );
