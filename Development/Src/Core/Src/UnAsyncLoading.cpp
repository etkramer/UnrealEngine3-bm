...
void UObject::LoadPackageAsync( const FString& InPackageName, FAsyncCompletionCallback CompletionCallback, void* CallbackUserData, const FGuid* PackageGuid )
{
	...
	// Add to (FIFO) queue.
	FAsyncPackage Package(PackageName, PackageGuid);
	if (CompletionCallback != NULL)
	{
	...
	EAsyncIOPriority Priority )
{
	...
	IORequest.bIsDestroyHandleRequest	= FALSE;
...
void FAsyncIOSystemBase::FulfillCompressedRead( const FAsyncIORequest& IORequest )
{
	...
	for (INT ChunkIndex = 1; ChunkIndex < TotalChunkCount; ChunkIndex++)
	{
		...
		if (CompressionChunks[ChunkIndex].UncompressedSize < CompressionChunkSize)
		{
			...
			{
				checkf(0, TEXT("Calculated too many chunks: %d should be last, there are %d from '%s'"),
					ChunkIndex, TotalChunkCount, *IORequest.FileName);
			}
		}
#endif//#if !FINAL_RELEASE
...
DWORD FAsyncIOSystemBase::Run()
{
	...
	while( IsRunning.GetValue() > 0 )
	{
		...
		if( bIsRequestPending )
		{
			...
			{
				// don't use GetCachedFileHandle, as that will attempt to create it if it doesn't exist
				FAsyncIOHandle*	FileHandlePtr	= NameToHandleMap.Find( IORequest.FileName );
				if (FileHandlePtr)
				{
