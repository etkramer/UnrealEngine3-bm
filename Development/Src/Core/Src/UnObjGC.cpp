...
#define DETAILED_PER_CLASS_GC_STATS				(_DEBUG || LOOKING_FOR_PERF_ISSUES)

#define VERIFY_DISREGARD_GC_ASSUMPTIONS			((!FINAL_RELEASE) && (!SHIPPING_PC_GAME))
 ...
 * is true.
 */
UBOOL		GShouldVerifyGCAssumptions				= TRUE;
 ...
 * @param	TotalCount			Total count, if INDEX_NONE will be calculated
 */
static void LogClassCountInfo( const TCHAR* LogText, TMap<const FName,INT>& ClassToCountMap, INT NumItemsToLog, INT TotalCount )
{
	...
	for( TMap<const FName,INT>::TIterator It(ClassToCountMap); It; ++It )
	{
...
public:
	...
	void PerformReachabilityAnalysis( EObjectFlags KeepFlags )
	{
		...
		while( CurrentIndex < ObjectsToSerialize.Num() )
		{
			...
			INT ClassDisregardedObjRefs = GClassToDisregardedObjectRefsMap.FindRef( ClassName );
			...
			INT ClassRegularObjRefs = GClassToRegularObjectRefsMap.FindRef( ClassName );
		...
		// Log summary stats.
		LogClassCountInfo( TEXT("references to regular objects from"), GClassToRegularObjectRefsMap, 20, INDEX_NONE );
		LogClassCountInfo( TEXT("references to permanent objects from"), GClassToDisregardedObjectRefsMap, 20, INDEX_NONE );		
#else
		}
#endif
#if CATCH_GC_CRASHES
...
DWORD UClass::EmitStructArrayBegin( INT Offset, INT Stride )
{
	...
	const DWORD SkipIndexIndex = ThisOnlyReferenceTokenStream.EmitSkipIndexPlaceholder();
...
{
	check( HasAnyClassFlags( CLASS_Intrinsic ) );
	const DWORD SkipIndex = ThisOnlyReferenceTokenStream.EmitReturn();
