...
#include "CorePrivate.h"
	...
	#pragma comment(lib, "../External/AgPerfMon/lib/AgPerfMon.lib")
	...
	bForOfflineProfiling( FALSE )
{
	...
	// Script function stats are never displayed in HUD
	if( InStatId >= STAT_FirstScriptFunctionStat && InStatId <= STAT_LastScriptFunctionStat )
	{
		bForOfflineProfiling = TRUE;
	}
}
...
void FStatManager::Destroy(void)
{
	...
	// Clean up stat factories
	{
		for( INT CurIndex = 0; CurIndex < ScriptFunctionStatFactories.Num(); ++CurIndex )
		{
			FStatFactory* CurFactory = ScriptFunctionStatFactories( CurIndex );

			// Delete string name
			delete[] CurFactory->CounterName;
			CurFactory->CounterName = NULL;

			// Delete the factory itself
			delete CurFactory;
		}
		ScriptFunctionStatFactories.Empty();
	}
	...
	else if (ParseCommand(&Cmd,TEXT("LIST")))
	{
		...
		else if (ParseCommand(&Cmd,TEXT("SETS")))
		{
			...
			if (Section)
			{
				...
				if (Section->GetKeys(UniqueCustomStatsGroupsKeys) > 0)
				{
