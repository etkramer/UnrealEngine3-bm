...
#error Please set SUPPORT_NAME_FLAGS to 1 in UnBuild.h if you want to support saving packages
	...
	GWarn->StatusUpdatef( CurSaveStep, TotalSaveSteps, LocalizeSecure(LocalizeProgress(TEXT("Saving"),TEXT("Core")), *CleanFilename) );
	UBOOL Success = 0;
	...
	if( Success == TRUE )
	{
		...
		else
		{
			...
			if( bWarnOfLongFilename == TRUE )
			{
				...
				{
					warnf( (SaveFlags & SAVE_NoError) ? NAME_Log : NAME_Error, *FString::Printf( LocalizeSecure(LocalizeError(TEXT("Error_FilenameIsTooLongForCooking"),TEXT("Core")), *BaseFilename, MaxFilenameLength) ) );
				}
			}
		}
	...
	if( Success == TRUE )
	{
		...
		if( GIsEditor && PackageSize > (GSys->PackageSizeSoftLimit * 1024 * 1024) )
		{
