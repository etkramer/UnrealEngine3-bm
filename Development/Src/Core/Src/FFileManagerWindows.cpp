...
FFileHandle FFileManagerWindows::FileOpen( const TCHAR* Filename, DWORD Flags )
{
	if ( Flags & IO_WRITE )
	{
		return InternalFileOpen( *ConvertAbsolutePathToUserPath(*ConvertToAbsolutePath(Filename)), Flags );
	}
	else
	{
		// first look in User Directory
		FFileHandle FileHandle = InternalFileOpen( *ConvertAbsolutePathToUserPath(*ConvertToAbsolutePath(Filename)), Flags );

		// if not found there, then look in the install directory
		if (FileHandle.IsValid() == FALSE)
		{
			FileHandle = InternalFileOpen( *ConvertToAbsolutePath(Filename), Flags );
		}
		return FileHandle;
	}
}
 ...
 *	@param Offset			Requested file offset, counted in bytes from the specified 'Base'.
 ...
 **/
INT FFileManagerWindows::FileSeek( FFileHandle FileHandle, INT Offset, EFileSeekFlags Base/*=IO_SEEK_BEGIN*/ )
{
	if ( FileHandle.IsValid() )
	{
		DWORD SeekFlags;
...
{
	INT ReturnValue = InternalFileTruncate( *ConvertAbsolutePathToUserPath(*ConvertToAbsolutePath(Filename)), FileSize );
	// then look in install dir if it failed to be found
	if (ReturnValue == INDEX_NONE)
	{
		ReturnValue = InternalFileTruncate( *ConvertToAbsolutePath(Filename), FileSize );
	}
	return ReturnValue;
}
...
{
	UBOOL ReturnValue = InternalSetFileTimestamp( *ConvertAbsolutePathToUserPath(*ConvertToAbsolutePath(Filename)), TimeStamp );
	// then look in install dir if it failed to be found
	if (ReturnValue == FALSE)
	{
		ReturnValue = InternalSetFileTimestamp( *ConvertToAbsolutePath(Filename), TimeStamp );
	}
	return ReturnValue;
}
