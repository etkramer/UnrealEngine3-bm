...
UBOOL UObject::ScriptConsoleExec( const TCHAR* Str, FOutputDevice& Ar, UObject* Executor )
{
	...
	for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(Function); It && (It->PropertyFlags & (CPF_Parm|CPF_ReturnParm))==CPF_Parm; ++It, NumParamsEvaluated++ )
	{
...
{
	UState* State = NULL;
	for (TFieldIterator<UState,CASTCLASS_UState> It(GetClass()); It && !State; ++It)
	{
		if (It->GetFName() == InName)
		{
			State = *It;
...
void UObject::LoadConfig( UClass* ConfigClass/*=NULL*/, const TCHAR* InFilename/*=NULL*/, DWORD PropagationFlags/*=LCPF_None*/, UProperty* PropertyToLoad/*=NULL*/ )
{
	...
	{
		debugf(NAME_DevSave, TEXT("(%s) '%s' loading configuration from %s"), *ConfigClass->GetName(), *GetName(), *Filename);
	}
	else
	{
		debugf(NAME_DevSave, TEXT("(%s) '%s' loading configuration for property %s from %s"), *ConfigClass->GetName(), *GetName(), *PropertyToLoad->GetName(), *Filename);
	}
#endif

// 	for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(ConfigClass); It; ++It )
...
#if !FINAL_RELEASE && !SHIPPING_PC_GAME
		debugf(NAME_DevSave, TEXT("   Loading value for %s from [%s]"), *Key, *ClassSection);
#endif
		...
		else
		{
			...
			if( Map )
			{
				...
				if( Sec )
				{
					...
					//Ptr->AddZeroed( List.Num(), Size );
					//for( INT i=List.Num()-1,c=0; i>=0; i--,c++ )
					//	Array->Inner->ImportText( *List(i), (BYTE*)Ptr->GetData() + c*Size, PPF_ConfigOnly );
				}
			}
			else
			{
				for( INT i=0; i<Property->ArrayDim; i++ )
				{
					...
					{
						Property->ImportText( *Value, (BYTE*)this + Property->Offset + i*Property->ElementSize, PPF_ConfigOnly, this );
					}
				}
			}
		}
...
void UObject::StaticExit()
{
	...
	// Tag all objects as unreachable. Can't use object iterators for this as they ignore certain objects.
...
void UObject::OutputReferencers( FOutputDevice& Ar, UBOOL bIncludeTransients )
{
	...
	for( FObjectIterator It; It; ++It )
	{
		...
		if ( Count > 0 )
		{
			if ( Object->IsIn(this) )
			{
				new(InternalReferences) FReferencerInformation(Object, Count, Referencers);
			}
			else
			{
				new(ExternalReferences) FReferencerInformation(Object, Count, Referencers);
			}
		}
	}
		...
		if ( ExternalReferences.Num() > 0 )
		{
			...
			for ( INT RefIndex = 0; RefIndex < ExternalReferences.Num(); RefIndex++ )
			{
				...
				for ( INT i = 0; i < RefInfo.TotalReferences; i++ )
				{
					...
					{
						Ar.Logf(TEXT("      %i) [[native reference]]"), i);
					}
				}
			}
		}
...
void UObject::RetrieveReferencers( TArray<UObject*>* OutInternalReferencers, TArray<UObject*>* OutExternalReferencers, UBOOL bIncludeTransients )
{
	for( FObjectIterator It; It; ++It )
	{
		UObject* Object = *It;
		...
		if ( Count > 0 )
		{
			if ( Object->IsIn(this) )
			{
				...
				{
					OutInternalReferencers->AddUniqueItem(Object);
				}
			}
			else
			{
				...
				{
					OutExternalReferencers->AddUniqueItem(Object);
				}
			}
		}
	}
...
{
	for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(Struct); It; ++It )
	{
...
UBOOL UObject::StaticExec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("OBJ")) )
	{
		...
		else if (ParseCommand(&Str, TEXT("SINGLEREF")))
		{
			...
			for (TObjectIterator<UObject> It; It; ++It)
			{
				...
				{
					TArray<UObject*> OutInternalReferencers;
					TArray<UObject*> OutExternalReferencers;
					Object->RetrieveReferencers(NULL, &OutExternalReferencers, TRUE);

					if (OutExternalReferencers.Num() == 1)
					{
						UObject* RefObj = OutExternalReferencers(0);
						if (RefObj)
						{
							UBOOL bDumpIt = TRUE;
							...
							if (bDumpIt)
							{
								...
								if (bListClass)
								{
									Ar.Logf(TEXT("%64s: %64s, %8d,%8d,%8d"), *(Object->GetClass()->GetName()), *(Object->GetPathName()),
										Count.GetNum(), Count.GetMax(), ResourceSize);
								}
								else
								{
									Ar.Logf(TEXT("%64s, %8d,%8d,%8d"), *(Object->GetPathName()),
										Count.GetNum(), Count.GetMax(), ResourceSize);
								}
		...
		else if( ParseCommand(&Str,TEXT("LIST")) )
		{
			...
			if( Objects.Num() )
			{
				...
				Ar.Logf( TEXT("%100ls % 10ls % 10ls % 10ls"), TEXT("Object"), TEXT("NumBytes"), TEXT("MaxBytes"), TEXT("ResKBytes") );
#else
				Ar.Logf( TEXT("%100s % 10s % 10s % 10s"), TEXT("Object"), TEXT("NumBytes"), TEXT("MaxBytes"), TEXT("ResKBytes") );
#endif
				for( INT i=0; i<Objects.Num(); i++ )
				{
					...
					if(bExportToFile)
					{
						...
						const FString Filename = Path * Objects(i).Object->GetOutermost()->GetName() + TEXT(".") +  *Objects(i).Object->GetName() + TEXT(".t3d");
						Ar.Logf( TEXT("%s"),*Filename);
						UExporter::ExportToFile(Objects(i).Object, NULL, *Filename, 1, 0);
					}					
					//MSEND
#if (PS3 || PLATFORM_UNIX)
					Ar.Logf( TEXT("%100ls % 10i % 10i % 10i"), *Objects(i).Object->GetFullName(), Objects(i).Num, Objects(i).Max, Objects(i).Object->GetResourceSize() / 1024 );
#else
					Ar.Logf( TEXT("%100s % 10i % 10i % 10i"), *Objects(i).Object->GetFullName(), Objects(i).Num, Objects(i).Max, Objects(i).Object->GetResourceSize() / 1024 );
#endif
				}
				Ar.Log( TEXT("") );
			}
...
#if (PS3 || PLATFORM_UNIX)
				Ar.Logf(TEXT(" %100ls % 6ls % 10ls % 10ls % 10ls"), TEXT("Class"), TEXT("Count"), TEXT("NumBytes"), TEXT("MaxBytes"), TEXT("ResBytes") );
#else
				Ar.Logf(TEXT(" %100s % 6s % 10s % 10s % 10s"), TEXT("Class"), TEXT("Count"), TEXT("NumBytes"), TEXT("MaxBytes"), TEXT("ResBytes") );
#endif
				for( INT i=0; i<List.Num(); i++ )
				{
#if (PS3 || PLATFORM_UNIX)
					Ar.Logf(TEXT(" %100ls % 6i % 10iK % 10iK % 10iK"), *List(i).Class->GetName(), List(i).Count, List(i).Num/1024, List(i).Max/1024, List(i).Res/1024 );
#else
					Ar.Logf(TEXT(" %100s % 6i % 10iK % 10iK % 10iK"), *List(i).Class->GetName(), List(i).Count, List(i).Num/1024, List(i).Max/1024, List(i).Res/1024 );
#endif
				}
				Ar.Log( TEXT("") );
			}
		...
		else if ( ParseCommand(&Str,TEXT("DUMP")) )
		{
			...
			if ( Obj )
			{
				...
				else
				{
					...
					for ( TFieldIterator<UProperty,CASTCLASS_UProperty> It(Obj->GetClass()); It; ++It )
					{
						Value.Empty();
	...
	else if( ParseCommand(&Str,TEXT("PROFILESCRIPT")) || ParseCommand(&Str,TEXT("SCRIPTPROFILER")) )
	{
		...
		{
			// See if there is a TIME provided
			INT Time = 0;
			Parse(Str, TEXT("TIME="), Time);
			// Clamp it to 30 minutes (which is too large, I think...)
			Time = Clamp<INT>(Time, 0, 60 * 30);

			delete GScriptCallGraph;
			GScriptCallGraph = new FScriptCallGraph( 16 * 1024 * 1024, Time );
			if (Time > 0)
			{
				debugf( TEXT( "ScriptProfile:  collecting script profile data for %d seconds." ), Time );
			}
			else
			{
				debugf( TEXT( "ScriptProfile:  collecting script profile data." ) );
			}
			return TRUE;
		}
		else if( ParseCommand(&Str,TEXT("STOP")) )
		{
			if( GScriptCallGraph )
			{
				...
				const FString PathName = *(appProfilingDir() + TEXT("Script") PATH_SEPARATOR );
				GFileManager->MakeDirectory( *PathName );
				//warnf( TEXT( "uprof path: %s" ), *PathName );

				const FString Filename = CreateProfileFilename( TEXT(".uprof"), TRUE );
				//warnf( TEXT( "uprof Filename: %s" ), *Filename );

				const FString FilenameFull = PathName + Filename;
				// Create a file writer and serialize the script profiling data to it.
				FArchive* Archive = GFileManager->CreateFileWriter( *(FilenameFull) );
				//warnf( TEXT( "uprof full: %s" ), *(FilenameFull) );
				...
				debugf( TEXT( "ScriptProfile:  profile data written and cleaned up." ), *Filename );

				SendDataToPCViaUnrealConsole( TEXT("UE_PROFILER!SCRIPT:"), *(FilenameFull) );
			}
			return TRUE;
		}
...
void UObject::ParseParms( const TCHAR* Parms )
{
	if( !Parms )
		return;
	for( TFieldIterator<UProperty,CASTCLASS_UProperty> It(GetClass()); It; ++It )
	{
...
void UObject::MarkPackageDirty( UBOOL InDirty ) const
{
	...
	if ( !HasAnyFlags(RF_Transient) )
	{
		...
		if( Package != NULL )
		{
			...
			//debugf( TEXT( "dirtied: %s"), *Package->GetPathName() );
		}
	}
}
