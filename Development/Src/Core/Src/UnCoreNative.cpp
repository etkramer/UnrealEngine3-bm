...
NativeLookup GNativeLookupFuncs[400];
...
UBOOL UEnum::HasMetaData( const TCHAR* Key, INT NameIndex/*=INDEX_NONE*/ ) const
{
	...
	if ( NameIndex != INDEX_NONE )
	{
		...
		const FString KeyString = Names(NameIndex).ToString() + TEXT(".") + Key;
		bResult = Package->GetMetaData()->HasValue( this, *KeyString );
	}
	else
	{
		bResult = Package->GetMetaData()->HasValue(this, Key);
	}
...
const FString& UEnum::GetMetaData( const TCHAR* Key, INT NameIndex/*=INDEX_NONE*/ ) const
{
	...
	if ( NameIndex != INDEX_NONE )
	{
		check(Names.IsValidIndex(NameIndex));
		const FString KeyString = Names(NameIndex).ToString() + TEXT(".") + Key;
		return Package->GetMetaData()->GetValue( this, *KeyString );
	}
	else
	{
...
void* UPackage::GetExport( const TCHAR* ExportName, UBOOL Checked ) const
{
	...
	if( !Result && Checked && !GIsUCCMake )
	{
		debugf(NAME_Warning, LocalizeSecure(LocalizeError(TEXT("NotInDll"),TEXT("Core")), ExportName, *GetName()) );
	}
...
void UPackage::AddNetObject(UObject* InObject)
{
	...
	{
		debugf(NAME_Error, TEXT("(AddNetObject) Object %s with invalid NetIndex %i (max: %i)"), *InObject->GetFullName(), InObject->GetNetIndex(), NetObjects.Num());
	}
...
void UPackage::RemoveNetObject(UObject* OutObject)
{
	...
	{
		debugf(NAME_Error, TEXT("(RemoveNetObject) Object %s with invalid NetIndex %i (max: %i)"), *OutObject->GetFullName(), OutObject->GetNetIndex(), NetObjects.Num());
	}
