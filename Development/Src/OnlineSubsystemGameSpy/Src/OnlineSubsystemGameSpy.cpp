 ...
 */
void appGetGameSpyChallengeResponse(UNetConnection* Connection,UBOOL bIsReauth)
{
 ...
 */
void appSubmitGameSpyAuthRequest(UNetConnection* ConnToAuth)
{
	if (GOnlineSubsystemGameSpy)
	{
 ...
 */
void appSubmitGameSpyReauthRequest(UNetConnection* Connection,INT Hint)
{
	if (GOnlineSubsystemGameSpy)
	{
...
void UOnlineSubsystemGameSpy::GPErrorCallback(GPErrorArg* Arg)
{
	...
	if (Arg->result != GP_NO_ERROR)
	{
		...
		if (bIsConnError)
		{
			...
			TriggerOnlineDelegates(this,LoginChangeDelegates,&Params);
		}
	}
}
	...
	if (Arg->result == GP_NO_ERROR)
	{
		...
		if (Result == GP_NO_ERROR)
		{
			...
			WSLoginValue LoginResult = (WSLoginValue)wsLoginRemoteAuth(GetPartnerId(),
				GetNamespaceId(),
				(const UCS2String)*RemoteAuthToken,
				(const UCS2String)*RemoteAuthPartnerChallenge,
...
void UOnlineSubsystemGameSpy::SakeRequestProfileCallback(SAKERequest Request, SAKERequestResult Result, SAKEGetMyRecordsInput* Input, SAKEGetMyRecordsOutput* Output)
{
	...
	if (Result == SAKERequestResult_SUCCESS)
	{
		if(Output->mNumRecords <= 0)
		{
			...
			SetProfileSaveCount(GetProfileSaveCount(CachedProfile->ProfileSettings) + 1,CachedProfile->ProfileSettings);
		...
		else
		{
			...
			FProfileSettingsReader Reader(TRUE,(const BYTE*)ProfileField->mValue.mBinaryData.mValue,(DWORD)ProfileField->mValue.mBinaryData.mLength);
			...
			if (Reader.SerializeFromBuffer(ProfileSettings))
			{
				...
				INT SakeSaveCount = GetProfileSaveCount(ProfileSettings);
				INT LocalSaveCount = GetProfileSaveCount(CachedProfile->ProfileSettings);
...
UBOOL UOnlineSubsystemGameSpy::InitGameSpy(void)
{
	...
	// Tell it which servers to use
	npNamespace = GetNamespaceId();
#endif
...
void UOnlineSubsystemGameSpy::InitNp()
{
	...
	NpData = new FNpData();
	// Load the PRX so we can make calls on it
	NpData->LoadNp();
	// Initialize NP
	NpData->InitNp();
}
...
UBOOL UOnlineSubsystemGameSpy::Logout(BYTE LocalUserNum)
{
	...
	if (CachedProfile == NULL ||
		(CachedProfile && CachedProfile->AsyncState == OPAS_None))
	{
...
void UOnlineSubsystemGameSpy::CreateStatsSession(void)
{
	...
	{
		debugf(NAME_DevOnline,TEXT("Stats gathering is disabled since no user is signed in"));
		FAsyncTaskDelegateResults Params(S_OK);
		TriggerOnlineDelegates(GameInterfaceImpl,GameInterfaceImpl->StartOnlineGameCompleteDelegates,&Params);
	}
}
	...
	if (LocalUserNum == LoggedInPlayerNum)
	{
			...
			CachedProfile->AsyncState == OPAS_None)
		{
			if (ProfileSettings != NULL)
			{
				...
				CachedProfile->AppendVersionToSettings();
				// Update the save count for roaming profile support
				SetProfileSaveCount(GetProfileSaveCount(CachedProfile->ProfileSettings) + 1,ProfileSettings->ProfileSettings);
 ...
 */
void UOnlineSubsystemGameSpy::SetProfileSaveCount(INT NewCount,TArray<FOnlineProfileSetting>& ProfileSettings)
{
	...
	for (INT Index = 0; Index < ProfileSettings.Num(); Index++)
	{
		...
		if (Setting.ProfileSetting.PropertyId == PSI_ProfileSaveCount)
		{
	...
	Setting.ProfileSetting.PropertyId = PSI_ProfileSaveCount;
 ...
 */
INT UOnlineSubsystemGameSpy::GetProfileSaveCount(const TArray<FOnlineProfileSetting>& ProfileSettings)
{
...
void UOnlineSubsystemGameSpy::ProcessTalkingDelegates(void)
{
	...
	if (PlayerTalkingDelegates.Num() > 0)
	{
		...
		if (CurrentLocalTalker.bHasVoice && CurrentLocalTalker.bWasTalking)
		{
 ...
 * @return TRUE if the function succeeds, FALSE otherwise
 */
UBOOL UOnlineSubsystemGameSpy::MuteRemoteTalker(BYTE LocalUserNum,FUniqueNetId UniqueId)
{
	DWORD Return = E_FAIL;
	if (VoiceEngine != NULL)
	{
 ...
 * @return TRUE if the function succeeds, FALSE otherwise
 */
UBOOL UOnlineSubsystemGameSpy::UnmuteRemoteTalker(BYTE LocalUserNum,FUniqueNetId UniqueId)
{
	DWORD Return = E_FAIL;
	if (LocalUserNum == LoggedInPlayerNum)
	{
	...
	if (Result == GP_NO_ERROR && Index > -1)
	{
		...
		if (Result == GP_NO_ERROR)
		{
				...
				(CurrentLocation.Len() == 0 || appStrstr((const TCHAR*)BuddyStatus.locationString,*CurrentLocation) == NULL))
			{
				...
				Parms.GameInviteSettings = CachedGameInt->InviteGameSearch->Results(0).GameSettings;
				...
				if (SpaceAvail > 0)
				{
					...
					if (hr == S_OK)
					{
...
#endif
						{
							...
							if (SpaceAvail > 0)
							{
								CurrentLocalTalker.bWasTalking = TRUE;
							}
						}
						else
						{
...
UBOOL UOnlineSubsystemGameSpy::ShowLoginUI(UBOOL bShowOnlineOnly)
{
	...
	{
		// Already logged in, so just fire the delegates with success
		FAsyncTaskDelegateResults Params(S_OK);
		TriggerOnlineDelegates(this,LoginChangeDelegates,&Params);
	}
	return TRUE;
