...
void WxPhAT::MakeNewBody(INT NewBoneIndex)
{
	...
	if(ParentBodyIndex != INDEX_NONE)
	{
		...
		FMatrix RelTM = NewBoneTM * ParentBoneTM.InverseSafe();
...
void WxPhAT::SimMousePress(FViewport* Viewport, UBOOL bConstrainRotation, FName Key)
{
	...
	FSceneViewFamilyContext ViewFamily(Viewport,PhATViewportClient->GetScene(),PhATViewportClient->ShowFlags,GWorld->GetTimeSeconds(),GWorld->GetDeltaSeconds(),GWorld->GetRealTimeSeconds(),NULL);
