	...
	, StatGroupSelectionIndex( -1 )
{
	...
	wxBoxSizer* TopVSizer = new wxBoxSizer(wxVERTICAL);
	{
		wxFlexGridSizer* SliderGrid = new wxFlexGridSizer(10, 2, 0, 0);
		{
			...
			StatDrawSizeSlider = new wxSlider( this, IDM_SENTINEL_STATSIZESLIDER, 25, 1, 100, wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL );
	...
	// Create connection to stats database
	ConnectToDatabase();
	if(!Connection)
	{
...
void WxSentinel::ConnectToDatabase()
{
...
void WxSentinel::GrabDataSet()
{
	if(Connection)
	{
		...
		const INT Changelist = (INT)ChangelistCombo->GetClientData(ChangelistComboSelection);
