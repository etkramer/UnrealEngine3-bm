...
public:
	...
	static void ExecuteModalDialog();
...
/** Static: Displays the dialog, waits for the user to change settings and dismiss the dialog */
void WxTaskBrowserConfigDialog::ExecuteModalDialog()
{
...
void WxTaskBrowser::Create( INT DockID, const TCHAR* FriendlyName, wxWindow* Parent )
{
	...
	// Localize all of the static text in the loaded window
	FLocalizeWindow( TaskBrowserPanel );
...
{\
	...
	eh->Connect( id, event, *(wxObjectEventFunction*)&(func) );\
}
		...
		AddWxEventHandler( XRCID( "ID_TaskBrowser_TaskListControl" ), wxEVT_COMMAND_LIST_COL_CLICK, &WxTaskBrowser::OnTaskListColumnButtonClicked );
		AddWxEventHandler( XRCID( "ID_TaskBrowser_TaskListControl" ), wxEVT_COMMAND_LIST_ITEM_ACTIVATED, &WxTaskBrowser::OnTaskListItemActivated );
		AddWxEventHandler( XRCID( "ID_TaskBrowser_TaskListControl" ), wxEVT_COMMAND_LIST_ITEM_SELECTED, &WxTaskBrowser::OnTaskListItemSelected );

		AddWxEventHandler( XRCID( "ID_TaskBrowser_TaskDescriptionControl" ), wxEVT_COMMAND_TEXT_URL, &WxTaskBrowser::OnURLLaunched )
...
int WxTaskBrowser::TaskListItemSortCallback( long InItem1, long InItem2, long InSortData )
{
	const WxTaskBrowser* TaskBrowser = ( WxTaskBrowser* )InSortData;
...
void WxTaskBrowser::RefreshGUI( const ETaskBrowserGUIRefreshOptions::Type Options )
{
	...
	// @todo: Temporary hack to disallow task browser until QA finishes testing it out
	const UBOOL bAllowTaskBrowser = ParseParam( appCmdLine(), TEXT( "TaskBrowser" ) );
	TaskBrowserPanel->Enable( bAllowTaskBrowser ? true : false );
	...
	// Enable/disable window controls
	{
		...
		if( !bAllowTaskBrowser ||
			( TaskDataManager->GetConnectionStatus() != ETaskDataManagerStatus::Connected &&
	...
	if( Options & ETaskBrowserGUIRefreshOptions::RebuildTaskList )
	{
		...
		TSet< INT > SelectedTaskNumbers;
		{
			...
			for( ; ; )
			{
				...
				if( TaskEntries.IsValidIndex( LocalTaskIndex ) )
				{
					...
					SelectedTaskNumbers.Add( SelectedTask.Number );
				}
			}
		}
		...
		const FString CurrentMapName = GWorld->GetWorldInfo()->GetOutermost()->GetName();
		...
		for( TArray< FTaskDatabaseEntry >::TConstIterator TaskIter( TaskEntries ); TaskIter != NULL; ++TaskIter )
		{
			...
			if( ( !bOnlyOpen || CurTask.Status.InStr( TEXT( "Open" ), FALSE, TRUE ) != INDEX_NONE ) &&
				...
				( !bCurrentMap || CurTask.Name.InStr( CurrentMapName, FALSE, TRUE ) != INDEX_NONE ) )
			{
				...
				if( SelectedTaskNumbers.Contains( CurTask.Number ) )
				{
	...
	{
		// Sort the task list
		TaskListControl->SortItems( &TaskListItemSortCallback, ( long )this );
	}
	...
	if( Options & ETaskBrowserGUIRefreshOptions::UpdateTaskDescription )
	{
		...
		wxStaticText* TaskNameControl = static_cast< wxStaticText* >( FindWindow( XRCID( "ID_TaskBrowser_TaskNameText" ) ) );
		check( TaskNameControl != NULL );
		TaskNameControl->SetLabel( *TaskName );
	...
	// Update the 'Fix' button
	{
		...
		if( bAllowTaskBrowser && TaskDatabaseSystem::IsConnected() )
		{
			// Only enable the 'Mark as Fixed' button if at least one task is selected
			if( TaskListControl != NULL )
			{
				if( TaskListControl->GetSelectedItemCount() > 0 )
				{
	...
	// Update the 'Connect' button
	{
		...
		else if( TaskDataManager->GetConnectionStatus() == ETaskDataManagerStatus::FailedToInit )
		{
			// No database available to connect to, so grey out the button
			ConnectButton->Enable( FALSE );
...
void WxTaskBrowser::OnSize( wxSizeEvent& In )
{
	...
	if( bAreWindowsInitialized )
	{
		...
		// Update task list column sizes
		RefreshGUI( ETaskBrowserGUIRefreshOptions::ResetColumnSizes );
