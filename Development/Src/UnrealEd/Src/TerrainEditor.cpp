...
INT WxMBTerrainEditorRightClick::FillMenu_Add(INT InAddOptions, UBOOL bPreviousEntries)
{
	...
	{
		MenuString	= FString::Printf(LocalizeSecure(LocalizeUnrealEd("TerrainBrowser_AddSelectedFoliage"),
			*LocalizeUnrealEd("TerrainBrowser_CreateBefore")));
	...
	{
		MenuString	= FString::Printf(LocalizeSecure(LocalizeUnrealEd("TerrainBrowser_AddSelectedFoliage"), TEXT("")));
...
void WxTerrainEditor::OnTerrainWireframeColor(wxCommandEvent& In)
{
	...
	if(CurrentTerrain)
	{
		...
		Data.SetColour(wxColour(CurrentTerrain->WireframeColor.R, CurrentTerrain->WireframeColor.G, CurrentTerrain->WireframeColor.B));

		wxColourDialog Dlg(this, &Data);
		if(Dlg.ShowModal() == wxID_OK)
		{
			Data = Dlg.GetColourData();
			CurrentTerrain->WireframeColor.R = Data.GetColour().Red();
			CurrentTerrain->WireframeColor.G = Data.GetColour().Green();
			CurrentTerrain->WireframeColor.B = Data.GetColour().Blue();
			ViewSettingsWireframeColor.Button->SetBackgroundColour(Data.GetColour());
...
void WxTerrainEditor::OnDelete(wxCommandEvent& In)
{
	...
	if (CurrentRightClickIndex != -1)
	{
		...
		if (ClickedInfo)
		{
			...
			FString	DeletePrompt	= FString::Printf(LocalizeSecure(LocalizeUnrealEd("TerrainBrowser_ConfirmDeletion"), GetTerrainLayerTypeString(ClickedInfo->GetLayerType())));
			UBOOL	bDoDelete		= appMsgf(AMT_YesNo, *DeletePrompt);
			if (bDoDelete == FALSE)
			{
				return;
