...
void ULensFlareEditorPropertyWrapper::PostEditChange(UProperty* PropertyThatChanged)
{
...
//	LensFlareComp->BeginDeferredReattach();
	...
	for (TObjectIterator<ULensFlareComponent> It;It;++It)
	{
		if (It->Template == LensFlare)
		{
			...
			{
				LocalLFComp->BeginDeferredReattach();
			}
			else
			{
				LocalLFComp->SetTemplate(LensFlare);
			}
		}
	}
}
...
void WxLensFlareEditor::SetSelectedElement(INT NewSelectedElement)
{
	...
	PropertyWindow->SetObject(NULL, 1, 0, 1);
...
//	LensFlareElementWrapper->Element = SelectedElement;
	...
	PropertyWindow->SetObject(LensFlareElementWrapper, 1, 0, 1);
