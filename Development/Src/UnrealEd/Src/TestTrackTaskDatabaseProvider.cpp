...
// TestTrack library
#ifdef _DEBUG
	#pragma comment( lib, "../External/TestTrack/Lib/Debug/TestTrack.lib" )
#else
	#pragma comment( lib, "../External/TestTrack/Lib/Release/TestTrack.lib" )
#endif
 ...
 * @return	True if successful
 */
UBOOL FTestTrackProvider::ConnectToDatabase( const FString& InServerURL, const FString& InUserName, const FString& InPassword, const FString& InDatabaseName, FString& OutUserRealName, TArray< FString >& OutResolutionValues )
{
	TDLOG( TEXT( "TestTrack: %s" ), *FString( __FUNCTION__ ) );
...
UBOOL FTestTrackProvider::QueryTasks( const FString& InFilterName, TArray< FTaskDatabaseEntry >& OutTaskList )
{
	...
	ANSICHAR* ANSIFilterName = TCHAR_TO_ANSI( *InFilterName );
...
UBOOL FTestTrackProvider::MarkTaskComplete( const UINT InNumber, const FTaskResolutionData& InResolutionData )
{
	...
	ANSICHAR* ANSIUserRealName = TCHAR_TO_ANSI( *UserRealName );
	check( ANSIUserRealName != NULL );
	...
	// NOTE: Comments are often more than 128 characters (default in-place string conversion buffer size), so
	//       we allocate our own storage for the ANSI buffer here
	ANSICHAR* ANSIComments = new ANSICHAR[ InResolutionData.Comments.Len() + 1 ];
	appStrncpyANSI( ANSIComments, TCHAR_TO_ANSI( *InResolutionData.Comments ), InResolutionData.Comments.Len() + 1 );

	ANSICHAR* ANSIResolutionType = TCHAR_TO_ANSI( *InResolutionData.ResolutionType );
	ANSICHAR* ANSIChangelistNumber = TCHAR_TO_ANSI( *FString( appItoa( InResolutionData.ChangelistNumber ) ) );
