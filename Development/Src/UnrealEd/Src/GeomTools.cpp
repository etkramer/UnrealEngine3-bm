...
static FClipSMVertex GetVert(const UStaticMesh* StaticMesh, INT VertIndex)
{
	...
	Result.Color = StaticMesh->LODModels(0).VertexBuffer.VertexColor(VertIndex);
	return Result;
}
...
void TriangulatePoly(TArray<FClipSMTriangle>& OutTris, const FClipSMPolygon& InPoly)
{
...
{
	FClipSMPolygon Result;
	appMemzero(&Result, sizeof(FClipSMPolygon));
...
/** Given a set of points, and current set of cluster centers, iterate to move clusters based on points. */
void GenerateClusterCenters(TArray<FVector>& Clusters, const TArray<FVector>& Points, INT NumIterations)
{
	...
	{
		// Temp storage for each cluster
		TArray<FCluster> ClusterData;
		...
		for(INT i=0; i<Points.Num(); i++)
		{
			...
			{
				FLOAT Dist = (Pos - Clusters(j)).Size();
			...
			check(NearestClusterIndex != INDEX_NONE);
