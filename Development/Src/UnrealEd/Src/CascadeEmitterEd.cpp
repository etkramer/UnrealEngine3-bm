...
//			if (!(Module->IsA(UParticleModuleTypeDataBase::StaticClass()) || 
//				  Module->IsA(UParticleModuleEventGenerator::StaticClass())))
			{
...
void FCascadeEmitterEdViewportClient::DrawDraggedModule(UParticleModule* Module, FViewport* Viewport, FCanvas* Canvas)
{
	...
	if (!Module->IsA(UParticleModuleTypeDataBase::StaticClass()))
	{
		...
		if (Module->GetModuleType() == EPMT_Spawn)
		{
					...
					if (bLODIsValid)
					{
						...
						if (Module || Emitter)
						{
							...
							else
							{
								...
								UParticleLODLevel* LODLevel = Cascade->GetCurrentlySelectedLODLevel(Emitter);
								wxColorIn	= LODLevel ? 
									wxColour(LODLevel->RequiredModule->EmitterEditorColor.R, 
											 LODLevel->RequiredModule->EmitterEditorColor.G, 
											 LODLevel->RequiredModule->EmitterEditorColor.B) :
