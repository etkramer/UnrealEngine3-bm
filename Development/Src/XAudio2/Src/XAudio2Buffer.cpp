...
INT FXAudio2SoundBuffer::GetSize( void )
{
	...
	case SoundFormat_PCMRT:
		TotalSize = DecompressionState->SrcBufferDataSize + ( MONO_PCM_BUFFER_SIZE * 2 );
		break;
...
void FXAudio2SoundBuffer::InitXMA2( UXAudio2Device* XAudio2Device, USoundNodeWave* Wave, FXMAInfo* XMAInfo )
{
	...
	if( Wave->HasAnyFlags( RF_RootSet ) )
	{
		...
		XMA2.XMA2Data = ( BYTE* )XAudio2Device->AllocatePermanentMemory( XMA2.XMA2DataSize );
		bAllocationInPermanentPool = TRUE;
	}
	else
	{
...
{
	return( DecompressionState->ReadCompressedData( Destination, bLooping, MONO_PCM_BUFFER_SIZE ) );
}
