...
simulated function SetTexturesToBeResident( float TimeToBeResident )
{
	...
	for( MatIdx = 0; MatIdx < GibMeshComp.Materials.Length; ++MatIdx )
	{
		...
		{
			//`log( "Texture setting SetForceMipLevelsToBeResident(): " $ TimeToBeResident @ GibMeshComp.Materials[MatIdx] @ Textures[TexIdx] );
			Texture2D(Textures[TexIdx]).SetForceMipLevelsToBeResident( TimeToBeResident );
		}
	}
}
