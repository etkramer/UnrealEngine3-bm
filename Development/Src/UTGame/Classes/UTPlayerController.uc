...
var AudioComponent HeartBeatSoundComponent;
...
exec function ChangeClass(string NewClassName)
{
	...
	if (NewClassName ~= "Corrupt")
	{
		RealClassName = "UTGame.UTFamilyInfo_Liandri";
	}
	else if (NewClassName ~= "NecrisF")
	{
		RealClassName = "UTGame.UTFamilyInfo_Necris_Female";
	}
	else if(NewClassName ~= "NecrisM")
	{
		RealClassName = "UTGame.UTFamilyInfo_Necris_Male";
	}
	else if(NewClassName ~= "HumanF")
	{
		RealClassName = "UTGame.UTFamilyInfo_Human_Female";
	}
	else if(NewClassName ~= "HumanM")
	{
		RealClassName = "UTGame.UTFamilyInfo_Human_Male";
	}
	else if(NewClassName ~= "Rat")
	{
		RealClassName = "UTGame.UTFamilyInfo_Rat";
	}
	else if(NewClassName ~= "Skaarj")
	{
		RealClassName = "UTGame.UTFamilyInfo_Skaarj";
	}
	else if(NewClassName ~= "Krall")
	{
		RealClassName = "UTGame.UTFamilyInfo_Krall";
	}

	if (RealClassName != "")
	{
		`Log("UTPlayerController::ChangeClass() - Changing class to"@NewClassName);
		ServerChangeClass(RealClassName);
	}
	else
	{
		`Log("UTPlayerController::ChangeClass() - Invalid class"@NewClassName);
	}
}

reliable server function ServerChangeClass(string NewClass)
{
	local class<UTFamilyInfo> NewPlayerClass;

	`Log("UTPlayerController::ServerChangeClass() - Changing class to"@NewClass);
	NewPlayerClass = class<UTFamilyInfo>(DynamicLoadObject(NewClass, class'Class'));
	if (NewPlayerClass != None)
	{
		ServerSetCharacterClass(NewPlayerClass);
		if (UTPawn(Pawn) != None)
		{
			UTPawn(Pawn).PlayerChangedClass();
		}
	}
}
...
function UTUIScene_ConnectionStatus OpenProgressMessageScene()
{
	...
	if ( SceneClass != None )
	{
		...
		if ( SceneClient != None )
		{
			...
			{
				ProgressMessageScene = UTUIScene_ConnectionStatus(ProgressMessageScene.OpenScene(ProgressMessageScene, LocalPlayer(Player)));
			}
		}
	}
	else
...
{
	if ( HeartBeatSoundComponent == None )
	{
		HeartBeatSoundComponent = GetPooledAudioComponent(soundcue'Temp_BreathHeart.Heartbeatfastcue', None, true);
		if (HeartBeatSoundComponent == None)
		{
			return;
		}
		HeartBeatSoundComponent.bAllowSpatialization = false;
...
exec function ShowMap()
{
	...
	if ( (UTCTFHUD(myHUD) != None) && (PlayerReplicationInfo != None) && (PlayerReplicationInfo.Team != None) )
	{
...
exec function ShowMenu()
{
	...
	if ( Role == ROLE_Authority && WorldInfo.GRI.bMatchHasBegun && UTGame(WorldInfo.Game) != None &&
		(WorldInfo.IsPlayInEditor() || UTGame(WorldInfo.Game).SinglePlayerMissionID != INDEX_NONE) )
	{
...
function CharacterProcessingComplete()
{
	...
	if ( LP != None )
	{
		if ( OnlineSub != None && !OnlineSub.SystemInterface.IsControllerConnected(LP.ControllerId) )
		{
	...
	if(Profile != none)
	{
		...
		*/
		...
		if(Profile.GetProfileSettingValueIdByName('GoreLevel', OutIntValue))
		{
			...
			if ( class'UTGame'.static.IsLowGoreVersion() )
			{
...
// FIXME - cheat, remove
exec function HeroTest()
{
	ServerHeroTest();
}

reliable server function ServerHeroTest()
{
	UTPlayerReplicationInfo(PlayerReplicationInfo).HeroMeter = UTPlayerReplicationInfo(PlayerReplicationInfo).HeroThreshold + 1;
	ServerTriggerHero();
}
