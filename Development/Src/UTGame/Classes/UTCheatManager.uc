...
exec function DoubleUp()
{
	...
	local class<UTWeap_Enforcer> EnforcerClass;

	EnforcerClass = class<UTWeap_Enforcer>(DynamicLoadObject("UTGameContent.UTWeap_Enforcer_Content", class'Class'));

	if (EnforcerClass != None)
	{
		MyEnforcer = UTWeap_Enforcer(Pawn.FindInventoryType(EnforcerClass));
		MyEnforcer.DenyPickupQuery(EnforcerClass, None);
	}
}
