...
var UTAnimBlendBase		FlyingBlend;
...
replication
{
	...
	if ( bNetDirty )
		...
		WeaponOverlayFlags, CustomGravityScaling, bIsInvisible, BigTeleportCount, bIsHero;
...
event ProvideAuraEffect(Pawn P);
...
/** Called when a player changes class via exec ChangeClass() */
...
simulated event ClearXRayEffect()
{
		...
		FamilyInfo.default.DeathMeshSkelMesh != None )
	{
		if ( !IsInState('Dying') )
		{
			OverlayMesh.SetSkeletalMesh( FamilyInfo.default.CharacterMesh );
			SetOverlayMaterial( None );
		}
	}
}

simulated function SetXRayEffect(bool bTurnOn)
{
	local int i;
	local class<UTFamilyInfo> FamilyInfo;
	...
	SetSkin(Material(GetShieldMaterialInstance(WorldInfo.Game.bTeamGame)));
	if( FamilyInfo != None &&
		FamilyInfo.default.DeathMeshSkelMesh != None )
	{
...
simulated function SetCharacterClassFromInfo(class<UTFamilyInfo> Info)
{
	...
	if (Info != CurrCharClassInfo)
	{
		...
		// Gender
		bIsFemale = Info.default.bIsFemale;

		//Set some properties on the PRI
		if (PRI != None)
		{
			PRI.bIsFemale = bIsFemale;
...
simulated function SetTeamColor()
{
	...
	if ( PRI.Team == None )
	{
		if ( VerifyBodyMaterialInstance() )
		{
			for (i = 0; i < BodyMaterialInstances.length; i++)
			{
				BodyMaterialInstances[i].SetVectorParameterValue('Char_TeamColor', MakeLinearColor(2,2,0,1));
	...
	else if (VerifyBodyMaterialInstance())
	{
		if ( PRI.Team.TeamIndex == 0 )
		{
			for (i = 0; i < BodyMaterialInstances.length; i++)
			{
				BodyMaterialInstances[i].SetVectorParameterValue('Char_TeamColor', MakeLinearColor(2,0,0,1));
				BodyMaterialInstances[i].SetScalarParameterValue('Char_DistSaturateSwitch', 1.0);
			}
		}
		else
		{
			for (i = 0; i < BodyMaterialInstances.length; i++)
			{
				BodyMaterialInstances[i].SetVectorParameterValue('Char_TeamColor', MakeLinearColor(0,0,2,1));
				BodyMaterialInstances[i].SetScalarParameterValue('Char_DistSaturateSwitch', 1.0);
			}
		}
	}
...
simulated event PostInitAnimTree(SkeletalMeshComponent SkelComp)
{
	if (SkelComp == Mesh)
	{
		...
		FlyingBlend = UTAnimBlendBase( mesh.FindAnimNode('FlyingBlend') );
	}
}
		...
		if (EmoteIndex != INDEX_None)
		{
			...
			if(Health > 0 && WorldInfo.NetMode != NM_DedicatedServer && UTPRI != None && !IsInState('FeigningDeath'))
			{
				...
				if(EInfo.EmoteAnim != '')
				{
					...
					if(FamilyInfo == class'UTFamilyInfo_Krall' && EInfo.EmoteAnim == 'Taunt_UB_ComeHere')
					{
...
function AdjustDamage( out int inDamage, out Vector momentum, Controller instigatedBy, Vector hitlocation, class<DamageType> damageType, optional TraceHitInfo HitInfo )
{
	...
	{
		inDamage *= class'UTDmgType_XRay'.default.XRayDamageMult;
	}
	// When x-ray effect is active, damage will ignore armor
	else if( DamageType.default.bArmorStops && (inDamage > 0) )
	{
		PreDamage = inDamage;
...
simulated function SpawnGibs(class<UTDamageType> UTDamageType, vector HitLocation)
{
	...
	if ( !bGibbed )
	{
		...
		if (GetFamilyInfo().default.GibExplosionTemplate != None && EffectIsRelevant(Location, false, 7000))
		{
			...
			{
				GibInfo = CurrCharClassInfo.default.Gibs[i];
...
reliable server function bool BecomeHero()
{
	return false;
}
...
defaultproperties
{
	...
	Begin Object Class=DynamicLightEnvironmentComponent Name=MyLightEnvironment
		...
		AmbientGlow=(R=0.4,G=0.4,B=0.4,A=1.0)
		AmbientShadowColor=(R=0.25,G=0.25,B=0.25)
	End Object
	...
	bRechargeHealth=true
	HealthRechargeRate=1.0
	HealthRechargeDelay=30.0
}
