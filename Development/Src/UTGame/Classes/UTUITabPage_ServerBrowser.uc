...
function OnFindOnlineGamesCompleteDelegate(bool bWasSuccessful)
{
	...
	{
		GameSearch = GameInterface.GetGameSearch();
		if (LastServerAdded < GameSearch.Results.Length)
		{
			LastServerAdded = GameSearch.Results.Length - 1;
			// NOTE: DO NOT DO THIS IN A SHIPPING GAME!!!!
			// GameSpy requires "aux" updates only when requested by the user
			// but the UI is not set up to function this way.
			// This will fail GameSpy cert so do it properly please
			GameInterface.QueryNonAdvertisedData(LastServerAdded,1);
		}
	}
