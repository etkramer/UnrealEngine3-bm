 ...
 */
function TossFlagToPlayer()
{
	local UTPawn BotPawn;
	local UTPlayerReplicationInfo PRI;
	local PlayerController PC;
	local UTPlayerController NearestPC;
	local UTCarriedObject Flag;
	local float NearestPlayerDistSq;
	local float CurrentPlayerDistSq;
	...
	{
		CurrentPlayerDistSq = VSizeSq(PC.Location - Pawn.Location);
		if (NearestPlayerDistSq ~= 0.0 || CurrentPlayerDistSq < NearestPlayerDistSq)
		{
			NearestPlayerDistSq = CurrentPlayerDistSq;
			NearestPC = UTPlayerController(PC);
		}
	}

	// If near the player, hand off the flag / orb
	if (NearestPC != None && NearestPlayerDistSq < Flag.TossDistance * Flag.TossDistance)
	{
		Flag.Drop();
		Flag.SetHolder(NearestPC);
	}
	// Else just drop the flag, and don't pick it up for a while
	else
	{
		Flag.Drop();
		BotPawn.bCanPickupInventory = false;
		SetTimer(10.0, false, 'EnableInventoryPickup', BotPawn);
	}
}
...
function ForceCelebrate()
{
	...
	if ( Enemy == None )
	{
		...
		if ( Squad.SquadLeader.Pawn != None )
			SetFocalPoint( Squad.SquadLeader.Pawn.Location );
	}
	StopFiring();
	// if blew self up, return
	if ( (Pawn == None) || (Pawn.Health <= 0) )
...
Begin:
	...
	if ( FocusOnLeader(false) )
		FireWeaponAt(Focus);
	else if ( (Pawn.Weapon != None) && !Pawn.Weapon.bMeleeWeapon && Squad.ShouldSuppressEnemy(self) && ClearShot(GetFocalPoint(),true) )
	{
