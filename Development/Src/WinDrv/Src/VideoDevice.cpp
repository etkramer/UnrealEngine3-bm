...
void VideoDevice::DetectVertexShaderVersion()
{
	...
	//valid shader version is between 0 and 3
	if(m_dwVertexShaderVersion < 0 || m_dwVertexShaderVersion > 3)
	{
		//Couldn't determine PS support, so assume none
		m_dwVertexShaderVersion = 0;
	}
...
void VideoDevice::DoDeviceDetection(void)
{
	...
	if(m_pD3DObject)
	{
		...
		{
			//Need to have succesfully obtained a D3DADAPTER_IDENTIFER struct before collecting this stuff
			DetectDeviceName();
			DetectVendorID();
			DetectDeviceID();
			DoDxDetection();
