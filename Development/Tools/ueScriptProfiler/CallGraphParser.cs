...
namespace ueScriptProfiler
{
	...
	public class NodePayload
	{
        ...
        /** Maximum inclusive cycles spent on a single call to this node */
        public double   MaxCycles;
	...
	public class CallGraphParser
	{
        ...
        delegate string GetCallGraphTextDelegate(NodePayload Payload, int NumTopLevelCalls);
		 ...
		 */
        private void RecursiveLabelNodes(TreeNode Parent, int NumTopLevelCalls, GetCallGraphTextDelegate GetCallGraphText)
		{
            // Update text with inclusive usec per number of toplevel function calls.
			NodePayload		Payload		= (NodePayload) Parent.Tag;
			FunctionInfo	Function	= (FunctionInfo) PointerToFunctionInfo[ Payload.FunctionPointer ];
            String usecPerCall          = GetCallGraphText(Payload, NumTopLevelCalls);
			Parent.Text			    	= usecPerCall + " usec  " + Function.Name;

			foreach( TreeNode ChildNode in Parent.Nodes )
			{
				// And recurse.
				RecursiveLabelNodes( ChildNode, NumTopLevelCalls, GetCallGraphText );
			}
		}

        private void CreateCallGraph(TreeNode ParentNode, TreeView CallGraph,GetCallGraphTextDelegate GetCallGraphText)
        {
            // Keep track of top level functions in a sortable array.
            ArrayList FunctionsList = new ArrayList();
            foreach (TreeNode Node in ParentNode.Nodes)
            {
                FunctionsList.Add(Node);
            }

            // Recursively apply labels to the callgraph nodes.
            foreach (TreeNode Node in FunctionsList)
            {
                NodePayload Payload = (NodePayload)Node.Tag;
                RecursiveLabelNodes(Node,Payload.Calls,GetCallGraphText);
            }

            // Sort function tree view top nodes.
            NodeTextDescendingComparer Comparer = new NodeTextDescendingComparer();
            FunctionsList.Sort(Comparer);
		 ...
		 */
		public void ParseStream( String Filename, TreeView CallGraph, TreeView SpikeCallGraph, TreeView ExpensiveFunctions, ListView SortedView, ProgressBar LoadingProgress )
		{
			// Empty views as we might be opening a stream for the second time.
			CallGraph.Nodes.Clear();
            SpikeCallGraph.Nodes.Clear();
					...
					else if( Token.IsCycleCount() )
					{
                        ...
                        if (CallStack.Count != 0)
                        {
                            ...
                            else
                            {
                                ...
                                Payload.MaxCycles = Math.Max(Payload.MaxCycles, Token.GetValue());
            ...
            TreeNode TopExpensiveFunctionNode = (TreeNode)TopNode.Clone();
            TreeNode TopSpikeFunctionNode = (TreeNode)TopNode.Clone();
            ...
            CreateCallGraph(
                TopExpensiveFunctionNode,
                ExpensiveFunctions,
                delegate(NodePayload Payload, int NumTopLevelCalls)
                {
                    return String.Format("{0:0.00}", usecsPerCycle * Payload.Cycles / NumTopLevelCalls).PadLeft(9, ' ');
                }
                );

            CreateCallGraph(
                TopSpikeFunctionNode,
                SpikeCallGraph,
                delegate(NodePayload Payload, int NumTopLevelCalls)
                {
                    return String.Format("{0:0.00}", usecsPerCycle * Payload.MaxCycles).PadLeft(9, ' ');
                }
                );
