...
namespace UnrealDatabaseProxy
{
	...
	public class ClientConnection : IDisposable
	{
		...
		unsafe void OnRecv(object sender, SocketAsyncEventArgs e)
		{
			try
			{
				if(e.BytesTransferred > 0)
				{
					fixed(byte* Ptr = &mRecvBuf[0])
					{
						char* CharPtr = (char*)Ptr;
						for(int i = 0; i < e.BytesTransferred; i += sizeof(char), ++CharPtr)
						{
							if(*CharPtr != 0)
							{
								mCurrentCmd.Append((char)IPAddress.NetworkToHostOrder((short)(*CharPtr)));
							}
							else
							{
								//System.Threading.ThreadPool.QueueUserWorkItem(OnProcessCommand, mCurrentCmd.ToString());
								OnProcessCommand(mCurrentCmd.ToString());
								mCurrentCmd.Length = 0;
							}
						}
					}
		...
		void OnProcessCommand(object State)
		{
