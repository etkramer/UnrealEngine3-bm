                ...
                try
                {
                    ...
                    foreach( Tasks.TasksRow Task in Tasks.Tables[0].Rows )
                    {
                        ...
                        if( DateTime.Now > Task.ScheduleTime )
                        {
                            ...
                            if( UPMS_Service.UPDSList.Count > 0 )
                            {
                                ...
                                if( BestUPDSName.Length > 0 && MinRunningTasks < MaxTasksOnUPDS )
                                {
                                    ...
                                    if( Task.Recurring )
                                    {
                                        ...
                                        DataHelper.Task_AddNew( Task.PlatformBuildID, NewTime, Task.ClientMachineID, Task.UserName, Task.RunAfterProp, Task.BuildConfig, Task.CommandLine, Task.Recurring );
                                    }
                                }
                                else
                                {
                            ...
                            {
                                Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Important, "Cannot find any UPDS to execute task ID: {0}!", Task.ID );
                            }
                        }
                    }
