    ...
    public partial class TasksTableAdapter : global::System.ComponentModel.Component {
        ...
        private void InitCommandCollection() {
            ...
            this._commandCollection[0].CommandText = @"SELECT Tasks.ID, Tasks.RunAfterProp, UPDSNames.UPDSName AS AssignedUPDS, Tasks.ClientMachineID, Tasks.SubmissionTime, Tasks.ScheduleTime, Tasks.Recurring, Tasks.CompletionTime, BuildConfigs.BuildConfig AS BuildConfig, Tasks.StatusID, 
                         ...
                         PlatformBuilds.Title AS Title, Projects.Title AS Project, TaskStatuses.Description AS Status, Tasks.Priority, UserNames.UserName AS UserName, ClientMachines.Path AS Path, ClientMachines.Name AS FriendlyName, Tasks.UserNameID AS TaskerUserNameID, ClientMachines.UserNameID AS TaskeeUserNameID, ClientMachines.Reboot AS Reboot
FROM [Tasks] 
...
INNER JOIN BuildConfigs ON Tasks.BuildConfigID = BuildConfigs.ID
            ...
            this._commandCollection[2].CommandText = "SELECT Tasks.ID, Tasks.RunAfterProp, UPDSNames.UPDSName AS AssignedUPDS, Tasks.Cl" +
                ...
                "Tasks.Priority, UserNames.UserName AS UserName, ClientMachines.Path AS Path, Cli" +
                "entMachines.Name AS FriendlyName, Tasks.UserNameID AS TaskerUserNameID, ClientMa" +
                "chines.UserNameID AS TaskeeUserNameID, ClientMachines.Reboot AS Reboot\r\nFROM [Ta" +
                "sks] \r\nINNER JOIN UserNames ON Tasks.UserNameID = UserNames.ID\r\nINNER JOIN Error" +
                "s ON Tasks.ErrorID = Errors.ID\r\nINNER JOIN UPDSNames ON Tasks.AssignedUPDSID = U" +
                "PDSNames.ID \r\nINNER JOIN CommandLines ON Tasks.CommandLineID = CommandLines.ID\r\n" +
                "INNER JOIN ClientMachines ON Tasks.ClientMachineID = ClientMachines.ID \r\nINNER J" +
                "OIN PlatformBuilds ON Tasks.PlatformBuildID = PlatformBuilds.ID \r\nINNER JOIN Pla" +
                "tforms ON ClientMachines.PlatformID = Platforms.ID \r\nINNER JOIN Projects ON Plat" +
                "formBuilds.ProjectID = Projects.ID \r\nINNER JOIN TaskStatuses ON Tasks.StatusID =" +
                " TaskStatuses.ID \r\nINNER JOIN BuildConfigs ON Tasks.BuildConfigID = BuildConfigs" +
                ".ID\r\n                         WHERE ( Tasks.StatusID = 1 )\r\n                    " +
                "     ORDER BY Tasks.ScheduleTime ASC";
