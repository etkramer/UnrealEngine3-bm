...
const TCHAR *       g_tszFXNames[] = { L"Chorus", L"Compressor", L"Distortion", L"Echo",
                                       L"Flanger", L"Gargle", L"Param Eq", L"Reverb" };
...
VOID OnInitDialog( HWND hDlg )
{
    ...
    if( GetWindowsDirectory( strFile, MAX_PATH ) == 0 )
        return;
    StringCchCat( strFile, MAX_PATH, L"\\media\\ding.wav" );
...
HRESULT OnPlaySound( HWND hDlg )
{
    ...
    // We would only use CTRLFX control on DirectSound buffer
                          ...
                          DWORD dwPrecision = 1 )
{
    ...
    {
        case 0:
            StringCchPrintf( tszstr, MAX_PATH, L"%.0f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.0f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.0f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        case 1:
            StringCchPrintf( tszstr, MAX_PATH, L"%.1f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.1f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.1f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
        default:
        case 2:
            StringCchPrintf( tszstr, MAX_PATH, L"%.2f", val );
            SendMessage( GetDlgItem( hwndDlg, id + 1 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.2f", min );
            SendMessage( GetDlgItem( hwndDlg, id + 3 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            StringCchPrintf( tszstr, MAX_PATH, L"%.2f", max );
            SendMessage( GetDlgItem( hwndDlg, id + 4 ), WM_SETTEXT, 0, ( LPARAM ) tszstr );
            break;
    }
...
VOID LoadParameterUI( HWND hwndDlg, DWORD dwFXType )
{
    ...
    StringCchPrintf( tszstr, MAX_PATH, L"Parameters for [ %s ]", g_tszFXNames[dwFXType] );
...
HRESULT ValidateWaveFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    else // The load call succeeded
    {
        ...
        if( pwfx && pwfx->wFormatTag != WAVE_FORMAT_PCM && pwfx->wFormatTag != WAVE_FORMAT_IEEE_FLOAT )
        {
        ...
        StringCchCopy( g_tszFilename, MAX_PATH, strFileName );
        return S_OK;
    }
}
