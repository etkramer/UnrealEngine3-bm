    ...
    if( '\0' == strPath[0] )
    {
        if( GetWindowsDirectory( strPath, MAX_PATH ) != 0 )
        {
            if( wcscmp( &strPath[wcslen(strPath)], L"\\" ) )
                StringCchCat( strPath, 260, L"\\" );
            StringCchCat( strPath, 260, TEXT("MEDIA") );
        }
    }
...
VOID ValidateWaveFile( HWND hDlg, TCHAR* strFileName )
{
    ...
    else // The load call succeeded
    {
        ...
        StringCchCopy( m_strWaveFileName, MAX_PATH, strFileName );
...
VOID OnSliderChanged( HWND hDlg )
{
    ...
    StringCchPrintf( strBuffer, 10, TEXT("%ld"), lFrequency );
    SetWindowText( GetDlgItem( hDlg, IDC_FREQUENCY ), strBuffer );

    StringCchPrintf( strBuffer, 10, TEXT("%ld"), lPan );
    SetWindowText( GetDlgItem( hDlg, IDC_PAN       ), strBuffer );

    StringCchPrintf( strBuffer, 10, TEXT("%ld"), lVolume );
...
VOID UpdateBehaviorText( HWND hDlg )
{
    ...
    if( bFocusSticky )
    {
        StringCchCopy( strExcepted, 1024, L"Buffers with \"sticky\" focus will continue to play "
    ...
    {
        StringCchCopy( strExcepted, 1024, L"Buffers with global focus will continue to play if the user "
    ...
    {
        // Normal focus
        StringCchCopy( strExcepted, 1024, L"Buffers with normal focus will mute if the user switches "
