...
namespace Luminance
{
    ...
    HRESULT CreateResources( IDirect3DDevice9 *pDevice, const D3DSURFACE_DESC* pDisplayDesc )
    {
        ...
        //-------------------------------
            ...
            for( int i = 0; i < Luminance::g_dwLumTextures; i++ )
            {
                ...
                if(FAILED(hr))
                {
                    ...
                    WCHAR str[MAX_PATH];
                    StringCchPrintf(str, MAX_PATH, L"Luminance::CreateResources() : Unable to create luminance"
        ...
        //-------------------------------
            ...
            if(FAILED(hr))
            {
                ...
                OutputDebugString( L"Luminance::CreateResources() - Compiling of 'LuminanceDisplay' from 'Luminance.psh' failed!\n" );
                return hr;

            }

            V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &Luminance::g_pLumDispPS ) );
            if(FAILED(hr))
            {

                // Couldn't turn the compiled shader into an actual, usable, pixel shader!
                OutputDebugString( L"Luminance::CreateResources() - Could not create a pixel shader object for 'LuminanceDisplay'.\n" );
                pCode->Release( );
                return hr;
        ...
        //-----------------------------------------
            ...
            if(FAILED(hr))
            {
                ...
                // Couldn't compile the shader, use the 'compile_shaders.bat' script
                // in the 'Shader Code' folder to get a proper compile breakdown.
                OutputDebugString( L"Luminance::CreateResources() - Compiling of 'GreyScaleDownSample' from 'Luminance.psh' failed!\n" );
                return hr;
            ...
            if(FAILED(hr))
            {
                ...
                // Couldn't turn the compiled shader into an actual, usable, pixel shader!
                OutputDebugString( L"Luminance::CreateResources() - Could not create a pixel shader object for 'GreyScaleDownSample'.\n" );
                pCode->Release( );
                return hr;
        ...
        //--------------------------------------
            ...
            V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &Luminance::g_pLum3x3DSPS ) );
            if(FAILED(hr))
            {
    ...
    HRESULT MeasureLuminance( IDirect3DDevice9 *pDevice )
    {
        ...
        if(FAILED(pDestTex->GetSurfaceLevel( 0, &pDestSurf )))
        {
            ...
            OutputDebugString( L"Luminance::MeasureLuminance( ) : Couldn't acquire surface level for hi-res luminance map!\n" );
            return E_FAIL;
    ...
    //
    //--------------------------------------------------------------------------------------
    HRESULT DisplayLuminance( IDirect3DDevice9 *pDevice, ID3DXFont* pFont, ID3DXSprite *pTextSprite, IDirect3DTexture9* pArrowTex )
    {
        ...
        if( FAILED( pDevice->GetRenderTarget( 0, &pSurf ) ) )
        {
            ...
            OutputDebugString( L"Luminance::DisplayLuminance() - Could not get current render target to extract dimensions.\n" );
            return E_FAIL;
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX ) + 2, static_cast< int >( (2.0f * fCellH) + 32.0f + fLumCellSize ) - 24 );
            ...
            WCHAR str[100];
            StringCchPrintf(str,100,L"%dx%d",d.Width,d.Height);
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX ) + 2, static_cast< int >( (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize) ) - 24 );
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX ) + 2, static_cast< int >( (2.0f * fCellH) + 64.0f + (3.0f * fLumCellSize) ) - 24 );
        ...
        //--------------------------
        v[0].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 64.0f + (3.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 64.0f + (3.0f * fLumCellSize), 0.0f, 1.0f );
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX + fLumCellSize + 16.0f ) + 2, static_cast< int >( (2.0f * fCellH) + 64.0f + (3.0f * fLumCellSize) ) - 24 );
        ...
        //--------------------------
        v[0].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX + fLumCellSize + 16.0f ) + 2, static_cast< int >( (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize) ) - 24 );
        ...
        //--------------------------
        v[0].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 32.0f + (0.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 32.0f + (0.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,            (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >( fLumStartX + fLumCellSize + 16.0f ) + 2, static_cast< int >( (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize) ) - 24 );
        ...
        v[0].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) - 8.0f,    (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) + 8.0f,    (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) - 8.0f,    (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) + 8.0f,    (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );

        pDevice->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, v, sizeof( Luminance::TLVertex ) );

        // From 2nd down to 3rd

        v[0].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) - 8.0f,    (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) + 8.0f,    (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) - 8.0f,    (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (fLumCellSize / 2.0f) + 8.0f,    (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        ...
        v[1].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,    (2.0f * fCellH) + 56.0f + (2.5f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + fLumCellSize,            (2.0f * fCellH) + 72.0f + (2.5f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + fLumCellSize + 16.0f,    (2.0f * fCellH) + 72.0f + (2.5f * fLumCellSize), 0.0f, 1.0f );
        ...
        v[0].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 8.0f,    (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 24.0f,   (2.0f * fCellH) + 48.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 8.0f,    (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 24.0f,   (2.0f * fCellH) + 64.0f + (2.0f * fLumCellSize), 0.0f, 1.0f );

        pDevice->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, v, sizeof( Luminance::TLVertex ) );

        // Up from 5th to 6th

        v[0].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 8.0f,    (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 24.0f,   (2.0f * fCellH) + 32.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 8.0f,    (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (1.5f * fLumCellSize) + 24.0f,   (2.0f * fCellH) + 48.0f + (1.0f * fLumCellSize), 0.0f, 1.0f );
        ...
        v[0].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 24.0f + (0.5f * fLumCellSize), 0.0f, 1.0f );
        v[1].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 32.0f,   (2.0f * fCellH) + 24.0f + (0.5f * fLumCellSize), 0.0f, 1.0f );
        v[2].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 16.0f,   (2.0f * fCellH) + 40.0f + (0.5f * fLumCellSize), 0.0f, 1.0f );
        v[3].p = D3DXVECTOR4( fLumStartX + (2.0f * fLumCellSize) + 32.0f,   (2.0f * fCellH) + 40.0f + (0.5f * fLumCellSize), 0.0f, 1.0f );
