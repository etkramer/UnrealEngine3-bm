...
namespace HDRScene
{
    ...
    HRESULT CreateResources( IDirect3DDevice9 *pDevice, const D3DSURFACE_DESC* pDisplayDesc )
    {
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"HDRScene::CreateResources() - Could not create floating point render target. Examine D3D Debug Output for details.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"HDRScene::CreateResources() : Couldn't create a pixel shader object from 'HDRSource.psh'.\n" );
            pCode->Release( );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"HDRScene::CreateResources() - Couldn't create a vertex declaration for the HDR-Cube mesh.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"HDRScene::CreateResources() - Could not create a VS object from the compiled 'HDRSource.vsh' code.\n" );
            pCode->Release( );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // Couldn't create the declaration for the loaded mesh..
            OutputDebugString( L"HDRScene::CreateResources() - Couldn't create a vertex declaration for the occlusion mesh.\n" );
            return hr;
        ...
        V( pDevice->CreateVertexShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &HDRScene::g_pOcclusionVS ) );
        if(FAILED(hr))
        {

            // Couldn't turn the compiled shader into an actual, usable, vertex shader!
            OutputDebugString( L"HDRScene::CreateResources() - Could not create a VS object from the compiled 'OcclusionMesh.vsh' code.\n" );
            pCode->Release( );
            return hr;
