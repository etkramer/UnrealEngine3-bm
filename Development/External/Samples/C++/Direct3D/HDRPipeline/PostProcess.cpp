...
namespace PostProcess
{
    ...
    HRESULT CreateResources( IDirect3DDevice9 *pDevice, const D3DSURFACE_DESC *pDisplayDesc )
    {
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"PostProcess::CreateResources() - Could not create bright-pass render target. Examine D3D Debug Output for details.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            OutputDebugString( L"PostProcess::CreateResources() - Compiling of 'BrightPass' from 'PostProcessing.psh' failed!\n" );
            return hr;

        }

        V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &PostProcess::g_pBrightPassPS ) );
        if(FAILED(hr))
        {

            // Couldn't turn the compiled shader into an actual, usable, pixel shader!
            OutputDebugString( L"PostProcess::CreateResources() - Could not create a pixel shader object for 'BrightPass'.\n" );
            pCode->Release( );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // We couldn't create the texture - lots of possible reasons for this...
            OutputDebugString( L"PostProcess::CreateResources() - Could not create downsampling render target. Examine D3D Debug Output for details.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // Couldn't compile the shader, use the 'compile_shaders.bat' script
            // in the 'Shader Code' folder to get a proper compile breakdown.
            OutputDebugString( L"PostProcess::CreateResources() - Compiling of 'DownSample' from 'PostProcessing.psh' failed!\n" );
            return hr;

        }

        V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &PostProcess::g_pDownSamplePS ) );
        if(FAILED(hr))
        {

            // Couldn't turn the compiled shader into an actual, usable, pixel shader!
            OutputDebugString( L"PostProcess::CreateResources() - Could not create a pixel shader object for 'DownSample'.\n" );
            pCode->Release( );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // We couldn't create the texture - lots of possible reasons for this...
            OutputDebugString( L"PostProcess::CreateResources() - Could not create horizontal bloom render target. Examine D3D Debug Output for details.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // Couldn't compile the shader, use the 'compile_shaders.bat' script
            // in the 'Shader Code' folder to get a proper compile breakdown.
            OutputDebugString( L"PostProcess::CreateResources() - Compiling of 'HorizontalBlur' from 'PostProcessing.psh' failed!\n" );
            return hr;

        }

        V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &PostProcess::g_pHBloomPS ) );
        if(FAILED(hr))
        {

            // Couldn't turn the compiled shader into an actual, usable, pixel shader!
            OutputDebugString( L"PostProcess::CreateResources() - Could not create a pixel shader object for 'HorizontalBlur'.\n" );
            pCode->Release( );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // We couldn't create the texture - lots of possible reasons for this...
            OutputDebugString( L"PostProcess::CreateResources() - Could not create vertical bloom render target. Examine D3D Debug Output for details.\n" );
            return hr;
        ...
        if(FAILED(hr))
        {
            ...
            // Couldn't compile the shader, use the 'compile_shaders.bat' script
            // in the 'Shader Code' folder to get a proper compile breakdown.
            OutputDebugString( L"PostProcess::CreateResources() - Compiling of 'VerticalBlur' from 'PostProcessing.psh' failed!\n" );
            return hr;

        }

        V( pDevice->CreatePixelShader( reinterpret_cast< DWORD* >( pCode->GetBufferPointer() ), &PostProcess::g_pVBloomPS ) );
        if(FAILED(hr))
        {

            // Couldn't turn the compiled shader into an actual, usable, pixel shader!
            OutputDebugString( L"PostProcess::CreateResources() - Could not create a pixel shader object for 'VerticalBlur'.\n" );
            pCode->Release( );
            return hr;
    ...
    HRESULT PerformPostProcessing( IDirect3DDevice9 *pDevice )
    {
        ...
        if( FAILED( PostProcess::g_pBrightPassTex->GetSurfaceLevel( 0, &pBrightPassSurf ) ) )
        {
            ...
            OutputDebugString( L"PostProcess::PerformPostProcessing() - Couldn't retrieve top level surface for bright pass render target.\n" );
            return E_FAIL;
        ...
        if( FAILED( PostProcess::g_pDownSampledTex->GetSurfaceLevel( 0, &pDownSampleSurf ) ) )
        {
            ...
            // Can't get the render target. Not good news!
            OutputDebugString( L"PostProcess::PerformPostProcessing() - Couldn't retrieve top level surface for down sample render target.\n" );
            return E_FAIL;
        ...
        if( FAILED( PostProcess::g_pBloomHorizontal->GetSurfaceLevel( 0, &pHBloomSurf ) ) )
        {
            ...
            // Can't get the render target. Not good news!
            OutputDebugString( L"PostProcess::PerformPostProcessing() - Couldn't retrieve top level surface for horizontal bloom render target.\n" );
            return E_FAIL;
        ...
        if( FAILED( PostProcess::g_pBloomVertical->GetSurfaceLevel( 0, &pVBloomSurf ) ) )
        {
            ...
            // Can't get the render target. Not good news!
            OutputDebugString( L"PostProcess::PerformPostProcessing() - Couldn't retrieve top level surface for vertical bloom render target.\n" );
            return E_FAIL;
    ...
    HRESULT DisplaySteps( IDirect3DDevice9 *pDevice, ID3DXFont* pFont, ID3DXSprite *pTextSprite, IDirect3DTexture9* pArrowTex )
    {
        ...
        if( FAILED( pDevice->GetRenderTarget( 0, &pSurf ) ) )
        {
            ...
            OutputDebugString( L"PostProcess::DisplaySteps() - Could not get current render target to extract dimensions.\n" );
            return E_FAIL;
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >(2.0f * fCellW + 32.0f) + 5, static_cast< int >( fCellH - 25.0f ) );
        ...
        txtHelper.Begin( );
        {
            txtHelper.SetInsertionPos( static_cast< int >(3.0f * fCellW + 48.0f) + 5, static_cast< int >( fCellH - 25.0f ) );
