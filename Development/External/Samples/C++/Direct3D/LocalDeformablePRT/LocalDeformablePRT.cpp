...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V( D3DXSHProjectCubeMap( 6, g_Skybox.GetEnvironmentMap(), g_fSkyBoxLightSH[0], g_fSkyBoxLightSH[1], g_fSkyBoxLightSH[2] ) );
...
HRESULT LoadTechniqueObjects( const char* szMedia )
{
    ...
    StringCchCopyA( strTechnique, MAX_PATH, strComboTech );
    ...
    if( D3DFMT_Q16W16V16U16 != g_fmtTexture && D3DFMT_Q8W8V8U8 != g_fmtTexture )
        StringCchCatA( strTechnique, MAX_PATH, "_Unbias" );
    ...
    // Load the mesh
    StringCchPrintfW( strFileName, MAX_PATH, TEXT("media\\%S"), szMedia ); 
    V_RETURN(LoadLDPRTData( pDevice, strFileName ));

    // Albedo texture
    StringCchPrintfW(strFileName, MAX_PATH, TEXT("media\\%SAlbedo.dds"), szMedia); 
    ...
    // Normal map 
    StringCchPrintfW(strFileName, MAX_PATH, TEXT("media\\%SNormalMap.dds"), szMedia); 
    DXUTFindDXSDKMediaFileCch(strPath, MAX_PATH, strFileName);
...
HRESULT LoadLDPRTData( IDirect3DDevice9* pd3dDevice, WCHAR* strFilePrefixIn )
{
    ...
    StringCchPrintfW(strFilePrefix, MAX_PATH, TEXT("%s"), strFilePrefixIn); strFilePrefix[MAX_PATH-1] = 0;
    StringCchPrintfW(strFileName, MAX_PATH, TEXT("%s.x"), strFilePrefix); strFileName[MAX_PATH-1] = 0;
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        V( g_LightControl.OnRender9( D3DXCOLOR(1, 1, 1, 1), (D3DXMATRIX*)g_Camera.GetViewMatrix(), (D3DXMATRIX*)g_Camera.GetProjMatrix(), g_Camera.GetEyePt() ) );
        DXUT_EndPerfEvent();
    ...
    { 
        ...
        for( UINT iPaletteEntry = 0; iPaletteEntry < pMeshContainer->NumPaletteEntries; ++iPaletteEntry )
        {
            ...
            if( iMatrixIndex != UINT_MAX) 
                D3DXMatrixMultiply(&BoneMatrices[iPaletteEntry], &pMeshContainer->pBoneOffsetMatrices[iMatrixIndex], pMeshContainer->ppBoneMatrixPtrs[iMatrixIndex]);
        }
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_TECHNIQUE:        LoadTechniqueObjects("bat"); break;
    }
}
