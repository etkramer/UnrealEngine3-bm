...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
...

        {
            ...
            // Show frame rate
            SetRect( &rc, 2, 0, 0, 0 );
            g_pStatsFont->DrawText( NULL, DXUTGetFrameStats( DXUTIsVsyncEnabled() ), -1, &rc, DT_NOCLIP, D3DXCOLOR( 1.0f, 1.0f, 0.0f, 1.0f ));
            SetRect( &rc, 2, 15, 0, 0 );
    ...
    switch( nControlID )
    {
        ...
        case IDC_CHANGEFONT:    
        {
            ...
            if( ChooseFont( &cf ) )
            {
                ...
                if( SUCCEEDED( D3DXCreateFont( DXUTGetD3D9Device(), nHeight, 0, bBold ? FW_BOLD : FW_NORMAL, 1, bItalic, DEFAULT_CHARSET, 
                                               OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, 
                                               pstrFontNameNew, &pFontNew ) ) )
                {
                    if( SUCCEEDED( CreateD3DXTextMesh( DXUTGetD3D9Device(), &pMesh3DTextNew, pstrFontNameNew, dwFontSizeNew, bBold, bItalic ) ) )
                    {
                        bSuccess = true;
