...
void InitApp()
{
   ...
   iY += ciPadding;
   StringCchPrintf( sz, s_iMAX_STRING_SIZE, L"Minium number of samples: %d", g_nMinSamples ); 
   g_SampleUI.AddStatic( IDC_MIN_NUM_SAMPLES_STATIC, sz, ciLeft, iY, ciWidth * 2, ciHeight );
   g_SampleUI.GetStatic( IDC_MIN_NUM_SAMPLES_STATIC )->GetElement( 0 )->dwTextFormat = DT_LEFT | DT_TOP | DT_WORDBREAK;
                                    ...
                                    void*               pUserContext )
{
   ...
   if( pDeviceSettings->d3d9.DeviceType != D3DDEVTYPE_REF && 
       (( caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) == 0 || caps.VertexShaderVersion < D3DVS_VERSION(3, 0)) )
   {
...
void SwitchPOMTextures( DXUTComboBoxItem* pSelectedItem)
{
   ...
   if ( pMinStatic )
   {
      StringCchPrintf( strLabel, s_iMAX_STRING_SIZE, L"Minium number of samples: %d", g_nMinSamples ); 
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_MIN_NUM_SAMPLES_SLIDER:
        {
            ...
            WCHAR strLabel[s_iMAX_STRING_SIZE];
            StringCchPrintf( strLabel, s_iMAX_STRING_SIZE, L"Minimum number of samples: %d", g_nMinSamples ); 
