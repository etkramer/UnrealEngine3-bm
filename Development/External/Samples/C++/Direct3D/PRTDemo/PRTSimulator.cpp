...
CPRTSimulator::CPRTSimulator(void)
{
    ...
    m_dwThreadId = 0;
    m_fPercentDone = 0.0f;
    StringCchCopy( m_strCurPass, 256, L"" );
}
...
//--------------------------------------------------------------------------------------
HRESULT CPRTSimulator::RunInWorkerThread( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CPRTMesh* pPRTMesh )
{
    if( IsRunning() ) 
        return E_FAIL;
...
//--------------------------------------------------------------------------------------
HRESULT CPRTSimulator::CompressInWorkerThread( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CPRTMesh* pPRTMesh )
{
    if( IsRunning() ) 
        return E_FAIL;
    ...
    m_nCurPass = 1;
    m_nNumPasses = 1;
    StringCchCopy( m_strCurPass, 256, L"Compressing Buffer" );
...
DWORD CPRTSimulator::PRTCompressThreadProc()
{
    ...
    hr = m_pPRTMesh->CompressPRTBuffer( D3DXSHCQUAL_SLOWHIGHQUALITY, m_Options.dwNumClusters, m_Options.dwNumPCA, StaticPRTSimulatorCB, NULL ); 
    if( FAILED(hr ) )
...
DWORD CPRTSimulator::PRTSimulationThreadProc()
{
    ...
    m_nCurPass = 1;
    m_fPercentDone = -1.0f;
    StringCchCopy( m_strCurPass, 256, L"Initializing PRT engine" );
    ...
    if( !m_Options.bSubsurfaceScattering )
    {
        ...
        if( m_Options.bAdaptive && m_Options.bRobustMeshRefine ) 
        {
            m_nCurPass++;
            m_fPercentDone = -1.0f;
            StringCchCopy( m_strCurPass, 256, L"Robust Mesh Refine" );
        ...
        m_nCurPass++;
        StringCchCopy( m_strCurPass, 256, L"Computing Direct Lighting" );
        m_fPercentDone = 0.0f;
        ...
        {
            m_nCurPass++;
            StringCchPrintf( m_strCurPass, 256, L"Computing Bounce %d Lighting", iBounce+1 );
            m_fPercentDone = 0.0f;
    ...
    else
    {
        ...
        m_nCurPass++;
        StringCchCopy( m_strCurPass, 256, L"Computing Subsurface Direct Lighting" );
        ...
        for( UINT iBounce=1; iBounce<m_Options.dwNumBounces; ++iBounce )
        {
            ...
            m_nCurPass++;
            StringCchPrintf( m_strCurPass, 256, L"Computing Subsurface Bounce %d Lighting", iBounce+1 );
            hr = m_pPRTEngine->ComputeSS( pBufferA, pBufferB, pDataTotal );
            if( FAILED(hr ) )
                goto LEarlyExit; // handle user aborting simulator via callback 
        }
    ...
    StringCchCopy( szBaseFile, MAX_PATH, m_Options.strResultsFile );
