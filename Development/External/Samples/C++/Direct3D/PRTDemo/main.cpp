...
void InitApp()
{
    ...
    iY += 10;
    StringCchPrintf( sz, 100, L"Light scale: %0.2f", 0.0f ); sz[99] = 0;
    g_RenderingUI2.AddStatic( IDC_LIGHT_SCALE_STATIC, sz, 35, iY += 24, 125, 22 );
    g_RenderingUI2.AddSlider( IDC_LIGHT_SCALE, 50, iY += 24, 100, 22, 0, 200, (int)(g_fLightScaleForSHTechs * 100.0f) );

    StringCchPrintf( sz, 100, L"# Lights: %d", g_nNumActiveLights ); sz[99] = 0;
    ...
    StringCchPrintf( sz, 100, L"Cone Angle: 1" ); sz[99] = 0;
...
void RenderPRT( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        if( g_bRenderEnvMap && !g_bRenderWireframe )
        {
            ...
            g_LightProbe[g_dwLightProbeA].Render( &mViewProjection, (1.0f-fEnvBlendScaler), fEnv1Scaler, g_bRenderSHProjection );
            g_LightProbe[g_dwLightProbeB].Render( &mViewProjection, (fEnvBlendScaler), fEnv2Scaler, g_bRenderSHProjection );
        ...
        if( g_bRenderMesh )
        {
            switch( g_AppTechnique )
            {
                ...
                case APP_TECH_LDPRT: 
                {
                    g_PRTMesh.RenderWithLDPRT( pd3dDevice, &mWorldViewProjection, NULL, false, g_bRenderWithAlbedoTexture );
                    break;
                }
            }
...
void UpdateLightingEnvironment()
{
    ...
    D3DXSHScale( fLightProbe1[0], dwOrder, g_LightProbe[g_dwLightProbeA].GetSHData(0), fEnv1Scaler*(1.0f-fEnvBlendScaler) );
    D3DXSHScale( fLightProbe1[1], dwOrder, g_LightProbe[g_dwLightProbeA].GetSHData(1), fEnv1Scaler*(1.0f-fEnvBlendScaler) );
    D3DXSHScale( fLightProbe1[2], dwOrder, g_LightProbe[g_dwLightProbeA].GetSHData(2), fEnv1Scaler*(1.0f-fEnvBlendScaler) );
    ...
    D3DXSHScale( fLightProbe2[0], dwOrder, g_LightProbe[g_dwLightProbeB].GetSHData(0), fEnv2Scaler*fEnvBlendScaler );
    D3DXSHScale( fLightProbe2[1], dwOrder, g_LightProbe[g_dwLightProbeB].GetSHData(1), fEnv2Scaler*fEnvBlendScaler );
    D3DXSHScale( fLightProbe2[2], dwOrder, g_LightProbe[g_dwLightProbeB].GetSHData(2), fEnv2Scaler*fEnvBlendScaler );
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
    if( bKeyDown )
    {
        switch( nChar )
        {
            ...
            case VK_F8:
            case 'W': g_bRenderWireframe = !g_bRenderWireframe; g_RenderingUI2.GetCheckBox( IDC_WIREFRAME )->SetChecked(g_bRenderWireframe); break;
        }
    }
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_SCENE_1: KeyboardProc( 'Z', true, false, NULL ); break;
        case IDC_SCENE_2: KeyboardProc( 'X', true, false, NULL ); break;
        case IDC_SCENE_3: KeyboardProc( 'C', true, false, NULL ); break;
        case IDC_SCENE_4: KeyboardProc( 'V', true, false, NULL ); break;
        case IDC_SCENE_5: KeyboardProc( 'B', true, false, NULL ); break;
        ...
        case IDC_NUM_LIGHTS:
            ...
            {
                WCHAR sz[100];
                StringCchPrintf( sz, 100, L"# Lights: %d", g_RenderingUI2.GetSlider( IDC_NUM_LIGHTS )->GetValue() ); sz[99] = 0;
        ...
        case IDC_LIGHT_SCALE: 
        {
            WCHAR sz[100];
            float fLightScale = (float) (g_RenderingUI2.GetSlider( IDC_LIGHT_SCALE )->GetValue() * 0.01f);
            StringCchPrintf( sz, 100, L"Light scale: %0.2f", fLightScale ); sz[99] = 0;
        ...
        case IDC_LIGHT_ANGLE:
        {
            ...
            StringCchPrintf( sz, 100, L"Cone Angle: %d", nLightAngle ); sz[99] = 0;
        ...
        case IDC_TECHNIQUE_LDPRT:   
        {
            ...
            switch( nControlID )
            {
                case IDC_TECHNIQUE_NDOTL:   g_AppTechnique = APP_TECH_NDOTL; break;
            ...
            g_RenderingUI2.EnableNonUserEvents( true );
            float fOldLightScale = (( g_AppTechnique == APP_TECH_NDOTL ) ? g_fLightScaleForNdotL : g_fLightScaleForSHTechs);
        ...
        case IDC_COMPRESSION:
            ...
            {
                MessageBox( DXUTGetHWND(), L"To change compression settings during rendering, please load an uncompressed buffer.  To make one use the simulator settings dialog to save an uncompressed buffer.", L"PRTDemo", MB_OK );
            }
            break;
        ...
        case IDC_WIREFRAME: g_bRenderWireframe = g_RenderingUI2.GetCheckBox( IDC_WIREFRAME )->GetChecked(); break;
...
void LoadScene( IDirect3DDevice9* pd3dDevice, WCHAR* strInputMesh, WCHAR* strResultsFile )
{
    ...
    if( FAILED(hr) )
    {
        StringCchCopy( g_strLastError, MAX_PATH, L"Error loading mesh file" );
        g_AppState = APP_STATE_STARTUP;
        return;
    }
    ...
    if( FAILED(hr) )
    {
        StringCchCopy( g_strLastError, MAX_PATH, L"Error loading results file" );
        g_AppState = APP_STATE_STARTUP;
        return;
    }
    ...
    if( FAILED( g_PRTMesh.LoadLDPRTFromFiles( szLDPRTFile, szNormalsFile ) ) )
    {
        ...
        if( FAILED(hr) )
        {
            StringCchCopy( g_strLastError, MAX_PATH, L"Error creating LDPRT data" );
            g_AppState = APP_STATE_STARTUP;
            return;
        }
    }
...
HRESULT FindPRTMediaFile( WCHAR* strDestPath, int cchDest, LPCWSTR strFilename, bool bCreatePath )
{
    ...
    StringCchCat( strPath, MAX_PATH, L"\\" );
