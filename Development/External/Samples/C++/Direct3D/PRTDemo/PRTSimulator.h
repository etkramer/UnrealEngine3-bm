...
public:
    ...
    HRESULT RunInWorkerThread( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CPRTMesh* pPRTMesh );
    HRESULT CompressInWorkerThread( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CPRTMesh* pPRTMesh );

    bool    IsRunning();
    bool    IsCompressing() { return m_bCompressInWorkerThread; }
    HRESULT Stop();
