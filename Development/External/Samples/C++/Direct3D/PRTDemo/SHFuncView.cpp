...
HRESULT CSHFunctionViewer::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice )
{
    ...
    D3DXCreateMeshFVF( m_uNumPhi*2 + (m_uNumTheta-3)*(m_uNumPhi*2), 2+m_uNumPhi*(m_uNumTheta-2), D3DXMESH_MANAGED, D3DFVF_DIFFUSE|D3DFVF_NORMAL|D3DFVF_XYZ, pd3dDevice,&m_pSphereMesh);
...
//--------------------------------------------------------------------------------------
void CSHFunctionViewer::Render( IDirect3DDevice9* pd3dDevice, float fObjectRadius, D3DXMATRIX& mWorld, D3DXMATRIX& mView, D3DXMATRIX& mProj )
{
    ...
    const float fMatScale = 1.0f/sqrtf(mJustRot(0,0)*mJustRot(0,0) + mJustRot(0,1)*mJustRot(0,1) + mJustRot(0,2)*mJustRot(0,2));
    for(int i=0;i<3;i++) 
    {
        for(int j=0; j<3; j++) 
        {
...
// Updates the transfer visualization data under the currently selected vertex 
//-----------------------------------------------------------------------------
void CSHFunctionViewer::GetVertexUnderMouse( CPRTMesh* pPRTMesh, int nAppTechnique, const D3DXMATRIX* pmProj, const D3DXMATRIX* pmView, const D3DXMATRIX* pmWorld )
{
...
void CSHFunctionViewer::CompDerivs(float sT, float cT, float sP, float cP, float retval[2][36])
{
    ...
    retval[0][34] = -0.3e1f / 0.16e2f * t174 * t52 * t10 * (0.40e2f * t156 * t24 - t157 - 0.40e2f * t89 + t141 + t78 - 0.1e1f) * t3;
