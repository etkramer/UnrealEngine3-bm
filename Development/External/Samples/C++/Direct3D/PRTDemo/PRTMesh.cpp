...
HRESULT CPRTMesh::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice, D3DFORMAT fmt )
{
    ...
    for(i=0; i < 9; i++) 
    {
        ...
        D3DXFillCubeTexture(m_pSHBasisTextures[i], StaticFillCubeTextureWithSHCallback, (LPVOID) (LONG_PTR) (i*4));
    }
...
HRESULT CPRTMesh::LoadMesh( IDirect3DDevice9* pd3dDevice, WCHAR* strMeshFileName )
{
    ...
    StringCchCopy( m_ReloadState.strMeshFileName, MAX_PATH, str );
...
HRESULT CPRTMesh::LoadPRTBufferFromFile( WCHAR* strFile )
{
    ...
    StringCchCopy( m_ReloadState.strPRTBufferFileName, MAX_PATH, str );
...
HRESULT CPRTMesh::LoadLDPRTFromFiles( WCHAR* strLDPRTFile, WCHAR* strShadingNormalsFile )
{
    ...
    StringCchCopy( m_ReloadState.strLDPRTFile, MAX_PATH, strLDPRTFile );
    StringCchCopy( m_ReloadState.strShadingNormalsFile, MAX_PATH, strShadingNormalsFile );
    ...
    HANDLE hFile = CreateFile( strShadingNormalsFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
    if( hFile )
    {
        DWORD dwRead;
...
HRESULT CPRTMesh::SaveLDPRTToFiles( WCHAR* strLDPRTFile, WCHAR* strShadingNormalsFile )
{
    ...
    HANDLE hFile = CreateFile( strShadingNormalsFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
    if( hFile )
    {
        DWORD dwWritten;
...
//--------------------------------------------------------------------------------------
HRESULT CPRTMesh::CompressPRTBuffer( D3DXSHCOMPRESSQUALITYTYPE Quality, UINT NumClusters, UINT NumPCA, LPD3DXSHPRTSIMCB pCB, LPVOID lpUserContext )
{ 
    HRESULT hr;
...
{ 
    ...
    m_pPRTBuffer->AddRef();
    m_dwPRTOrder = GetOrderFromNumCoeffs( m_pPRTBuffer->GetNumCoeffs() );
    StringCchCopy( m_ReloadState.strPRTBufferFileName, MAX_PATH, strFile );
}
...
HRESULT CPRTMesh::LoadEffects( IDirect3DDevice9* pd3dDevice, const D3DCAPS9* pDeviceCaps )
{
    ...
    StringCchPrintfA( szMaxNumClusters, 64, "%d", dwNumClusters );
    szMaxNumClusters[63] = 0;
    CHAR szMaxNumPCA[64];
    StringCchPrintfA( szMaxNumPCA, 64, "%d", dwNumPCA );
    szMaxNumPCA[63] = 0;
...
void CPRTMesh::ComputeShaderConstants( float* pSHCoeffsRed, float* pSHCoeffsGreen, float* pSHCoeffsBlue, DWORD dwNumCoeffsPerChannel )
{
    ...
    for( DWORD iCluster = 0; iCluster < dwNumClusters; iCluster++ ) 
    {
        ...
        m_aPRTConstants[iCluster*dwClusterStride + 1] = D3DXSHDot( dwOrder, &m_aPRTClusterBases[iCluster*dwBasisStride + 1*dwNumCoeffs], pSHCoeffsGreen );
        m_aPRTConstants[iCluster*dwClusterStride + 2] = D3DXSHDot( dwOrder, &m_aPRTClusterBases[iCluster*dwBasisStride + 2*dwNumCoeffs], pSHCoeffsBlue );
        ...
        for( DWORD iPCA = 0; iPCA < dwNumPCA; iPCA++ ) 
        {
            ...
            pPCAStart[0*dwNumPCA + iPCA] = D3DXSHDot( dwOrder, &m_aPRTClusterBases[nOffset + 0*dwNumCoeffs], pSHCoeffsRed );
            pPCAStart[1*dwNumPCA + iPCA] = D3DXSHDot( dwOrder, &m_aPRTClusterBases[nOffset + 1*dwNumCoeffs], pSHCoeffsGreen );
            pPCAStart[2*dwNumPCA + iPCA] = D3DXSHDot( dwOrder, &m_aPRTClusterBases[nOffset + 2*dwNumCoeffs], pSHCoeffsBlue );
        }
    }
...
//--------------------------------------------------------------------------------------
void CPRTMesh::ComputeLDPRTConstants(float* pSHCoeffsR, float* pSHCoeffsG, float* pSHCoeffsB, DWORD dwNumCoeffsPerChannel )
{
    HRESULT hr;
...
//--------------------------------------------------------------------------------------
void CPRTMesh::RenderWithLDPRT(IDirect3DDevice9* pd3dDevice, D3DXMATRIX* pmWorldViewProj, D3DXMATRIX *pmNormalXForm, bool bUniform, bool bRenderWithAlbedo )
{
    ...
    for (iPass = 0; iPass < cPasses; iPass++)
    {
        ...
        {            
            if ( bRenderWithAlbedo ) 
            {
                ...
                {
                    // if no texture for this attrib, then just use the material's diffuse color
                    V( m_pLDPRTEffect->SetValue("MaterialDiffuseColor", &m_pMaterials[i].MatD3D.Diffuse, sizeof(D3DCOLORVALUE) ) );
                }
    ...
    switch( uTechnique )
    {
        ...
        case 4: // APP_TECH_NEAR_LOSSLESS_PRT
        {
            ...
            if( pPCAWts )
            {
                ...
                {
                    pfVals[i] = m_aPRTClusterBases[uClusterID*uNumCoeffs*uNumChannels*(uNumPCA+1)+uChan*uNumCoeffs + i];
                }
    
                for(j=0;j<uNumPCA;j++)
                {
                    float *pStartBasis = m_aPRTClusterBases + uClusterID*uNumCoeffs*uNumChannels*(uNumPCA+1)+(j+1)*uNumCoeffs*uNumChannels + uChan*uNumCoeffs;
                    for(i=0;i<uNumCoeffs;i++)
                    {
...
//--------------------------------------------------------------------------------------
void CPRTMesh::GetVertexUnderMouse( const D3DXMATRIX* pmProj, const D3DXMATRIX* pmView, const D3DXMATRIX* pmWorld, unsigned int *puVert)
{
    POINT pt;
    GetCursorPos( &pt );
