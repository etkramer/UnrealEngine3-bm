...
//-----------------------------------------------------------------------------
HRESULT CLightProbe::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice, const WCHAR* strCubeMapFile, bool bCreateSHEnvironmentMapTexture )
{
    HRESULT hr;
    ...
    if( desc.Format != D3DFMT_A16B16G16R16F )
    {
        ...
        hr = D3DXCreateCubeTextureFromFileEx( pd3dDevice, strPath, 512, 1, 0, D3DFMT_A16B16G16R16F, 
                            D3DPOOL_SCRATCH, D3DX_FILTER_LINEAR, D3DX_FILTER_LINEAR, 0, NULL, NULL, &pScratchEnvironmentMap );
        if( SUCCEEDED(hr) )
        {
    ...
    if( bCreateSHEnvironmentMapTexture )
    {
        ...
        V( D3DXCreateCubeTexture( pd3dDevice, 256, 1, 0, D3DFMT_A16B16G16R16F, D3DPOOL_MANAGED, &m_pSHEnvironmentMap ) );
        SHCubeProj projData;
