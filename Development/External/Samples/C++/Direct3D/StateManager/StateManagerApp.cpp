...
void InitApp()
{
    ...
    WCHAR szMessage[200];
    StringCchPrintf( szMessage, 200, L"# Rocks: %d", g_nMaxRocksToRender ); szMessage[199] = 0;
    ...
    StringCchCopy( szMessage, 200, L"Goal: 0.0ms/frame" ); szMessage[199] = 0;
    ...
    StringCchCopy( szMessage, 200, L"Remaining for logic: 0%%" ); szMessage[199] = 0;
    g_SampleUI.AddStatic( IDC_CPUUSAGE_STATIC, szMessage, 0, iY += 18, 200, 22 );

    StringCchCopy( szMessage, 200, L"If rendering takes less\n"
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
    if( bKeyDown )
    {
        switch( nChar )
        {
            ...
            case 'U': g_nShowUI--; if( g_nShowUI < 0 ) g_nShowUI = 2; break;
        }
    }
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_TOGGLEUI:         KeyboardProc( 'U', true, false, NULL ); break;  

        case IDC_NUM_ROCKS:
            g_nMaxRocksToRender =  g_SampleUI.GetSlider( IDC_NUM_ROCKS )->GetValue();

            StringCchPrintf( szMessage, 100, L"# Rocks: %d", g_nMaxRocksToRender ); szMessage[99] = 0;
