...
bool ParseCommandLine( SETTINGS* pSettings )
{
    ...
    for( int iArg=1; iArg<nNumArgs; iArg++ )
    {
        ...
        if( *strCmdLine == L'/' || *strCmdLine == L'-' )
        {
            ...
            if( IsNextArg( strCmdLine, L"fa" ) )
            {
                ...
                if( iArg+1 < nNumArgs )
                {
                    ...
                    StringCchCopy( pSettings->szAdjacencyFilename, 256, strArg );
                    continue;
                }
            ...
            if( IsNextArg( strCmdLine, L"fe" ) )
            {
                if( iArg+1 < nNumArgs )
                {
                    ...
                    StringCchCopy( pSettings->szFalseEdgesFilename, 256, strArg );
                    continue;
                }
            ...
            if( IsNextArg( strCmdLine, L"o" ) )
            {
                ...
                if( iArg+1 < nNumArgs )
                {
                    ...
                    StringCchCopy( pSettings->szOutputFilename, 256, strArg );
                    continue;
                }
            ...
            if( IsNextArg( strCmdLine, L"it" ) )
            {
                ...
                {
                    strArg = pstrArgList[++iArg];
                    StringCchCopy( pSettings->szIMTInputFilename, 256, strArg );
                    pSettings->bIMT = true;
            ...
            if( IsNextArg( strCmdLine, L"rt" ) )
            {
                if( iArg+1 < nNumArgs )
                {
                    ...
                    strArg = pstrArgList[++iArg];
                    StringCchCopy( pSettings->szResampleTextureFile, 256, strArg );
                    continue;
                }
        ...
        else 
        {
            ...
            if( strNewArg )
            {
                StringCchCopy( strNewArg, nArgLen+1, strCmdLine );
...
void SearchSubdirsForFile( IDirect3DDevice9* pd3dDevice, WCHAR* strDir, WCHAR* strFile, SETTINGS* pSettings )
{
    ...
    StringCchCopy( strSearchDir, MAX_PATH, strDir );
    StringCchCat( strSearchDir, MAX_PATH, L"*" );
    ...
    if( hFindFile != INVALID_HANDLE_VALUE )
    {
        ...
        while( bSuccess )
        {
            if( (fileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0 )
            {
                ...
                {
                    StringCchCopy( strFullPath, MAX_PATH, strDir );
                    StringCchCat( strFullPath, MAX_PATH, fileData.cFileName );
                    StringCchCat( strFullPath, MAX_PATH, L"\\" );
...
void SearchDirForFile( IDirect3DDevice9* pd3dDevice, WCHAR* strDir, WCHAR* strFile, SETTINGS* pSettings )
{
    ...
    WCHAR strSearchDir[MAX_PATH];
    StringCchCopy( strSearchDir, MAX_PATH, strDir );
    StringCchCat( strSearchDir, MAX_PATH, strFile );
...
//--------------------------------------------------------------------------------------
HRESULT PerVertexPRTIMT(ID3DXMesh *pMesh, ID3DXPRTBuffer *pPRTBuffer, const UINT cuNumCoeffs, ID3DXBuffer **ppIMTData) {
    HRESULT hr;
    ...
    if( FAILED(hr = D3DXComputeIMTFromPerVertexSignal(pMesh, pPRTSignal, cuNumCoeffs, uStride, 0L, NULL, NULL, ppIMTData)))
    {
...
//--------------------------------------------------------------------------------------
HRESULT TextureSignalIMT(IDirect3DDevice9 *pDevice, ID3DXMesh *pMesh, DWORD dwTextureIndex, const WCHAR *szFilename, ID3DXBuffer **ppIMTData) {
    HRESULT hr;
...
//--------------------------------------------------------------------------------------
HRESULT PRTSignalIMT(ID3DXMesh *pMesh, DWORD dwTextureIndex, SETTINGS* pSettings, const UINT cuNumCoeffs, ID3DXBuffer **ppIMTData) {
    HRESULT hr;
    FLOAT  *pPRTSignal = NULL;
    ...
    if( FAILED(hr = D3DXComputeIMTFromPerTexelSignal(pMesh, dwTextureIndex, pPRTSignal, pPRTBuffer->GetWidth(), pPRTBuffer->GetHeight(), cuNumCoeffs, cuDimension, 0L, NULL, NULL, ppIMTData)))
    {
...
HRESULT ProcessFile( IDirect3DDevice9* pd3dDevice, WCHAR* strFile, SETTINGS* pSettings )
{
    ...
    if( FAILED(hr = StringCbLengthW(strFile, sizeof(WCHAR)*1024, &ustrLen)) )
    {
        wprintf(L"Unable to get string length.\n");
        goto FAIL;
    }
    ...
    if( FAILED(hr = StringCbCopyN(strResult, (ustrLen + 8)*sizeof(WCHAR), strFile, uPeriodPos * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to cat original string.\n");
        goto FAIL;
    }

    if( FAILED(hr = StringCbCatN(strResult + uPeriodPos, (ustrLen + 8 - uPeriodPos)*sizeof(WCHAR), L"_result", 7 * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to append _result to original string.\n");
        goto FAIL;
    }

    if( FAILED(hr = StringCbCatN(strResult + uPeriodPos + 7, (ustrLen + 8 - uPeriodPos - 7)*sizeof(WCHAR), strFile + uPeriodPos, (ustrLen - uPeriodPos) * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to append rest of original string.\n");
        goto FAIL;
    }

    if( FAILED(hr = StringCbCopyN(strResultTexture, (ustrLen + 9)*sizeof(WCHAR), strFile, uPeriodPos * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to cat original string.\n");
        goto FAIL;
    }

    if( FAILED(hr = StringCbCatN(strResultTexture + uPeriodPos, (ustrLen + 9 - uPeriodPos)*sizeof(WCHAR), L"_texture", 8 * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to append _texture to original string.\n");
        goto FAIL;
    }

    if( FAILED(hr = StringCbCatN(strResultTexture + uPeriodPos + 8, (ustrLen + 9 - uPeriodPos - 8)*sizeof(WCHAR), strFile + uPeriodPos, (ustrLen - uPeriodPos) * sizeof(WCHAR))) )
    {
        wprintf(L"Unable to append rest of original string.\n");
        goto FAIL;
    }
    ...
    const D3DVERTEXELEMENT9* pDeclElement = GetDeclElement(decl, D3DDECLUSAGE_TEXCOORD, pSettings->nOutputTextureIndex);
    ...
    if( FALSE == CheckMeshValidation(pMesh, &pMeshValid, &pAdjacency, pSettings->bTopologicalAdjacency, pSettings->bGeometricAdjacency, pOrigAdj) )
    {
    ...
    if (pSettings->bIMT) {
        ...
        {
            wprintf(L"Computing IMT from %s, Index %d\n", declCrack.DeclUsageToString(pSettings->VertexSignalUsage), pSettings->VertexSignalIndex);
            hr = PerVertexIMT(pMesh, decl, pSettings->VertexSignalUsage, pSettings->VertexSignalIndex, &declCrack, &pIMTBuffer);
        } else
        {
            hr = E_FAIL;
            assert(false);
                                      ...
                                      &numchartsOut)) )
    {
        ...
        switch(hr)
        {
            case D3DXERR_INVALIDMESH: wprintf(L"Non-manifold mesh\n"); break;
            default:
    ...
    if( pSettings->bResampleTexture )
    {
        ...
        wprintf(L"Resampling texture %s using data from %s[%d]\n", pSettings->szResampleTextureFile, TraceD3DDECLUSAGEtoString(pSettings->ResampleTextureUsage), pSettings->ResampleTextureUsageIndex );
        ...
        if( FAILED(hr = D3DXCreateTextureFromFileEx(pd3dDevice, pSettings->szResampleTextureFile, 
            ...
            D3DX_DEFAULT, D3DX_DEFAULT, 0, &TextureInfo, NULL, &pOriginalTex)) )
        {
            wprintf(L"Texture creation and loading failed: ");
            switch(hr)
            {
            case D3DERR_NOTAVAILABLE: wprintf(L"This device does not support the queried technique. \n"); break;
            case D3DERR_OUTOFVIDEOMEMORY: wprintf(L"Microsoft Direct3D does not have enough display memory to perform the operation.\n"); break;
            case D3DERR_INVALIDCALL: wprintf(L"The method call is invalid. For example, a method's parameter may have an invalid value.\n"); break;
            case D3DXERR_INVALIDDATA: wprintf(L"The data is invalid.\n"); break;
        ...
        if( FAILED(hr = D3DXCreateTextureGutterHelper(TextureInfo.Width, TextureInfo.Height, pTexResampleMesh, pSettings->gutter, &pGutterHelper)) )
        {
        ...
        if( FAILED(hr = pGutterHelper->ResampleTex(pOriginalTex, pTexResampleMesh, D3DDECLUSAGE_TEXCOORD, 1, pResampledTex)) )
        {
        ...
        if( FAILED(hr = StringCbLengthW(pSettings->szResampleTextureFile, sizeof(WCHAR)*1024, &ustrLen)) )
        {
            wprintf(L"Unable to get string length.\n");
            goto FAIL;
        }
        ...
        for( UINT i = 0; i < ustrLen; i++ )
            if( (pSettings->szResampleTextureFile)[i] == L'.' )
                uPeriodPos = i;
        if( FAILED(hr = StringCbCopyN(szResampledTextureFile, (ustrLen + 11)*sizeof(WCHAR), pSettings->szResampleTextureFile, uPeriodPos * sizeof(WCHAR))) )
        {
            wprintf(L"Unable to copy original string.\n");
            goto FAIL;
        }
        if( FAILED(hr = StringCbCatN(szResampledTextureFile + uPeriodPos, (ustrLen + 11 - uPeriodPos)*sizeof(WCHAR), L"_resampled", 10 * sizeof(WCHAR))) )
        {
            wprintf(L"Unable to append _resampled to original string.\n");
            goto FAIL;
        }
        if( FAILED(hr = StringCbCatN(szResampledTextureFile + uPeriodPos + 10, (ustrLen + 11 - uPeriodPos - 10)*sizeof(WCHAR), pSettings->szResampleTextureFile + uPeriodPos, (ustrLen - uPeriodPos) * sizeof(WCHAR))) )
        {
            wprintf(L"Unable to append rest of original string.\n");
            goto FAIL;
        }
        ...
        if( FAILED(hr = D3DXSaveTextureToFile(szResampledTextureFile, TextureInfo.ImageFileFormat, pResampledTex, NULL)) )
        {
    ...
    {
        StringCchCopy( strOutputFilename, MAX_PATH, pSettings->szOutputFilename );
    }
    else
    {
        StringCchCopy( strOutputFilename, MAX_PATH, strResult );
    }
    ...
    if( pSettings->bResampleTexture && !pSettings->bColorMesh && szResampledTextureFile )
    {
        ...
        WideCharToMultiByte(CP_ACP, 0, pSettings->szResampleTextureFile, -1, szResampleTextureFileA, (int) ustrLen*sizeof(CHAR), NULL, NULL);
        szResampleTextureFileA[ustrLen] = 0;

        WideCharToMultiByte(CP_ACP, 0, szResampledTextureFile, -1, szResampledTextureFileA, (int) (ustrLen + 11)*sizeof(CHAR), NULL, NULL);
        szResampledTextureFileA[ustrLen + 11] = 0;

        for(DWORD index = 0; index < dwNumMaterials; index++)
        {
            if( 0 == strcmp( ((D3DXMATERIAL*)(pMaterials->GetBufferPointer()) + index )->pTextureFilename, szResampleTextureFileA ) )
            {
                ((D3DXMATERIAL*)(pMaterials->GetBufferPointer()) + index )->pTextureFilename = szResampledTextureFileA;
            }
        }
    }
    ...
    if( FAILED(hr = D3DXSaveMeshToX(strOutputFilename,
                                    pMeshResult,
                                    NULL,
                                    pSettings->bColorMesh?pGeneratedMaterials:(D3DXMATERIAL*)pMaterials->GetBufferPointer(),
                                    NULL,
...
HRESULT LoadFile( WCHAR *szFile, ID3DXBuffer **ppBuffer ) 
{
    ...
    *ppBuffer = NULL;
    ...
    hFile = CreateFile( szFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    if( !hFile ) 
        goto LCleanup;
