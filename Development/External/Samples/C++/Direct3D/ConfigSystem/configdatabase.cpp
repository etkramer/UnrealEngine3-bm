            ...
            if( SUCCEEDED(hr) )
            {
                ...
                if( pKsPropertySet )
                {
                    ...
                    if( SUCCEEDED(hr) )
                    {
                        ...
                        // Fill in the info.
                        StringCchCopy( pSndDev->name, 64, pDataW->Description );
                        StringCchCopy( wszModule, MAX_PATH, pDataW->Module );
                        ...
                        else
                        {
                            ...
                            StringCchCopy( wszInterface, 512, pDataW->Interface );
                        }
                    }
                    ...
                    StringCchCat( wszDriverPath, MAX_PATH, L"\\" );
                    ...
                    {
                        ::GetSystemDirectory( wszDriverPath, MAX_PATH );
                        StringCchCat( wszDriverPath, MAX_PATH, L"\\drivers\\" );
                        StringCchCat( wszDriverPath, MAX_PATH, wszModule );
                    }
...
bool CPropertySet::Get(const char* Property, char* Value, int cbValue)
{
    ...
    while (it != end)
    {
        ...
        if (0 == _stricmp(pr.first.c_str(), Property))
        {
            StringCbCopyA( Value, cbValue, pr.second.c_str() );
            return true;
        }
        ++it;
    }
...
void CConfigDatabase::SyntaxError( char* ErrorText )
{
    if( !m_fError )
    {
        ...
        *dest=0;
        ...
        StringCchPrintfA( Buffer, k_cchBuffer, "%s on line %d - '%s'", ErrorText, m_LineNumber, tempBuffer );
                                ...
                                DWORD CPUSpeed )
{
    ...
    if( hFile == INVALID_HANDLE_VALUE )
    {
        ...
        if( GetCurrentDirectory( MAX_PATH, wszBadFile ) == 0 )
            wszBadFile[0] = 0;            
        StringCchCat( wszBadFile, MAX_PATH, L"\\" );
        StringCchCat( wszBadFile, MAX_PATH, FileName );
        StringCchPrintf( wszErrorMsg, MAX_PATH+128, L"Cannot find '%s'", wszBadFile );
    ...
    do
    {
        ...
        if( *m_pchCurrent!=13 && *(WORD*)m_pchCurrent!='//' )       // Ignore comments and blank lines
        {
            ...
            if (NextStringIs(k_KeywordMaxOverallGraphicDetail))
            {
                ...
                //  StringCchPrintf works best because it will keep extra spaces or comments out
                ...
                if (dwMaxOGD != (DWORD)-1)
                {
                    ...
                    StringCchPrintfA(maxValue, 16, "%d", dwMaxOGD);
