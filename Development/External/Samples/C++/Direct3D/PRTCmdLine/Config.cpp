...
HRESULT COptionsFile::SaveOptions( WCHAR* strFile, SIMULATOR_OPTIONS* pOptions )
{
    ...
    for( DWORD iMesh=0; iMesh<pOptions->dwNumMeshes; iMesh++ )
    {
        ...
        for( DWORD iSH=0; iSH<pInputMesh->dwNumSHMaterials; iSH++ )
        {
            ...
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"Absorption.r", pInputMesh->pSHMaterials[iSH].Absorption.r );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"Absorption.g", pInputMesh->pSHMaterials[iSH].Absorption.g );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"Absorption.b", pInputMesh->pSHMaterials[iSH].Absorption.b );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"EnableSubsurfaceScattering", (DWORD)pInputMesh->pSHMaterials[iSH].bSubSurf );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"RelativeIndexOfRefraction", pInputMesh->pSHMaterials[iSH].RelativeIndexOfRefraction );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"ReducedScattering.r", pInputMesh->pSHMaterials[iSH].ReducedScattering.r );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"ReducedScattering.g", pInputMesh->pSHMaterials[iSH].ReducedScattering.g );
            CXMLHelper::CreateNewValue( pDoc, pSHMaterialNode, L"ReducedScattering.b", pInputMesh->pSHMaterials[iSH].ReducedScattering.b );
    ...
    CXMLHelper::CreateNewValue( pDoc, pMTNode, L"RobustMeshRefineMinEdgeLength", pOptions->fRobustMeshRefineMinEdgeLength );
    ...
    CXMLHelper::CreateNewValue( pDoc, pMTNode, L"AdaptiveBounceMinEdgeLength", pOptions->fAdaptiveBounceMinEdgeLength );
...
HRESULT COptionsFile::LoadOptions( WCHAR* strFile, SIMULATOR_OPTIONS* pOptions )
{
    ...
    for( int iMesh=0; iMesh<nNumMeshes; iMesh++ )
    {
        ...
        for( DWORD iSH=0; iSH<pInputMesh->dwNumSHMaterials; iSH++ )
        {
            ...
            CXMLHelper::GetValue( pNode, L"RelativeIndexOfRefraction", &pInputMesh->pSHMaterials[iSH].RelativeIndexOfRefraction );
    ...
    else
    {
        ...
        StringCchCopy( szBaseName, MAX_PATH, strFile );
        WCHAR* strLastDot = wcsrchr( szBaseName, L'.' );
        if( strLastDot )
            *strLastDot = 0;

        StringCchCopy( pOptions->strOutputConcatPRTMesh, MAX_PATH, szBaseName );
        StringCchCat( pOptions->strOutputConcatPRTMesh, MAX_PATH, L".x" );

        StringCchCopy( pOptions->strOutputConcatBlockerMesh, MAX_PATH, szBaseName );
        StringCchCat( pOptions->strOutputConcatBlockerMesh, MAX_PATH, L"_blocker.x" );

        StringCchCopy( pOptions->strOutputTessellatedMesh, MAX_PATH, szBaseName );
        StringCchCat( pOptions->strOutputTessellatedMesh, MAX_PATH, L"_tessellated.x" );

        pOptions->bBinaryXFile = false;

        StringCchCopy( pOptions->strOutputPRTBuffer, MAX_PATH, szBaseName );
        StringCchCat( pOptions->strOutputPRTBuffer, MAX_PATH, L"_prtresults.prt" );

        StringCchCopy( pOptions->strOutputCompPRTBuffer, MAX_PATH, szBaseName );
        StringCchCat( pOptions->strOutputCompPRTBuffer, MAX_PATH, L"_compprtresults.pca" );
    }
...
HRESULT COptionsFile::ResetOptions( SIMULATOR_OPTIONS* pOptions )
{
    ...
    StringCchCopy( pOptions->strOutputConcatPRTMesh, MAX_PATH, L"meshConcat.x" );
    StringCchCopy( pOptions->strOutputConcatBlockerMesh, MAX_PATH, L"blockerConcat.x" );
    StringCchCopy( pOptions->strOutputTessellatedMesh, MAX_PATH, L"tesslatedMesh.x" );
    StringCchCopy( pOptions->strOutputPRTBuffer, MAX_PATH, L"prtbuffer.prt" );
    StringCchCopy( pOptions->strOutputCompPRTBuffer, MAX_PATH, L"prtCompBuffer.pca" );   
    pOptions->bBinaryXFile = false;
    ...
    for( DWORD iMesh=0; iMesh<pOptions->dwNumMeshes; iMesh++ )
    {
        StringCchCopy( pOptions->pInputMeshes[iMesh].strMeshFile, MAX_PATH, L"PRT Demo\\LandShark.x" );
        ...
        pOptions->pInputMeshes[iMesh].pSHMaterials = new D3DXSHMATERIAL[pOptions->pInputMeshes[iMesh].dwNumSHMaterials];
        for( DWORD iSH=0; iSH<pOptions->pInputMeshes[iMesh].dwNumSHMaterials; iSH++ )
        {
            ...
            pOptions->pInputMeshes[iMesh].pSHMaterials[iSH].RelativeIndexOfRefraction = g_aPredefinedMaterials[0].fRelativeIndexOfRefraction;
            pOptions->pInputMeshes[iMesh].pSHMaterials[iSH].ReducedScattering = g_aPredefinedMaterials[0].ReducedScattering;
        }
    }
