...
// Function-prototypes
//-----------------------------------------------------------------------------
HRESULT RunPRTSimulator( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CONCAT_MESH* pPRTMesh, CONCAT_MESH* pBlockerMesh );
    ...
    while( !bStop )
    {
        ...
        if( pPRTState->bProgressMode )
        {
            if( pPRTState->fPercentDone < 0.0f || DXUTGetGlobalTimer()->GetTime() < fLastPercentAnnounceTime + 5.0f )        
                StringCchPrintf( sz, 256, L"." );
            else
            {
                StringCchPrintf( sz, 256, L"%0.0f%%%%.", pPRTState->fPercentDone*100.0f );
                         ...
                         CONCAT_MESH* pPRTMesh, CONCAT_MESH* pBlockerMesh, SETTINGS* pSettings )
{
    ...
    StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Initializing PRT engine..", prtState.nCurPass, prtState.nNumPasses );
...
//        bSetAlbedoFromMaterial = false;
    ...
    if( !bSubsurfaceScattering )
    {
        ...
        if( pOptions->bEnableTessellation && pOptions->bRobustMeshRefine ) 
        {
            ...
            prtState.nCurPass++;
            prtState.fPercentDone = -1.0f;
            StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Robust Mesh Refine..", prtState.nCurPass, prtState.nNumPasses );
            wprintf( prtState.strCurPass );
            LeaveCriticalSection( &prtState.cs );

            V( pPRTEngine->RobustMeshRefine( pOptions->fRobustMeshRefineMinEdgeLength, pOptions->dwRobustMeshRefineMaxSubdiv ) );
        }
        ...
        EnterCriticalSection( &prtState.cs );
        prtState.nCurPass++;
        StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Computing Direct Lighting..", prtState.nCurPass, prtState.nNumPasses );
        wprintf( prtState.strCurPass );
        ...
        if( pOptions->bEnableTessellation && pOptions->bAdaptiveDL )
        {
            hr = pPRTEngine->ComputeDirectLightingSHAdaptive( pOptions->dwOrder, 
                                                              pOptions->fAdaptiveDLThreshold, pOptions->fAdaptiveDLMinEdgeLength, pOptions->dwAdaptiveDLMaxSubdiv, 
                                                              pDataTotal );
            if( FAILED(hr ) )
        ...
        for( UINT iBounce=1; iBounce<pOptions->dwNumBounces; ++iBounce )
        {
            EnterCriticalSection( &prtState.cs );
            prtState.nCurPass++;
            StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Computing Bounce %d Lighting..", prtState.nCurPass, prtState.nNumPasses, iBounce+1 );
            wprintf( prtState.strCurPass );
            prtState.fPercentDone = 0.0f;
            LeaveCriticalSection( &prtState.cs );

            if( pOptions->bEnableTessellation && pOptions->bAdaptiveBounce )
                hr = pPRTEngine->ComputeBounceAdaptive( pBufferA, pOptions->fAdaptiveBounceThreshold, pOptions->fAdaptiveBounceMinEdgeLength, pOptions->dwAdaptiveBounceMaxSubdiv, pBufferB, pDataTotal );
            else
    ...
    else
    {
        ...
        EnterCriticalSection( &prtState.cs );
        prtState.nCurPass++;
        StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Computing Subsurface Direct Lighting..", prtState.nCurPass, prtState.nNumPasses );
        wprintf( prtState.strCurPass );
        LeaveCriticalSection( &prtState.cs );
        ...
        for( UINT iBounce=1; iBounce<pOptions->dwNumBounces; ++iBounce )
        {
            ...
            if( pOptions->bEnableTessellation && pOptions->bAdaptiveBounce )
                hr = pPRTEngine->ComputeBounceAdaptive( pBufferB, pOptions->fAdaptiveBounceThreshold, pOptions->fAdaptiveBounceMinEdgeLength, pOptions->dwAdaptiveBounceMaxSubdiv, pBufferA, NULL );
            else
            ...
            EnterCriticalSection( &prtState.cs );
            prtState.nCurPass++;
            StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Computing Subsurface Bounce %d Lighting..", prtState.nCurPass, prtState.nNumPasses, iBounce+1 );
            wprintf( prtState.strCurPass );
            LeaveCriticalSection( &prtState.cs );
    ...
    if( pOptions->bEnableTessellation )
    {
        ...
        prtState.nCurPass++;
        prtState.fPercentDone = 0.0f;
        StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Saving tessellated mesh to %s", prtState.nCurPass, prtState.nNumPasses, pOptions->strOutputTessellatedMesh );
        wprintf( prtState.strCurPass );
    ...
    prtState.nCurPass++;
    prtState.fPercentDone = 0.0f;
    StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Saving PRT buffer to %s", prtState.nCurPass, prtState.nNumPasses, pOptions->strOutputPRTBuffer );
    wprintf( prtState.strCurPass );
    prtState.bProgressMode = false;
    LeaveCriticalSection( &prtState.cs );
    ...
    if( pOptions->bEnableCompression )
    {
        ...
        prtState.nCurPass++;
        prtState.fPercentDone = 0.0f;
        StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Compressing PRT buffer..", prtState.nCurPass, prtState.nNumPasses );
        wprintf( prtState.strCurPass );
        LeaveCriticalSection( &prtState.cs );

        // Limit the max number of PCA vectors to # channels * # coeffs
        hr = D3DXCreatePRTCompBuffer( pOptions->Quality, pOptions->dwNumClusters, pOptions->dwNumPCA, StaticPRTSimulatorCB, &prtState, pDataTotal, &pPRTCompBuffer );
        if( FAILED(hr ) )
            goto LEarlyExit; // handle user aborting simulator via callback 
        ...
        prtState.nCurPass++;
        prtState.fPercentDone = 0.0f;
        StringCchPrintf( prtState.strCurPass, 256, L"\nStage %d of %d: Saving Compressed PRT buffer to %s", prtState.nCurPass, prtState.nNumPasses, pOptions->strOutputCompPRTBuffer );
        wprintf( prtState.strCurPass );
        StringCchPrintf( prtState.strCurPass, 256, L"" );
        LeaveCriticalSection( &prtState.cs );
