...
HRESULT LoadMeshes( IDirect3DDevice9* pd3dDevice, SIMULATOR_OPTIONS* pOptions, CONCAT_MESH* pPRTMesh, CONCAT_MESH* pBlockerMesh, SETTINGS* pSettings );
void DisplayUsage();
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    if( settings.aFiles.GetSize() == 0 )
    {
        ...
        if( strNewArg )
        {
            StringCchCopy( strNewArg, 256, L"options.xml" );
...
bool ParseCommandLine( SETTINGS* pSettings )
{
    ...
    for(;;)
    {
        ...
        else if( GetNextArg( strsettings, strArg, 256 ) )
        {
            ...
            StringCchCopy( strNewArg, nArgLen+1, strArg );
...
bool GetNextArg( WCHAR*& strsettings, WCHAR* strArg, int cchArg )
{
    ...
    V( StringCchCopy( strArg, 256, strsettings ) );
...
HRESULT ProcessOptionsFile( IDirect3DDevice9* pd3dDevice, WCHAR* strOptionsFileName, SETTINGS* pSettings )
{
    ...
    StringCchPrintf( sz, 256, L"Reading options file: %s\n", strOptionsFileName );
    wprintf( sz );
    ...
    if( prtMesh.pMesh )
    {
        StringCchPrintf( sz, 256, L"Saving concatenated PRT meshes: %s\n", options.strOutputConcatPRTMesh );
        wprintf( sz );
        if( FAILED( hr = D3DXSaveMeshToX( options.strOutputConcatPRTMesh, prtMesh.pMesh, NULL, prtMesh.materialArray.GetData(), NULL, prtMesh.dwNumMaterials, dwFlags ) ) )
        {
    ...
    {
        StringCchPrintf( sz, 256, L"Saving concatenated blocker meshes: %s\n", options.strOutputConcatBlockerMesh );
        wprintf( sz );
        if( FAILED( hr = D3DXSaveMeshToX( options.strOutputConcatBlockerMesh, blockerMesh.pMesh, NULL, blockerMesh.materialArray.GetData(), NULL, blockerMesh.dwNumMaterials, dwFlags ) ) )
        {
            wprintf( L"Error: Failed saving mesh\n" );
            goto LCleanup;
        }
                    ...
                    CONCAT_MESH* pPRTMesh, CONCAT_MESH* pBlockerMesh, SETTINGS* pSettings )
{
    ...
    for( iMesh=0; iMesh<pOptions->dwNumMeshes; iMesh++ )
    {
        ...
        StringCchCopy( strMediaDir, MAX_PATH, str );
        ...
        for(iMat=0; iMat<dwNumMaterials; iMat++ )
        {
            ...
            if( pMaterials[iMat].pTextureFilename )
            {
                ...
                if( mat.pTextureFilename )
                {
                    ...
                    StringCchCopyA( mat.pTextureFilename, MAX_PATH, strFullPathA );
                }
            }
            else
            {
        ...
        D3DXMatrixScaling( &mScale, pOptions->pInputMeshes[iMesh].vScale.x, pOptions->pInputMeshes[iMesh].vScale.y, pOptions->pInputMeshes[iMesh].vScale.z );
        D3DXMatrixRotationYawPitchRoll( &mRotate, pOptions->pInputMeshes[iMesh].fYaw, pOptions->pInputMeshes[iMesh].fPitch, pOptions->pInputMeshes[iMesh].fRoll );
        D3DXMatrixTranslation( &mTranslate, pOptions->pInputMeshes[iMesh].vTranslate.x, pOptions->pInputMeshes[iMesh].vTranslate.y, pOptions->pInputMeshes[iMesh].vTranslate.z );
    ...
    if( dwNumMeshes > 0 )
    {
        ...
        V_RETURN( D3DXConcatenateMeshes( ppMeshes, dwNumMeshes, dwFlags, pGeomXForms, NULL, NULL, pd3dDevice, &pBlockerMesh->pMesh ) );
    ...
    if( dwNumMeshes > 0 )
    {
        ...
        V_RETURN( D3DXConcatenateMeshes( ppMeshes, dwNumMeshes, dwFlags, pGeomXForms, pUVXForms, NULL, pd3dDevice, &pPRTMesh->pMesh ) );
