...
//=========================================================//
bool CIrradianceCacheGenerator::SetSamplingInfo ( DWORD dwMaxSubdivision, bool bAdaptiveSubdivision, DWORD dwMinSubdivision, float fHMDepthSubdivThreshold )
{
...
//======================================================================================================//
bool CIrradianceCacheGenerator::SetGPUResources ( IDirect3DDevice9* pD3DDevice, ID3DXRenderToEnvMap* pRenderToEnvMap, IDirect3DCubeTexture9* pCubeTexture )
{
...
bool CIrradianceCacheGenerator::CreateCache ( CIrradianceCache** ppCache, bool bFillCache )
{
    ...
    //============================//
    if ( ( NULL == ppCache ) || ( NULL == m_pD3DDevice ) || ( NULL == m_pRenderToEnvMap ) || ( NULL == m_pCubeTexture ) )
    {
        OUTPUT_ERROR_MESSAGE( L"Received NULL pointers!\n" );
        return false;
    }
...
bool CIrradianceCacheGenerator::ProgressiveCacheFill ( CIrradianceCache* pCache, bool* pDone, float* pPercent )
{
    ...
    //========================================================//
    DWORD dwDepth = 0;
    CIrradianceCacheOctree::OctreeNode* pNode = pCache->m_pOctree->FindUnsampledNode( pCache->m_pOctree->GetRootNode(), &dwDepth );
    ...
    for ( int sampleIndex = 0; sampleIndex < 8; sampleIndex++ )
    {
        ...
        if ( !( SampleIncidentRadiance(&pSample->vPosition, pSample->pRedCoefs, pSample->pGreenCoefs, pSample->pBlueCoefs ) ) )
        {
    ...
    *pDone = false;
    if ( NULL != pPercent )
    {
        ...
        else
        {
            float fS = powf ( 2.0f, (float) m_dwMaxOctreeSubdivision+1 );
            float fS3 = fS * fS * fS;
            *pPercent = 100.0f * ((float) (pCache->GetNodeCount() + pCache->m_dwNumSkippedSamples)  ) / ( ( fS3 - 1.0f ) / 7.0f );
        }
...
//===============================================================================================//
bool CIrradianceCacheGenerator::SampleIncidentRadiance ( D3DXVECTOR3* pPosition, float* pRed, float* pGreen, float* pBlue )
{
    ...
    for (DWORD faceIndex = 0; faceIndex < 6; faceIndex++)
    {
        ...
        hResult = m_pD3DDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(0, 0, 0, 0), 1.0f, 0 );
        if ( FAILED( hResult ) )
        {
...
//===================================================================//
bool CIrradianceCacheGenerator::SampleDepth ( D3DXVECTOR3* pPosition, float* pHMDepth, float* pMinDepth, float* pMaxDepth )
{
    ...
    for ( DWORD faceIndex = 0; faceIndex < 6; faceIndex++ )
    {
        ...
        for ( DWORD y = 0; y < rtDesc.Size; y++ )
        {
            ...
            {
                D3DXFloat16To32Array(pfScanLine,((D3DXFLOAT16*)lockedRect.pBits)+(lockedRect.Pitch/sizeof(D3DXFLOAT16))*y,rtDesc.Size*4);
            }
...
//=========================================================================================//
bool CIrradianceCache::SampleTrilinear ( D3DXVECTOR3* pPosition, CIrradianceCache::IrradianceSample* pSample, D3DXVECTOR3 pBox[8]  )
{
    ...
    {
        pSample->pRedCoefs[i] =  pSampleArray[0]->pRedCoefs[i] * ( 1.0f - xWeight ) * ( 1.0f - yWeight ) * ( 1.0f - zWeight );
        ...
        pSample->pGreenCoefs[i] =  pSampleArray[0]->pGreenCoefs[i] * ( 1.0f - xWeight ) * ( 1.0f - yWeight ) * ( 1.0f - zWeight );
        pSample->pGreenCoefs[i] += pSampleArray[1]->pGreenCoefs[i] * ( 1.0f - xWeight ) * ( 1.0f - yWeight ) * ( zWeight );
        pSample->pGreenCoefs[i] += pSampleArray[2]->pGreenCoefs[i] * ( 1.0f - xWeight ) * ( yWeight )        * ( 1.0f - zWeight );
        pSample->pGreenCoefs[i] += pSampleArray[3]->pGreenCoefs[i] * ( 1.0f - xWeight ) * ( yWeight )        * ( zWeight );
        pSample->pGreenCoefs[i] += pSampleArray[4]->pGreenCoefs[i] * ( xWeight )        * ( 1.0f - yWeight ) * ( 1.0f - zWeight );
        ...
        pSample->pBlueCoefs[i] =  pSampleArray[0]->pBlueCoefs[i] * ( 1.0f - xWeight ) * ( 1.0f - yWeight ) * ( 1.0f - zWeight );
        pSample->pBlueCoefs[i] += pSampleArray[1]->pBlueCoefs[i] * ( 1.0f - xWeight ) * ( 1.0f - yWeight ) * ( zWeight );
...
bool CIrradianceCache::SaveCache ( WCHAR* strFileName )
{
    ...
    //===========================//
    if ( 0 == WriteFile( pFile, IRRADIANCECACHE_FILE_VERSION_STRING, sizeof(IRRADIANCECACHE_FILE_VERSION_STRING), &dwWritten, NULL ) )
    {
        OUTPUT_ERROR_MESSAGE( L"Write failed!\n" );
        CloseHandle( pFile );
        return false;
    ...
    for ( DWORD index = 0; index < dwCacheSize; index++ )
    {
        ...
        if ( 0 == WriteFile( pFile, m_pCache[index]->pRedCoefs, sizeof(float)*IRRADIANCE_CACHE_MAX_SH_COEF, &dwWritten, NULL ) )
        {
            OUTPUT_ERROR_MESSAGE( L"Write failed!\n" );
            CloseHandle( pFile );
            return false;
        ...
        if ( 0 == WriteFile( pFile, m_pCache[index]->pGreenCoefs, sizeof(float)*IRRADIANCE_CACHE_MAX_SH_COEF, &dwWritten, NULL ) )
        {
            OUTPUT_ERROR_MESSAGE( L"Write failed!\n" );
            CloseHandle( pFile );
            return false;
        ...
        if ( 0 == WriteFile( pFile, m_pCache[index]->pBlueCoefs, sizeof(float)*IRRADIANCE_CACHE_MAX_SH_COEF, &dwWritten, NULL ) )
        {
            OUTPUT_ERROR_MESSAGE( L"Write failed!\n" );
            CloseHandle( pFile );
            return false;
...
bool CIrradianceCache::LoadCache ( WCHAR* strFileName )
{
    ...
    //=============================//
    if (0 != wcsncmp( strFileString, IRRADIANCECACHE_FILE_VERSION_STRING, sizeof(IRRADIANCECACHE_FILE_VERSION_STRING) ) )
    {
    ...
    for ( DWORD index = 0; index < dwCacheSize; index++ )
    {
        ...
        if ( 0 == ReadFile( pFile, pSample->pGreenCoefs, sizeof(float)*IRRADIANCE_CACHE_MAX_SH_COEF, &dwRead, NULL ) )
        {
            OUTPUT_ERROR_MESSAGE( L"Read failed!\n" );
            CloseHandle( pFile );
            return false;
        ...
        if ( 0 == ReadFile( pFile, pSample->pBlueCoefs, sizeof(float)*IRRADIANCE_CACHE_MAX_SH_COEF, &dwRead, NULL ) )
        {
            OUTPUT_ERROR_MESSAGE( L"Read failed!\n" );
            CloseHandle( pFile );
            return false;
...
//===========================================================//
CIrradianceCacheOctree::OctreeNode* CIrradianceCacheOctree::FindEnclosingNode ( D3DXVECTOR3* pPosition, CIrradianceCacheOctree::OctreeNode* pNode )
{
    ...
    //======================================//
    if ( ( ( pPosition->x >= pNode->vPosition[0].x ) && ( pPosition->y >= pNode->vPosition[0].y ) && ( pPosition->z >= pNode->vPosition[0].z ) ) &&
        ( ( pPosition->x <= pNode->vPosition[7].x ) && ( pPosition->y <= pNode->vPosition[7].y ) && ( pPosition->z <= pNode->vPosition[7].z ) ) )
    {
        if ( pNode->bHasChildren )
        {
...
//==============================================================================================//
DWORD CIrradianceCacheOctree::GetNodePositions ( CIrradianceCacheOctree::OctreeNode* pNode,  D3DXVECTOR3* pPositions ) const
{
    if ( NULL == pNode )
    {
        OUTPUT_ERROR_MESSAGE( L"Received NULL pointer!\n" );
...
//==================================================================================//
CIrradianceCacheOctree::OctreeNode* CIrradianceCacheOctree::FindUnsampledNode ( CIrradianceCacheOctree::OctreeNode* pNode,  DWORD* pDepth )
{
    if ( NULL == pNode )
    {
        OUTPUT_ERROR_MESSAGE( L"Received NULL pointer!\n" );
...
//=============================================//
bool CIrradianceCacheOctree::SetNodeBounds ( CIrradianceCacheOctree::OctreeNode* pNode, D3DXVECTOR3* pBoundBoxMin, D3DXVECTOR3* pBoundBoxMax )
{
