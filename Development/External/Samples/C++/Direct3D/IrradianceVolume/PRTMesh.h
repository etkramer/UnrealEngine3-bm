...
public:
    ...
    // Misc
    void    GetVertexUnderMouse( const D3DXMATRIX* pmProj, const D3DXMATRIX* pmView, const D3DXMATRIX* pmWorld, unsigned int *uVert );
    ...
    HRESULT CompressPRTBuffer( D3DXSHCOMPRESSQUALITYTYPE Quality, UINT NumClusters, UINT NumPCA, LPD3DXSHPRTSIMCB pCB=NULL, LPVOID lpUserContext=NULL );
    ...
    DWORD   GetPRTOrder() { return m_dwPRTOrder; }
    bool    IsPRTUncompressedBufferLoaded() { return (m_pPRTBuffer != NULL); }
    bool    IsPRTCompBufferLoaded() { return (m_pPRTCompBuffer != NULL); }
    ID3DXPRTCompBuffer* GetPRTCompBuffer() { return m_pPRTCompBuffer; }
    ID3DXPRTBuffer* GetPRTBuffer() { return m_pPRTBuffer; }
    bool    IsPRTShaderDataExtracted() { return (m_aPRTClusterBases != NULL); }
    bool    IsPRTEffectLoaded() { return (m_pPRTEffect != NULL); }  
    UINT    GetOrderFromNumCoeffs( UINT dwNumCoeffs );

    // LDPRT
    void    RenderWithLDPRT( IDirect3DDevice9* pd3dDevice, D3DXMATRIX* pmWorldViewProj, D3DXMATRIX* pmNormalXForm, bool bUniform, bool bRenderWithAlbedo );
    void    SetLDPRTData( ID3DXPRTBuffer* pLDPRTBuff, D3DXVECTOR3 *pShadingNormal );
    void    ComputeLDPRTConstants( float* pSHCoeffsRed, float* pSHCoeffsGreen, float* pSHCoeffsBlue, DWORD dwNumCoeffsPerChannel );
