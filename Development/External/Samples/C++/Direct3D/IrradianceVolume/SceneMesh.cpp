...
HRESULT CSceneMesh::OnCreateDevice ( LPDIRECT3DDEVICE9 pd3dDevice )
{
   ...
   if( m_ReloadState.bUseReloadState )
   {
      ...
      WCHAR str[MAX_PATH];
      StringCchCopy( str, MAX_PATH, m_ReloadState.strMeshFileName );
      StringCchCat( str, MAX_PATH, TEXT("bmp"));
...
HRESULT CSceneMesh::LoadMesh( IDirect3DDevice9* pd3dDevice, WCHAR* strMeshFileName, WCHAR* strTextureFileName )
{
   ...
   V_RETURN( D3DXLoadMeshFromX( m_ReloadState.strMeshFileName, D3DXMESH_MANAGED, pd3dDevice, NULL, &m_pMaterialBuffer, NULL, &m_dwNumMaterials, &m_pMesh) );
   m_pMaterials = (D3DXMATERIAL*)m_pMaterialBuffer->GetBufferPointer();

   DXUTFindDXSDKMediaFileCch( str, MAX_PATH, strTextureFileName );
   StringCchCopy( m_ReloadState.strTextureFileName, MAX_PATH, str );
   ...
   V_RETURN( D3DXComputeBoundingSphere( (D3DXVECTOR3*)pVertices, m_pMesh->GetNumVertices(), dwStride, &m_vObjectCenter, &m_fObjectRadius ) );
   V_RETURN( D3DXComputeBoundingBox( (D3DXVECTOR3*)pVertices, m_pMesh->GetNumVertices(), dwStride, &m_vBoundingBoxMin, &m_vBoundingBoxMax ) );
   ...
   V( m_pMesh->OptimizeInplace( D3DXMESHOPT_VERTEXCACHE | D3DXMESHOPT_ATTRSORT | D3DXMESHOPT_IGNOREVERTS, rgdwAdjacency, NULL, NULL, NULL) );
