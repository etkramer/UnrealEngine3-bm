...
void    GetSupportedTextureFormat( IDirect3D9 *pD3D, D3DCAPS9* pCaps, D3DFORMAT AdapterFormat, D3DFORMAT* pfmtTexture, D3DFORMAT* pfmtCubeMap, D3DFORMAT* pfmtIrrCubeMap );
...
void InitApp()
{
    ...
    StringCchPrintf( sz, 100, L"Max Octree Subdivision: %d", g_dwMaxOctreeSubdivision ); sz[99] = 0;
    g_PreprocessOptionsUI.AddStatic( IDC_MAX_SUBDIVISION_STATIC, sz, 0, iY += 24, 150, 22 );
    g_PreprocessOptionsUI.AddSlider( IDC_MAX_SUBDIVISION, 5, iY += 24, 100, 22, 0, MAX_SUBDIVISION, g_dwMaxOctreeSubdivision );
    g_PreprocessOptionsUI.AddCheckBox( IDC_ENABLE_ADAPTIVE_SUBDIVISION, L"Adaptive Subdivision", 0, iY += 24, 150, 22, g_bOctreeAdaptiveSubdivision);
    StringCchPrintf( sz, 100, L"Min Octree Subdivision: %d", g_dwMinOctreeSubdivision ); sz[99] = 0;
    g_PreprocessOptionsUI.AddStatic( IDC_MIN_SUBDIVISION_STATIC, sz, 0, iY += 24, 150, 22 );
    g_PreprocessOptionsUI.AddSlider( IDC_MIN_SUBDIVISION, 15, iY += 24, 100, 22, 0, g_dwMaxOctreeSubdivision, g_dwMinOctreeSubdivision );   
    ...
    StringCchPrintf( sz, 100, L"HM Depth Threshold: %0.3f", g_fOctreeAdaptiveSubdivisionHMDepthThreshold); sz[99] = 0;
    g_PreprocessOptionsUI.AddStatic( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD_STATIC, sz, 0, iY += 24, 150, 22 );
    g_PreprocessOptionsUI.AddSlider( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD, 15, iY += 24, 100, 22, 0, 1000, (int) (g_fOctreeAdaptiveSubdivisionHMDepthThreshold * 100.0f) );
    g_PreprocessOptionsUI.GetStatic( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD_STATIC )->SetEnabled( g_bOctreeAdaptiveSubdivision );
    g_PreprocessOptionsUI.GetSlider( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD )->SetEnabled( g_bOctreeAdaptiveSubdivision );
    ...
    g_RenderingUI2.AddCheckBox( IDC_RENDER_OCTREE_NODE, L"Voxel (9)", 35, iY += 24, 125, 22, g_bRenderOctreeSampleNode, '9' );  
    ...
    int iX = 10;
    iY = iStartY; 
    g_RenderingUI.AddCheckBox( IDC_TRANSFER_VISUAL, L"Visualize Transfer (T)", iX, iY += 20, 150, 22, g_bRenderSHVisual, 'T' );   
    ...
    StringCchPrintf( sz, 100, L"Number of PCA: %d", options.dwNumPCA );
    g_CompressionUI.GetStatic( IDC_NUM_PCA_TEXT )->SetText( sz );           
    StringCchPrintf( sz, 100, L"Number of Clusters: %d", options.dwNumClusters );
...
bool CALLBACK IsDeviceAcceptable( D3DCAPS9* pCaps, D3DFORMAT AdapterFormat, D3DFORMAT BackBufferFormat, bool bWindowed, void* pUserContext )
{
    ...
    if ( FAILED( pD3D->CheckDeviceFormat(pCaps->AdapterOrdinal, pCaps->DeviceType, AdapterFormat, 0, D3DRTYPE_CUBETEXTURE, D3DFMT_A32B32G32R32F ) ) )
    {
       return false;
    }
...
void GetSupportedTextureFormat( IDirect3D9 *pD3D, D3DCAPS9* pCaps, D3DFORMAT AdapterFormat, D3DFORMAT* pfmtTexture, D3DFORMAT* pfmtCubeMap, D3DFORMAT* pfmtIrrCubeMap ) 
{
    ...
    if (SUCCEEDED(pD3D->CheckDeviceFormat(pCaps->AdapterOrdinal, pCaps->DeviceType, AdapterFormat, 0, D3DRTYPE_CUBETEXTURE, D3DFMT_A32B32G32R32F))) 
    {
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( D3DXCreateRenderToEnvMap(pd3dDevice, g_dwRadianceCubeSize, 0, fmtIrrCubeMap, true, D3DFMT_D24S8, &g_pRenderToEnvMap ) );
    V_RETURN( D3DXCreateCubeTexture(pd3dDevice, g_dwRadianceCubeSize, 0, 0, fmtIrrCubeMap, D3DPOOL_MANAGED, &g_pCapturedRadiance ) );
    ...
    StringCchPrintf( sz, 256, L"Max VS Constants: %d", dwMaxVertexShaderConst );
...
HRESULT CALLBACK OnResetDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_PreprocessOptionsUI.SetLocation( (pBackBufferSurfaceDesc->Width-150)/2, (pBackBufferSurfaceDesc->Height-300)/2 );
    g_PreprocessOptionsUI.SetSize( 150, 200 );

    g_PreprocessRunningUI.SetLocation( (pBackBufferSurfaceDesc->Width-600)/2, (pBackBufferSurfaceDesc->Height-100)/2 );
   ...
   if( g_SettingsDlg.IsActive() )
   {
    ...
    switch( g_AppState )
    {
        ...
        case APP_STATE_PREPROCESS_SCENE_RUNNING:
         {
            WCHAR sz[256];
            StringCchPrintf( sz, 256, L"Step %d of %d: %0.1f%% done", 1, 1, g_fPercentScenePreprocessing );
            ...
            if( g_pIrradianceCache )
            {
                ...
                bResult = g_IrradianceCacheGenerator.ProgressiveCacheFill( g_pIrradianceCache, &bDone, &g_fPercentScenePreprocessing );
        ...
        case APP_STATE_SIMULATOR_RUNNING:
        {
            ...
            if( g_Simulator.GetPercentComplete() >= 0.0f )
                StringCchPrintf( sz, 256, L"Step %d of %d: %0.1f%% done", g_Simulator.GetCurrentPass(), g_Simulator.GetNumPasses(), g_Simulator.GetPercentComplete() );
            else
                StringCchPrintf( sz, 256, L"Step %d of %d (progress n/a)", g_Simulator.GetCurrentPass(), g_Simulator.GetNumPasses() );
...
*/
    ...
    if( g_bShowHelp && g_AppState == APP_STATE_RENDER_SCENE )
    {
        ...
        if( !(DXUTIsMouseButtonDown(MK_LBUTTON) || DXUTIsMouseButtonDown(MK_MBUTTON) || DXUTIsMouseButtonDown(MK_RBUTTON)) )
        {
            if( g_bChaseCamera )
            {
                txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 110, DXUTGetD3D9BackBufferSurfaceDesc()->Height - 40 );
                txtHelper.DrawTextLine( L"W/S/A/D/Q/E to move model and camera." );
            }
            else 
            {
                txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 170, DXUTGetD3D9BackBufferSurfaceDesc()->Height - 40 );
                txtHelper.DrawTextLine( L"W/S/A/D/Q/E to move model. Press C to enter camera chase mode" );
            }
            txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 130, DXUTGetD3D9BackBufferSurfaceDesc()->Height - 25 );
            txtHelper.DrawTextLine( L"Left click moves model, right click moves camera." );
        } else
        {
            txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 70, DXUTGetD3D9BackBufferSurfaceDesc()->Height - 40 );
            if( DXUTIsMouseButtonDown(MK_LBUTTON) )
            {
                txtHelper.DrawTextLine( L"Model Control Mode" );
                txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 170, DXUTGetD3D9BackBufferSurfaceDesc()->Height - 25 );
    ...
    else if( g_bShowHelp && g_AppState == APP_STATE_PREPROCESS_SCENE_OPTIONS )
    {
        ...
        txtHelper.SetForegroundColor( D3DXCOLOR( 1.0f, 1.0f, 1.0f, 1.0f ) );
        txtHelper.DrawTextLine( L"This preprocesses the scene to create a volume of radiance samples stored in an octree." );
        txtHelper.DrawTextLine( L"This allows a PRT object to use the local lighting environment as it moves through a scene." );
        txtHelper.DrawTextLine( L"The more subdivision done to the scene, the more accurate the octree will be, but it will" );
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
    if( bKeyDown )
    {
        switch( nChar )
        {
            ...
            case VK_F8:
            case '0': g_bRenderWireframe = !g_bRenderWireframe; g_RenderingUI2.GetCheckBox( IDC_WIREFRAME )->SetChecked(g_bRenderWireframe); break;
        }
    }
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_MIN_SUBDIVISION:
            {
                  ...
                  g_dwMinOctreeSubdivision = g_dwMinOctreeSubdivision <= g_dwMaxOctreeSubdivision ? g_dwMinOctreeSubdivision : g_dwMaxOctreeSubdivision;
         ...
         case IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD:
            {
                  WCHAR sz[100];
                  g_fOctreeAdaptiveSubdivisionHMDepthThreshold = (float) g_PreprocessOptionsUI.GetSlider( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD )->GetValue() / 100.0f;
                  StringCchPrintf( sz, 100, L"HM Depth Threshold: %0.3f", g_fOctreeAdaptiveSubdivisionHMDepthThreshold); sz[99] = 0;
         ...
         case IDC_ENABLE_ADAPTIVE_SUBDIVISION:
            {
               g_bOctreeAdaptiveSubdivision = g_PreprocessOptionsUI.GetCheckBox( IDC_ENABLE_ADAPTIVE_SUBDIVISION )->GetChecked();
               g_PreprocessOptionsUI.GetStatic( IDC_MIN_SUBDIVISION_STATIC )->SetEnabled( g_bOctreeAdaptiveSubdivision );
               g_PreprocessOptionsUI.GetSlider( IDC_MIN_SUBDIVISION )->SetEnabled( g_bOctreeAdaptiveSubdivision );
               g_PreprocessOptionsUI.GetStatic( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD_STATIC )->SetEnabled( g_bOctreeAdaptiveSubdivision );
               g_PreprocessOptionsUI.GetSlider( IDC_ADAPTIVE_SUBDIVISION_HMDEPTH_THRESHOLD )->SetEnabled( g_bOctreeAdaptiveSubdivision );
        ...
        case IDC_TECHNIQUE_LDPRT:   
        {
            ...
            switch( nControlID )
            {
                case IDC_TECHNIQUE_SHIRRAD: g_AppTechnique = APP_TECH_SHIRRADENVMAP; break;
                case IDC_TECHNIQUE_LDPRT:   g_AppTechnique = APP_TECH_LDPRT; break;
                case IDC_TECHNIQUE_PRT:     g_AppTechnique = APP_TECH_PRT; break;
            }
        ...
        case IDC_COMPRESSION:
            ...
            {
                MessageBox( DXUTGetHWND(), L"To change compression settings during rendering, please load an uncompressed buffer.  To make one use the simulator settings dialog to save an uncompressed buffer.", L"IrradianceVolume", MB_OK );
            }
            break;
        ...
        case IDC_NUM_PCA:
        {
            ...
            StringCchPrintf( sz, 256, L"Number of PCA: %d", dwNumPCA );
            g_CompressionUI.GetStatic( IDC_NUM_PCA_TEXT )->SetText( sz );           
        ...
        case IDC_NUM_CLUSTERS:
        {
            ...
            StringCchPrintf( sz, 256, L"Number of Clusters: %d", dwNumClusters );
            g_CompressionUI.GetStatic( IDC_NUM_CLUSTERS_TEXT )->SetText( sz );           
        ...
        case IDC_APPLY:
            {
                if (APP_STATE_PREPROCESS_SCENE_OPTIONS == g_AppState )
                {
                    ...
                    if (!(g_IrradianceCacheGenerator.SetSamplingInfo( g_dwMaxOctreeSubdivision, g_bOctreeAdaptiveSubdivision, g_dwMinOctreeSubdivision, g_fOctreeAdaptiveSubdivisionHMDepthThreshold )))
                    {
                    ...
                    if (!(g_IrradianceCacheGenerator.SetGPUResources( DXUTGetD3D9Device(), g_pRenderToEnvMap, g_pCapturedRadiance )))
                    {
                                ...
                                int nNumRays, int nNumBounces, bool bSubSurface )
{
    ...
    else
    {
        ...
        StringCchCopy( options.strInputMesh, MAX_PATH, strInputMesh );
        FindPRTMediaFile( options.strInitialDir, MAX_PATH, options.strInputMesh, true );
        StringCchCat( options.strInitialDir, MAX_PATH, L"" );
        FindPRTMediaFile( options.strInitialDir, MAX_PATH, options.strInputMesh, true );
        ...
        if( pLastSlash )
            StringCchCopy( options.strResultsFile, MAX_PATH, pLastSlash + 1 );
        else
            StringCchCopy( options.strResultsFile, MAX_PATH, strResultsFile );
        options.dwOrder = 6;
        ...
        StringCchCopy( options.strOutputMesh, MAX_PATH, L"shapes1_adaptive.x" );
...
void LoadScene( IDirect3DDevice9* pd3dDevice, WCHAR* strInputMesh, WCHAR* strResultsFile )
{
    ...
    StringCchCopy( szBaseFile, MAX_PATH, strResults );
    pLastDot = wcsrchr( szBaseFile, L'.' );
    if( pLastDot )
        *pLastDot = 0;
    StringCchCopy( szLDPRTFile, MAX_PATH, szBaseFile );
    StringCchCat( szLDPRTFile, MAX_PATH, L".ldprt" );
    StringCchCopy( szNormalsFile, MAX_PATH, szBaseFile );
    StringCchCat( szNormalsFile, MAX_PATH, L".sn" );
...
void UpdateConstText()
{
    ...
    StringCchPrintf( sz, 256, L"%d * (1 + (3*%d/4)) + 4 = %d", dwNumClusters, dwNumPCA, dwNumVConsts );
...
void RenderBoundingBoxLines( IDirect3DDevice9* pd3dDevice, bool bRenderOctree, bool bRenderOctreeSampleNode )
{
    ...
    if ( bRenderOctree )
    {
        ...
        pd3dDevice->DrawPrimitiveUP( D3DPT_LINELIST, g_dwOctreeLineListSize / 2, g_pOctreeLineList, sizeof(D3DXVECTOR3) );
    }
...
HRESULT FindPRTMediaFile( WCHAR* strDestPath, int cchDest, LPCWSTR strFilename, bool bCreatePath )
{
    ...
    StringCchCat( strPath, MAX_PATH, g_szAppFolder );
    ...
    StringCchCat( strPath, MAX_PATH, strFilename );
    if( bCreatePath || (0xFFFFFFFF != GetFileAttributes( strPath )) )
    {
        StringCchCopy( strDestPath, cchDest, strPath );
        return S_OK;
    }
