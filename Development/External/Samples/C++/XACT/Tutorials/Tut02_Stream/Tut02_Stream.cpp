...
INT WINAPI wWinMain( HINSTANCE hInst, HINSTANCE, LPWSTR, INT )
{
    ...
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), 0, MsgProc, 0L, 0L, hInst, NULL, 
...
HRESULT PrepareXACT()
{
    ...
    if( SUCCEEDED( hr ) )
    {
        ...
        hr = XACTCreateEngine( dwCreationFlags, &g_audioState.pEngine );
    }
    ...
    switch( msg )
    {
        ...
        case WM_TIMER:
        {
            ...
            if( g_audioState.pZeroLatencyRevCue )
            {
                ...
                switch( dwState )
                {
                    case XACT_CUESTATE_CREATED: StringCchCopy( szState, 512, L"Created, but nothing else" ); break;
                    case XACT_CUESTATE_PREPARING: StringCchCopy( szState, 512, L"Preparing to play" ); break;
                    case XACT_CUESTATE_PREPARED: StringCchCopy( szState, 512, L"Prepared, but not yet played" ); break;
                    case XACT_CUESTATE_PLAYING: StringCchCopy( szState, 512, L"Playing, but can be paused" ); break;
                    case XACT_CUESTATE_STOPPING: StringCchCopy( szState, 512, L"Stopping" ); break;
                    case XACT_CUESTATE_STOPPED: StringCchCopy( szState, 512, L"Stopped" ); break;
                    case XACT_CUESTATE_PAUSED: StringCchCopy( szState, 512, L"Paused" ); break;
                }
            }
            else
            {
