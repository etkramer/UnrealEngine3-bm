...
void ComputeMeshScalingBox( CDXUTXFileMesh &Mesh, D3DXMATRIX *pmScalingCenter, D3DXVECTOR3 vNewMin, D3DXVECTOR3 vNewMax );
...
void InitApp()
{
    ...
    g_Render.VideoMenuDlg.AddCheckBox( IDC_ANTI_ALIASING, L"Anti-Aliasing", (250-200)/2, iY += 26, 200, 22, false );
    g_Render.VideoMenuDlg.AddCheckBox( IDC_HIGH_MODEL_RES, L"High res models", (250-200)/2, iY += 26, 200, 22, true );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    *(LPDWORD)lr.pBits = D3DCOLOR_RGBA( 127, 127, 255, 0 );
    ...
    ComputeMeshScalingBox( g_Render.meshCell, &g_Render.mCellWorld, g_MinBound + D3DXVECTOR3(-0.2f,-0.2f,-0.2f), g_MaxBound + D3DXVECTOR3(0.2f,0.2f,0.2f) );
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_Render.MainMenuDlg.SetLocation( (pBackBufferSurfaceDesc->Width-250)/2, (pBackBufferSurfaceDesc->Height-300)/2 );
    ...
    g_Render.AudioMenuDlg.SetBackgroundColors( D3DCOLOR_ARGB(200, 98, 138, 206), 
                                         ...
                                         D3DCOLOR_ARGB(200, 54, 105, 192),
                                         D3DCOLOR_ARGB(200, 10,  73, 179) );
    g_Render.AudioMenuDlg.SetLocation( (pBackBufferSurfaceDesc->Width-250)/2, (pBackBufferSurfaceDesc->Height-300)/2 );
    ...
    g_Render.VideoMenuDlg.SetBackgroundColors( D3DCOLOR_ARGB(200, 98, 138, 206), 
                                         ...
                                         D3DCOLOR_ARGB(200, 54, 105, 192),
                                         D3DCOLOR_ARGB(200, 10,  73, 179) );
    g_Render.VideoMenuDlg.SetLocation( (pBackBufferSurfaceDesc->Width-250)/2, (pBackBufferSurfaceDesc->Height-300)/2 );
        ...
        if( g_GameState.DroidQ[A].nHitPoints > 0 )
        {
            ...
            switch( g_GameState.DroidQ[A].aiState ) 
            {
                case AI_TURNING:
                {
                    ...
                    D3DXQuaternionSlerp( &g_GameState.DroidQ[A].qCurrent, &g_GameState.DroidQ[A].qStart, &g_GameState.DroidQ[A].qTarget, g_GameState.DroidQ[A].fRotInterp );
...
void CheckForAmmoToDroidCollision( int A )
{
    for( int B=0; B<MAX_DROID; B++ )
    {
        ...
        if( DistSq < ((AMMO_SIZE+DROID_SIZE)/2.0f)*((AMMO_SIZE+DROID_SIZE)/2.0f) )
        {
            ...
            float fImpact = D3DXVec3Dot( &vAToB, &g_GameState.AmmoQ[A].vVelocity ) - D3DXVec3Dot( &vAToB, &g_GameState.DroidQ[B].vVelocity );
            if( fImpact > 0.0f )
            {
    ...
    for( int A=0; A<MAX_AMMO-1; A++ )
    {
        ...
        for( int B=A+1; B<MAX_AMMO; B++ )
        {
            ...
            if( DistSq < AMMO_SIZE * AMMO_SIZE )
            {
                ...
                // If so, skip collision.  This can happen when a lot of instances are
                // bunched up next to each other.
                float fImpact = D3DXVec3Dot( &vAToB, &g_GameState.AmmoQ[A].vVelocity ) - D3DXVec3Dot( &vAToB, &g_GameState.AmmoQ[B].vVelocity );
                if( fImpact > 0.0f )
                {
                    // Compute the normal and tangential compAnts of A's velocity.
                    D3DXVECTOR3 vVelocityAN = ( 1 - BOUNCE_LOST ) * D3DXVec3Dot( &vAToB, &g_GameState.AmmoQ[A].vVelocity ) * vAToB;
                    ...
                    D3DXVECTOR3 vVelocityBN = ( 1 - BOUNCE_LOST ) * D3DXVec3Dot( &vAToB, &g_GameState.AmmoQ[B].vVelocity ) * vAToB;
                    D3DXVECTOR3 vVelocityBT = ( 1 - BOUNCE_LOST ) * g_GameState.AmmoQ[B].vVelocity - vVelocityBN;

                    // Compute post-collision velocity
                    g_GameState.AmmoQ[A].vVelocity = vVelocityAT - vVelocityAN * ( 1 - BOUNCE_TRANSFER ) + vVelocityBN * BOUNCE_TRANSFER;
                    g_GameState.AmmoQ[B].vVelocity = vVelocityBT - vVelocityBN * ( 1 - BOUNCE_TRANSFER ) + vVelocityAN * BOUNCE_TRANSFER;
...
void CheckForAmmoToWallCollision( int A )
{
    ...
    if( GRAVITY * ( g_GameState.AmmoQ[A].vPosition.y + GROUND_Y - ( AMMO_SIZE * 0.5f ) ) + 0.5f * g_GameState.AmmoQ[A].vVelocity.y * g_GameState.AmmoQ[A].vVelocity.y < REST_THRESHOLD )
    {
        g_GameState.AmmoQ[A].vPosition.y = -GROUND_Y + ( AMMO_SIZE * 0.5f );
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( g_GameState.fAmmoColorLerp > 1.0f )
    {
        ...
        switch( rand() % 5 )
        {
            case 0: g_GameState.BlendToColor = D3DXCOLOR(0.6f,0,0,1); break;
            case 1: g_GameState.BlendToColor = D3DXCOLOR(0,0.6f,0,1); break;
            case 2: g_GameState.BlendToColor = D3DXCOLOR(0,0,0.6f,1); break;
            case 3: g_GameState.BlendToColor = D3DXCOLOR(0,0,0,1); break;
            case 4: g_GameState.BlendToColor = D3DXCOLOR(0.6f,0.6f,0,1); break;
        }
    }
    ...
    for( DWORD iPort=0; iPort<DXUT_MAX_CONTROLLERS; iPort++ )
    {
        if( g_GameState.gamePad[iPort].bConnected )
        {
            if( (g_GameState.gamePad[iPort].wButtons & (XINPUT_GAMEPAD_LEFT_SHOULDER|XINPUT_GAMEPAD_RIGHT_SHOULDER|XINPUT_GAMEPAD_A) ) ||
...
void CreateDroid()
{
    ...
    switch( rand() % 7 )
    {
        case 0: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C1, C1, C2, 1.0f ); break;
        case 1: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C1, C2, C1, 1.0f ); break;
        case 2: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C1, C2, C2, 1.0f ); break;
        case 3: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C2, C1, C1, 1.0f ); break;
        case 4: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C2, C1, C2, 1.0f ); break;
        case 5: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C2, C2, C1, 1.0f ); break;
        case 6: g_GameState.DroidQ[A].Specular = D3DXCOLOR( C2, C2, C2, 1.0f ); break;
    }
...
void RenderDroid( IDirect3DDevice9* pd3dDevice, int A, D3DXMATRIXA16& mView, D3DXMATRIXA16& mProj, bool bExplode )
{
    ...
    {
        if( bExplode )
            g_Render.meshDroidExplosion.Render( g_Render.pEffect, g_Render.hDiffuseTexture, NULL, 0, NULL, 0, NULL, true, false );
        else
    ...
    {
        if( bExplode )
            g_Render.meshDroidExplosionLow.Render( g_Render.pEffect, g_Render.hDiffuseTexture, NULL, 0, NULL, 0, NULL, true, false );
        else
            g_Render.meshDroidLow.Render( g_Render.pEffect, g_Render.hDiffuseTexture, NULL, 0, NULL, 0, NULL, true, false );
    }
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        g_Render.meshCell.Render( g_Render.pEffect, g_Render.hDiffuseTexture, g_Render.hDiffuse, 0, g_Render.hSpecular, 0, g_Render.hPower, true, false );
        ...
        {
            case GAME_RUNNING: RenderText(); break;
            case GAME_MAIN_MENU: V( g_Render.MainMenuDlg.OnRender( fElapsedTime ) ); break;
            case GAME_AUDIO_MENU: V( g_Render.AudioMenuDlg.OnRender( fElapsedTime ) ); break;    
            case GAME_VIDEO_MENU: V( g_Render.VideoMenuDlg.OnRender( fElapsedTime ) ); break;
        }
...
void RenderText()
{
    ...
    txtHelper.DrawFormattedTextLine( L"Pos: %0.2f, %0.2f, %0.2f", g_Camera.GetEyePt()->x, g_Camera.GetEyePt()->y, g_Camera.GetEyePt()->z );
    txtHelper.End();
}
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext )
{
    ...
    switch( g_GameState.gameMode )
    {
        case GAME_RUNNING: g_Camera.HandleMessages( hWnd, uMsg, wParam, lParam ); break;
        case GAME_MAIN_MENU: g_Render.MainMenuDlg.MsgProc( hWnd, uMsg, wParam, lParam ); break;
        case GAME_AUDIO_MENU: g_Render.AudioMenuDlg.MsgProc( hWnd, uMsg, wParam, lParam ); break;
        case GAME_VIDEO_MENU: g_Render.VideoMenuDlg.MsgProc( hWnd, uMsg, wParam, lParam ); break;
    }
...
void UpdateAspectRatioList( DXUTDeviceSettings* pDS )
{
    ...
    CD3D9EnumDeviceSettingsCombo* pDeviceSettingsCombo = DXUTGetD3D9Enumeration()->GetDeviceSettingsCombo( &pDS->d3d9 );
    ...
    UINT nAdapterMonitorWidth = g_Render.rcAdapterWork[pDS->d3d9.AdapterOrdinal].right - g_Render.rcAdapterWork[pDS->d3d9.AdapterOrdinal].left;
    UINT nAdapterMonitorHeight = g_Render.rcAdapterWork[pDS->d3d9.AdapterOrdinal].bottom - g_Render.rcAdapterWork[pDS->d3d9.AdapterOrdinal].top;
    ...
    for( int i=0; i< aspectRatioList.GetSize(); i++ )
    {
        ...
        if( fabsf(fAspect - 1.3333f) < 0.05f ) StringCchPrintf( sz, 256, L"4:3" );
        else if( fabsf(fAspect - 1.25f) < 0.05f ) StringCchPrintf( sz, 256, L"5:4" );
        else if( fabsf(fAspect - 1.77f) < 0.05f ) StringCchPrintf( sz, 256, L"16:9" );
        else if( fabsf(fAspect - 1.6f) < 0.05f ) StringCchPrintf( sz, 256, L"16:10" );
        else if( fabsf(fAspect - 1.5f) < 0.05f ) StringCchPrintf( sz, 256, L"3:2" );
        else if( fabsf(fAspect - 0.8f) < 0.05f ) StringCchPrintf( sz, 256, L"4:5" );
        else if( fabsf(fAspect - 1.66f) < 0.05f ) StringCchPrintf( sz, 256, L"5:3" );
        else if( fabsf(fAspect - 0.75f) < 0.05f ) StringCchPrintf( sz, 256, L"3:4" );
        else if( fabsf(fAspect - 0.5625f) < 0.05f ) StringCchPrintf( sz, 256, L"9:16" );
        else StringCchPrintf( sz, 256, L"%0.2f:1", fAspect );
    ...
    g_Render.VideoMenuDlg.GetComboBox( IDC_ASPECT )->SetSelectedByData( UintToPtr( *(DWORD*)&fCurrentlySelectedAspect ) );
...
void UpdateResolutionList( DXUTDeviceSettings* pDS )
{
    ...
    for( int i=0; i< pDMList->GetSize(); i++ )
    {
            ...
            (!bFullScreenChecked && dm.Width <= nAdapterMonitorWidth && dm.Height <= nAdapterMonitorHeight) )
        {
            ...
            {
                WCHAR sz[256];
                StringCchPrintf( sz, 256, L"%d x %d", dm.Width, dm.Height );
                if( !g_Render.VideoMenuDlg.GetComboBox( IDC_RESOLUTION )->ContainsItem( sz ) )
                    g_Render.VideoMenuDlg.GetComboBox( IDC_RESOLUTION )->AddItem( sz, UIntToPtr(MAKELONG(dm.Width, dm.Height)) );
            }
        }
    }
    ...
    if( bSetToDefault )
    {
        ...
        D3DDISPLAYMODE dmDesktop;
        DXUTGetDesktopResolution( pDS->d3d9.AdapterOrdinal, &dmDesktop.Width, &dmDesktop.Height );
        g_Render.VideoMenuDlg.GetComboBox( IDC_RESOLUTION )->SetSelectedByData( UintToPtr(MAKELONG(dmDesktop.Width, dmDesktop.Height)) );
    }
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_VIDEO: 
        {
            ...
            g_Render.VideoMenuDlg.GetCheckBox( IDC_ANTI_ALIASING )->SetChecked( DXUTGetDeviceSettings().d3d9.pp.MultiSampleType != D3DMULTISAMPLE_NONE );
            g_Render.VideoMenuDlg.GetComboBox( IDC_RESOLUTION )->SetSelectedByData( UintToPtr(MAKELONG(DXUTGetDeviceSettings().d3d9.pp.BackBufferWidth, DXUTGetDeviceSettings().d3d9.pp.BackBufferHeight)) );
        ...
        case IDC_MUSIC_SCALE: 
            g_audioState.fMusicVolume = (float) (g_Render.AudioMenuDlg.GetSlider( IDC_MUSIC_SCALE )->GetValue() / 100.0f ); 
            SetMusicVolume( g_audioState.fMusicVolume ); 
            break;
        case IDC_SOUNDFX_SCALE: 
            g_audioState.fSoundFXVolume = (float) (g_Render.AudioMenuDlg.GetSlider( IDC_SOUNDFX_SCALE )->GetValue() / 100.0f ); 
        ...
        case IDC_MAX_DROIDS:
        {
            ...
            StringCchPrintf( sz, 256, L"Max Droids (%d)", nMaxObjects );
...
void ToggleMenu()
{
    switch( g_GameState.gameMode )
    {
        case GAME_RUNNING: g_GameState.gameMode = GAME_MAIN_MENU; break;
        case GAME_MAIN_MENU: g_GameState.gameMode = GAME_RUNNING; break;
        case GAME_AUDIO_MENU: g_GameState.gameMode = GAME_MAIN_MENU; break;
        case GAME_VIDEO_MENU: g_GameState.gameMode = GAME_MAIN_MENU; break;
    }
