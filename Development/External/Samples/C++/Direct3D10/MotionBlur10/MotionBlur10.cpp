...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( staticlayout, 4, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pStaticVertexLayout ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( skinnedlayout, 6, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pSkinnedVertexLayout ) );
...
//--------------------------------------------------------------------------------------
void RenderLinkedMesh( ID3D10Device* pd3dDevice, CDXUTSDKMesh* pLinkedMesh, CDXUTSDKMesh* pAnimMesh, UINT iInfluence, double fTime, bool bMotionBlur )
{
    ...
    for( int s=0; s<MAX_TIME_STEPS; s++ )
    {
        ...
        D3DXMATRIX* pMat = pAnimMesh->GetMeshInfluenceMatrix( 0, iInfluence );
...
void RenderSkinnedMesh( ID3D10Device* pd3dDevice, CDXUTSDKMesh* pAnimMesh, double fTime, bool bMotionBlur )
{
    ...
    for( UINT m=0; m<pAnimMesh->GetNumMeshes(); m++ )
    {
        ...
        for( int s=0; s<MAX_TIME_STEPS; s++ )
        {
            ...
            {
                D3DXMATRIX* pMat = pAnimMesh->GetMeshInfluenceMatrix( m, i );
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    for( UINT iMesh=0; iMesh < g_NumLinkedMeshes; iMesh++ )
        RenderLinkedMesh( pd3dDevice, &g_pLinkedMeshes[iMesh], &g_AnimMesh, g_MeshLinkages[iMesh].iBone, fTime, false );
    ...
    if( g_bUseMotionBlur )
    {
        ...
        for( UINT iMesh=0; iMesh < g_NumLinkedMeshes; iMesh++ ) 
            RenderLinkedMesh( pd3dDevice, &g_pLinkedMeshes[iMesh], &g_AnimMesh, g_MeshLinkages[iMesh].iBone, fTime, true );
    }
