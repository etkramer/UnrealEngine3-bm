...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_RENDERCAR:        g_bRenderCar = !g_bRenderCar; break;
        case IDC_RENDERINSTANCED:  g_bRenderWithInstancing = !g_bRenderWithInstancing; break;
    }
}
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layout, 3, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pVertexLayoutCM ) );

    g_pRenderCubeMapInstTech->GetPassByIndex( 0 )->GetDesc( &PassDesc );
    V_RETURN( pd3dDevice->CreateInputLayout( layout, 3, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pVertexLayoutCMInst ) );

    g_pRenderEnvMappedSceneTech->GetPassByIndex( 0 )->GetDesc( &PassDesc );
    V_RETURN( pd3dDevice->CreateInputLayout( layout, 3, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pVertexLayoutEnv ) );
...
HRESULT RenderScene( ID3D10Device* pd3dDevice, D3DXMATRIX &mView, D3DXMATRIX &mProj, bool bRenderCubeMap, ID3D10EffectTechnique *pTechnique )
{
    ...
    if( !bRenderCubeMap )
    {
        ...
        if( g_bRenderCar )
        {
            ...
            g_MeshCarInnards.Render( pd3dDevice, g_pRenderEnvMappedSceneNoTexTech, NULL, NULL, NULL, g_pvDiffuse, g_pvSpecular );

            // Finally, render the glass
            g_MeshCarGlass.Render( pd3dDevice, g_pRenderEnvMappedGlassTech, NULL, NULL, NULL, g_pvDiffuse, g_pvSpecular );
        }
        else
        {
