...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( DXUTIsAppRenderingWithD3D9() )
    {
        ...
        g_pConstantTable->SetFloat( pd3dDevice, "fTime", (float) fTime );
    }
    else 
    {
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( D3DX10CompileFromFile(str, NULL, NULL, "Ripple", "vs_4_0", dwShaderFlags, NULL, NULL, &pVSBuf, NULL, NULL ) );
    V_RETURN( pd3dDevice->CreateVertexShader( (DWORD*) pVSBuf->GetBufferPointer(), pVSBuf->GetBufferSize(), &g_pVS10 ) );

    // Compile the PS from the file
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"HLSLwithoutFX.psh" ) );
    ID3D10Blob* pPSBuf = NULL;
    V_RETURN( D3DX10CompileFromFile(str, NULL, NULL, "main", "ps_4_0", dwShaderFlags, NULL, NULL, &pPSBuf, NULL, NULL ) );
    V_RETURN( pd3dDevice->CreatePixelShader( (DWORD*) pPSBuf->GetBufferPointer(), pPSBuf->GetBufferSize(), &g_pPS10 ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layout, 1, pVSBuf->GetBufferPointer(), pVSBuf->GetBufferSize(), &g_pVertexLayout ) );
