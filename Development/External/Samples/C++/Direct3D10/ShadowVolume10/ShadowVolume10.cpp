...
void InitApp()
{
    ...
    g_SampleUI.AddButton( IDC_CHANGEMESH, L"Change mesh", 20, iY += 26, 120, 40 );
...
void RenderText()
{
    ...
    UINT nBackBufferWidth = ( DXUTIsAppRenderingWithD3D9() ) ? DXUTGetD3D9BackBufferSurfaceDesc()->Width : DXUTGetDXGIBackBufferSurfaceDesc()->Width;
    ...
    switch( nControlID )
    {
        ...
        case IDC_MESHFILENAME:
        {
            ...
            switch( DeviceSettings.ver )
            {
                ...
                {
                    g_Mesh10.Destroy();
                    if( FAILED( g_Mesh10.Create( DXUTGetD3D10Device(), 
        ...
        case IDC_CHANGEMESH:
        {
            ...
            if( DXUTIsAppRenderingWithD3D9() )
            {
                ...
                width = pd3dsdBackBuffer->Width;
            }
            else
            {
                const DXGI_SURFACE_DESC* pDXGIBackBuffer = DXUTGetDXGIBackBufferSurfaceDesc();
                width = pDXGIBackBuffer->Width;
            ...
            } else
            {
                ...
                g_SampleUI.AddEditBox( IDC_MESHFILENAME, L"", width - 550, 158, 415, 34, false, &pEditBox );
                if( pEditBox)
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( quadlayout, sizeof(quadlayout)/sizeof(quadlayout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pQuadLayout ) );

    // Load the meshes
    V_RETURN( g_Background10[0].Create( pd3dDevice, L"misc\\cell.sdkmesh", false, true ) );
    V_RETURN( g_Background10[1].Create( pd3dDevice, L"misc\\seafloor.sdkmesh", false, true ) );
    V_RETURN( g_Mesh10.Create( pd3dDevice, L"dwarf\\dwarf.sdkmesh", false, true ) );
...
{
    D3DXVECTOR4 vLight( g_aLights[iLight].m_Position.x, g_aLights[iLight].m_Position.y, g_aLights[iLight].m_Position.z, 1.0f );
