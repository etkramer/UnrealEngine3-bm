...
void    RenderToTexture( ID3D10Device* pd3dDevice, ID3D10RenderTargetView* pRTV, ID3D10ShaderResourceView* pSRV, bool bClear, ID3D10EffectTechnique* pTechnique );
...
int _cdecl main( int NumArgs, char** ppCmdLine )
{
    ...
    if( !::GetSystemDirectory( wszPath, MAX_PATH+1 ) )
        return false;
    StringCchCat( wszPath, MAX_PATH, L"\\d3d10.dll" );
...
HRESULT InitResources( ID3D10Device* pd3dDevice )
{
    ...
    if( FAILED(hr) )
        return hr;
    hr = D3DX10CreateEffectFromFile( str, NULL, NULL, "fx_4_0", dwShaderFlags, 0, pd3dDevice, NULL, NULL, &g_pEffect10, NULL, NULL );
    if(FAILED(hr))
        return hr;
...
HRESULT SaveSpectogramToFile( ID3D10Device* pd3dDevice, LPCTSTR szFileName, ID3D10Texture2D* pTex )
{
    ...
    // Open the file
    HANDLE hFile = CreateFile( szFileName, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    ...
    bfh.bfSize = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + (iWidth*3+iPadAmt)*iHeight*sizeof(unsigned char);
...
HRESULT FindMediaFileCch( WCHAR* strDestPath, int cchDest, LPCWSTR strFilename )
{
    ...
    while( strFilePart != NULL && *strFilePart != '\0' )
    {
        ...
        StringCchPrintf( strFullFileName, MAX_PATH, L"..\\%s\\%s", strExeName, strLeafName ); 
        if( GetFileAttributes( strFullFileName ) != 0xFFFFFFFF )
        {
            StringCchCopy( strDestPath, cchDest, strFullFileName ); 
            bFound = true;
