...
HRESULT OnD3D10CreateDevice( DEVICE_OBJECT* pDeviceObj )
{
    ...
    #endif
    V_RETURN( D3DX10CreateEffectFromFile( str, NULL, NULL, "fx_4_0", dwShaderFlags, 0, pDeviceObj->pDevice, NULL, NULL, &pDeviceObj->pEffect, NULL, NULL ) );
    ...
    V_RETURN( pDeviceObj->pDevice->CreateInputLayout( layout, sizeof(layout)/sizeof(layout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &pDeviceObj->pInputLayout ) );
...
HRESULT InitD3D10()
{
    ...
    HMODULE hD3DX10 = LoadLibrary( L"d3dx10_33.dll" );
...
HRESULT CreateViewsForWindowObject( WINDOW_OBJECT* pWindowObj )
{
    ...
    hr = pWindowObj->pDeviceObj->pDevice->CreateRenderTargetView( pBackBuffer, &RTVDesc, &pWindowObj->pRenderTargetView );
    pBackBuffer->Release();
    pBackBuffer = NULL;
    if( FAILED(hr) )
        return hr;
    ...
    hr = pWindowObj->pDeviceObj->pDevice->CreateDepthStencilView( pDepthStencil, &descDSV, &pWindowObj->pDepthStencilView );
...
HRESULT SetupMultiMon()
{
    ...
    if( FAILED(hr) )
    {
        ...
        if( FAILED(hr) )
        {
            MessageBox( NULL, L"Could not create a compatible Direct3D10 device.  This application will now exit.", L"Error", MB_OK );
            return hr;
        }
    }
...
void RenderText( WINDOW_OBJECT* pWindowObj )
{
    ...
    StringCchPrintf( strTxt, MAX_PATH, L"Adapter: %s", pWindowObj->AdapterDesc.Description );
    pWindowObj->pDeviceObj->pTxtHelper->DrawTextLine( strTxt );
    StringCchPrintf( strTxt, MAX_PATH, L"Output: %s", pWindowObj->OutputDesc.DeviceName );
    pWindowObj->pDeviceObj->pTxtHelper->DrawTextLine( strTxt );
    StringCchPrintf( strTxt, MAX_PATH, L"Device: %X", pWindowObj->pDeviceObj->pDevice );
    pWindowObj->pDeviceObj->pTxtHelper->DrawTextLine( strTxt );
