...
void InitApp()
{
	...
	iY += 24;
	WCHAR str[MAX_PATH];
    StringCchPrintf( str, MAX_PATH, L"Scaling: %.4f", g_fScale );
	...
	StringCchPrintf( str, MAX_PATH, L"Effect Limit: %.4f", g_fEffectLimit );
	...
	StringCchPrintf( str, MAX_PATH, L"Effect Life: %.4f", g_fEffectLife );
	...
	StringCchPrintf( str, MAX_PATH, L"Normalize: %.4f", g_fNormalization );
	...
	StringCchPrintf( str, MAX_PATH, L"Sample Frequency: %.4f", g_fSampleFrequency );
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( instlayout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pInstVertexLayout ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( streamlayout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pStreamVertexLayout ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( meshlayout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pMeshLayout ) );
    ...
    hr = pd3dDevice->CreateInputLayout( quadlayout, sizeof(quadlayout)/sizeof(quadlayout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pScreenQuadLayout );
    if( FAILED(hr) )
        return hr;
...
HRESULT InitSpectrogramResources( ID3D10Device* pd3dDevice )
{
	...
	StringCchPrintfA( strBuckets, MAX_PATH, "%d", g_uiTexX );
    ...
    #endif
    hr = D3DX10CreateEffectFromFile( str, Defines, NULL, "fx_4_0", dwShaderFlags, 0, pd3dDevice, NULL, NULL, &g_pSpectrogramEffect10, NULL, NULL );
    if(FAILED(hr))
        return hr;
    ...
    hr = pd3dDevice->CreateInputLayout( quadlayout, sizeof(quadlayout)/sizeof(quadlayout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pQuadVertexLayout );
    if( FAILED(hr) )
        return hr;
...
HRESULT LoadEffect( ID3D10Device* pd3dDevice )
{
	...
	DWORD dwShaderFlags = D3D10_SHADER_ENABLE_STRICTNESS;
	V_RETURN( D3DX10CreateEffectFromFile( g_strEffect, NULL, NULL, "fx_4_0", dwShaderFlags, 0, pd3dDevice, NULL, NULL, &g_pEffect10, NULL, NULL ) );
...
HRESULT LoadParticleEffect( ID3D10Device* pd3dDevice )
{
    ...
    V_RETURN( D3DX10CreateEffectFromFile( str, NULL, NULL, "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, pd3dDevice, NULL, NULL, &g_pParticleEffect, NULL, NULL ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layout, sizeof(layout)/sizeof(layout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pParticleVertexLayout ) );
