...
BOOL WINAPI PIXBeginExperiment( PIXCOUNTERID id, const WCHAR* pstrApplication )
{
    switch( id )
    {
    case CTR_EFFECTSCALING:		return SharedMemoryInit( pstrApplication );
    case CTR_EFFECTLIMIT:		return SharedMemoryInit( pstrApplication );
    case CTR_EFFECTLIFE:        return SharedMemoryInit( pstrApplication );
    case CTR_NORMALIZATION:		return SharedMemoryInit( pstrApplication );
	case CTR_UPDATEFREQUENCY:		return SharedMemoryInit( pstrApplication );
	case CTR_STREAMOUT:		return SharedMemoryInit( pstrApplication );
	case CTR_SCREENEFFECTS:		return SharedMemoryInit( pstrApplication );
	case CTR_FIREWORKS:		return SharedMemoryInit( pstrApplication );
...
BOOL WINAPI PIXEndFrame( PIXCOUNTERID id, UINT iFrame, DWORD* pdwReturnBytes, BYTE** ppReturnData )
{
    switch( id )
    {
    case CTR_EFFECTSCALING:		return GetScaling( pdwReturnBytes, ppReturnData );
    case CTR_EFFECTLIMIT:		return GetEffectLimit( pdwReturnBytes, ppReturnData );
    case CTR_EFFECTLIFE:        return GetEffectLife( pdwReturnBytes, ppReturnData );
    case CTR_NORMALIZATION:		return GetNormalization( pdwReturnBytes, ppReturnData );
	case CTR_UPDATEFREQUENCY:   return GetUpdateFrequency( pdwReturnBytes, ppReturnData );
	case CTR_STREAMOUT:			return GetStreamOut( pdwReturnBytes, ppReturnData );
	case CTR_SCREENEFFECTS:		return GetScreenEffects( pdwReturnBytes, ppReturnData );
	case CTR_FIREWORKS:			return GetFireworks( pdwReturnBytes, ppReturnData );
    default: return FALSE;
    }
}
...
BOOL WINAPI PIXEndExperiment( PIXCOUNTERID id )
{
    switch( id )
    {
	case CTR_EFFECTSCALING:		return SharedMemoryUninit( );
    case CTR_EFFECTLIMIT:		return SharedMemoryUninit( );
    case CTR_EFFECTLIFE:        return SharedMemoryUninit( );
    case CTR_NORMALIZATION:		return SharedMemoryUninit( );
	case CTR_UPDATEFREQUENCY:	return SharedMemoryUninit( );
	case CTR_STREAMOUT:			return SharedMemoryUninit( );
	case CTR_SCREENEFFECTS:		return SharedMemoryUninit( );
	case CTR_FIREWORKS:			return SharedMemoryUninit( );
    default: return FALSE;
    }
}
