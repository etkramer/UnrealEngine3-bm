...
HRESULT CALLBACK OnD3D9CreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    for( UINT i=0; i<NUM_EFFECTS; i++ )
    {
        ...
        if( i == 2 )
            StringCchPrintf( strEffect, MAX_PATH, L"EffectPools%d.fxo9", i+1 );
        else
        ...
        g_EffectData[i].hMaterialDiffuseColor = g_EffectData[i].pEffect->GetParameterByName( NULL, "g_MaterialDiffuseColor" );
    }
