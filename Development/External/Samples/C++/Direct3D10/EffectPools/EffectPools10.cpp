...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    for( UINT i=0; i<NUM_EFFECTS; i++ )
    {
        ...
        if( i == 2 )
            StringCchPrintf( strEffect, MAX_PATH, L"EffectPools%d.fxo10", i+1 );
        else
            StringCchPrintf( strEffect, MAX_PATH, L"EffectPools%d.fx", i+1 );
        ...
        g_EffectData[i].pRenderScene = g_EffectData[i].pEffect->GetTechniqueByName( "RenderScene" );
        g_EffectData[i].pmWorld = g_EffectData[i].pEffect->GetVariableByName( "g_mWorld" )->AsMatrix();
        g_EffectData[i].pMaterialDiffuseColor = g_EffectData[i].pEffect->GetVariableByName( "g_MaterialDiffuseColor" )->AsVector();
    }
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( vertlayout, iNumElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pVertexLayout ) );
