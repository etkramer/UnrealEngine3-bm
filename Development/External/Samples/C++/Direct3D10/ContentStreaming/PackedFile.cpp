                                    ...
                                    UINT SqrtNumTiles, UINT SidesPerTile, float fWorldScale, float fHeightScale )
{
    ...
    if( FAILED( DXUTFindDXSDKMediaFileCch( strDiffuseTexture.str, MAX_PATH, L"contentstreaming\\2kPanels_Diff.dds" ) ) )
        return false;
    ...
    if( FAILED( DXUTFindDXSDKMediaFileCch( strTerrainHeight.str, MAX_PATH, L"contentstreaming\\terrain1.bmp" ) ) )
        return false;
    CTerrain Terrain;
    HRESULT hr = Terrain.LoadTerrain( strTerrainHeight.str, SqrtNumTiles, SidesPerTile, fWorldScale, fHeightScale, true );
    if( FAILED(hr) )
        return false;
    ...
    for( UINT y=0; y<SqrtNumTiles; y++ )
    {
        for( UINT x=0; x<SqrtNumTiles; x++ )
        {
            ...
            StringCchPrintf( pFileIndex->szFileName, MAX_PATH, L"terrainVB%d_%d", x,y );
            ...
            STRING strTemp;
            StringCchCopy( strTemp.str, MAX_PATH, L"VB" );
            FullFilePath.Add( strTemp );

            // TerrainIB
            pFileIndex = new FILE_INDEX;
            StringCchPrintf( pFileIndex->szFileName, MAX_PATH, L"terrainIB%d_%d", x,y );
            ...
            StringCchCopy( strTemp.str, MAX_PATH, L"IB" );
            FullFilePath.Add( strTemp );

            // TerrainDiffuse
            pFileIndex = new FILE_INDEX;
            StringCchPrintf( pFileIndex->szFileName, MAX_PATH, L"terrainDiff%d_%d", x,y );
            ...
            // TerrainDiffuse
            pFileIndex = new FILE_INDEX;
            StringCchPrintf( pFileIndex->szFileName, MAX_PATH, L"terrainNorm%d_%d", x,y );
    ...
    UINT64 IndexSize = sizeof( PACKED_FILE_HEADER ) + sizeof( CHUNK_HEADER )*TempHeaderList.GetSize() + sizeof( FILE_INDEX )*TempFileIndices.GetSize();
    ...
    // Open the file
    hFile = CreateFile( szFileName, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    if( INVALID_HANDLE_VALUE == hFile )
        return bRet;
    ...
    for( int c=0; c<TempHeaderList.GetSize(); c++ )
    {
        for( int i=0; i<TempFileIndices.GetSize(); i++ )
        {
            ...
            if( pIndex->ChunkIndex == c )
            {
                ...
                {
                    HANDLE hIndexFile = CreateFile( FullFilePath.GetAt(i).str, FILE_READ_DATA, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
...
bool CPackedFile::LoadPackedFile( WCHAR* szFileName, bool b64Bit, CGrowableArray<LEVEL_ITEM*>* pLevelItemArray  )
{
    ...
    // Open the file
    m_hFile = CreateFile( szFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    ...
    for( UINT i=0; i<m_FileHeader.NumFiles; i+=4 )
    {
        ...
        StringCchCopy( pLevelItem->szVBName, MAX_PATH, m_pFileIndices[i].szFileName );
        StringCchCopy( pLevelItem->szIBName, MAX_PATH, m_pFileIndices[i+1].szFileName );
        StringCchCopy( pLevelItem->szDiffuseName, MAX_PATH, m_pFileIndices[i+2].szFileName );
        StringCchCopy( pLevelItem->szNormalName, MAX_PATH, m_pFileIndices[i+3].szFileName );
