...
extern void	CALLBACK CreateTextureFromFile9_Serial( IDirect3DDevice9* pDev, WCHAR* szFileName, IDirect3DTexture9** ppTexture, void* pContext );
extern void	CALLBACK CreateVertexBuffer9_Serial( IDirect3DDevice9* pDev, IDirect3DVertexBuffer9** ppBuffer, UINT iSizeBytes, DWORD Usage, DWORD FVF, D3DPOOL Pool, void* pData, void* pContext );
extern void	CALLBACK CreateIndexBuffer9_Serial( IDirect3DDevice9* pDev, IDirect3DIndexBuffer9** ppBuffer, UINT iSizeBytes, DWORD Usage, D3DFORMAT ibFormat, D3DPOOL Pool, void* pData, void* pContext );
extern void	CALLBACK CreateTextureFromFile9_Async( IDirect3DDevice9* pDev, WCHAR* szFileName, IDirect3DTexture9** ppTexture, void* pContext );
extern void	CALLBACK CreateVertexBuffer9_Async( IDirect3DDevice9* pDev, IDirect3DVertexBuffer9** ppBuffer, UINT iSizeBytes, DWORD Usage, DWORD FVF, D3DPOOL Pool, void* pData, void* pContext );
extern void	CALLBACK CreateIndexBuffer9_Async( IDirect3DDevice9* pDev, IDirect3DIndexBuffer9** ppBuffer, UINT iSizeBytes, DWORD Usage, D3DFORMAT ibFormat, D3DPOOL Pool, void* pData, void* pContext );

void	CALLBACK CreateTextureFromFile10_Serial( ID3D10Device* pDev, WCHAR* szFileName, ID3D10ShaderResourceView** ppRV, void* pContext );
void	CALLBACK CreateVertexBuffer10_Serial( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext );
void	CALLBACK CreateIndexBuffer10_Serial( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext );
void	CALLBACK CreateTextureFromFile10_Async( ID3D10Device* pDev, WCHAR* szFileName, ID3D10ShaderResourceView** ppRV, void* pContext );
void	CALLBACK CreateVertexBuffer10_Async( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext );
void	CALLBACK CreateIndexBuffer10_Async( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext );
...
void InitApp()
{
    ...
    // SampleUI
    StringCchPrintf( str, MAX_PATH, L"View Height: %0.2f", g_fViewHeight );
    ...
    StringCchPrintf( str, MAX_PATH, L"Visible Radius: %0.2f", g_fVisibleRadius );
    ...
    StringCchPrintf( str, MAX_PATH, L"Upload to VRam every\n%d frames", g_UploadToVRamEveryNthFrame );
    ...
    g_StartUpUI.AddRadioButton( IDC_ONDEMANDMULTITHREAD,  IDC_LOAD_TYPE_GROUP, L"On Demand Multi-Threaded",				 iX1, iY+=42, 250, 22 );
    g_StartUpUI.AddRadioButton( IDC_ONDEMANDSINGLETHREAD, IDC_LOAD_TYPE_GROUP, L"On Demand Single-Threaded",			 iX1, iY+=22, 250, 22 );
    ...
    if( pDev9 )
    {
        if( LOAD_TYPE_SINGLETHREAD == g_LoadType )
        {
            ...
            if( !g_PackFile.GetPackedFile( pItem->szVBName, &pData, &DataBytes ) )
                return;
            CreateVertexBuffer9_Serial( pDev9, &pItem->VB.pVB9, DataBytes, D3DUSAGE_WRITEONLY, 0, D3DPOOL_MANAGED, pData, NULL );
            if( !g_PackFile.GetPackedFile( pItem->szIBName, &pData, &DataBytes ) )
                return;
            CreateIndexBuffer9_Serial( pDev9, &pItem->IB.pIB9, DataBytes, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, pData, NULL );
        ...
        else if( LOAD_TYPE_MULTITHREAD == g_LoadType )
        {
            ...
            if( !g_PackFile.GetPackedFile( pItem->szVBName, &pData, &DataBytes ) )
                return;
            CreateVertexBuffer9_Async( pDev9, &pItem->VB.pVB9, DataBytes, D3DUSAGE_WRITEONLY, 0, D3DPOOL_MANAGED, pData, (void*)g_pAsyncLoader );
            if( !g_PackFile.GetPackedFile( pItem->szIBName, &pData, &DataBytes ) )
                return;
            CreateIndexBuffer9_Async( pDev9, &pItem->IB.pIB9, DataBytes, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, pData, (void*)g_pAsyncLoader );
            CreateTextureFromFile9_Async( pDev9, pItem->szDiffuseName, &pItem->Diffuse.pTexture9, (void*)g_pAsyncLoader );
            CreateTextureFromFile9_Async( pDev9, pItem->szNormalName, &pItem->Normal.pTexture9, (void*)g_pAsyncLoader );
        }
    }
    else if( pDev10 )
    {
        ...
        else if( LOAD_TYPE_MULTITHREAD == g_LoadType )
        {
            ...
            CreateTextureFromFile10_Async( pDev10, pItem->szDiffuseName, &pItem->Diffuse.pRV10, (void*)g_pAsyncLoader );
            CreateTextureFromFile10_Async( pDev10, pItem->szNormalName, &pItem->Normal.pRV10, (void*)g_pAsyncLoader );
        }
    }
}
...
void GetPackedFilePath( WCHAR* strPath, UINT cchPath )
{
    ...
    StringCchCat( strPath, cchPath, strFolder );
}
...
void LoadStartupResources( IDirect3DDevice9* pDev9, ID3D10Device* pDev10, double fTime )
{
    ...
    if( 0xFFFFFFFF == GetFileAttributes( strPath ) )
    {
        ...
        {
            MessageBox( NULL, L"There was an error creating the pack file.  ContentStreaming will now exit.", L"Error", MB_OK );
            PostQuitMessage(0);
            return;
        }
    }
    ...
    StringCchCat( strPath, MAX_PATH, g_strFile );
    ...
    {
        HANDLE hFile = CreateFile( strPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    ...
    {
        if( IDNO == MessageBox( NULL, L"This is the first time ContentStreaming has been run.  The sample will need to create a 3.3 gigabyte pack file in order to demonstrate loading assets from a packed file format.  Do you wish to continue?", L"Warning", MB_YESNO ) )
        {
            PostQuitMessage(0);
            return;
        }

        if( !g_PackFile.CreatePackedFile( pDev10, pDev9, strPath, SqrtNumTiles, SidesPerTile, fWorldScale, fHeightScale ) )
        {
            MessageBox( NULL, L"There was an error creating the pack file.  ContentStreaming will now exit.", L"Error", MB_OK );
            PostQuitMessage(0);
            return;
    ...
    {
        MessageBox( NULL, L"There was an error loading the pack file.  ContentStreaming will now exit.", L"Error", MB_OK );
        PostQuitMessage(0);
        return;
    }
...
void RenderText()
{
    ...
    StringCchPrintf( str, MAX_PATH, L"InitialLevel Loaded in %.02f seconds", g_fInitialLoadTime );
    g_pTxtHelper->DrawTextLine( str );
    StringCchPrintf( str, MAX_PATH, L"Models in Use: %d", g_NumModelsInUse );
    g_pTxtHelper->DrawTextLine( str );
    ...
    if( g_pResourceReuseCache )
    {
        ...
        StringCchPrintf( str, MAX_PATH, L"Estimated video memory used: %d (mb) of %d (mb)", (int)(EstimatedManagedMemory/(1024*1024)), (int)(g_AvailableVideoMem/(1024*1024)) );
        g_pTxtHelper->DrawTextLine( str );

        StringCchPrintf( str, MAX_PATH, L"TextureCache: Total textures: %d", NumTextures );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"TextureCache: Used textures: %d", NumUsed );
        g_pTxtHelper->DrawTextLine( str );

        // LOD list
        int TextureSize = (int)powf( 2.0f,11.0f-g_SkipMips );
        StringCchPrintf( str, MAX_PATH, L"Texture LOD: %d x %d", TextureSize, TextureSize );
        g_pTxtHelper->DrawTextLine( str );
        ...
        StringCchPrintf( str, MAX_PATH, L"BufferCache: Total buffers: %d", NumVBs + NumIBs );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"    VBs: %d", NumVBs );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"    IBs: %d", NumIBs );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"BufferCache: Used buffers: %d", NumUsedVBs + NumUsedIBs );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"    VBs: %d", NumUsedVBs );
        g_pTxtHelper->DrawTextLine( str );
        StringCchPrintf( str, MAX_PATH, L"    IBs: %d", NumUsedIBs );
        g_pTxtHelper->DrawTextLine( str );
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( D3DX10CreateEffectFromFile( str, Defines, NULL, "fx_4_0", dwShaderFlags, 0, pd3dDevice, NULL, NULL, &g_pEffect10, NULL, NULL ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layoutObject, ARRAYSIZE(layoutObject), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pLayoutObject ) );
...
HRESULT CALLBACK OnD3D10ResizedSwapChain( ID3D10Device* pd3dDevice, IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_StartUpUI.SetLocation( (pBackBufferSurfaceDesc->Width-300)/2-40, (pBackBufferSurfaceDesc->Height-200)/2 );
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_VIEWHEIGHT:
        {
            ...
            StringCchPrintf( str, MAX_PATH, L"View Height: %.02f", g_fViewHeight );
        ...
        case IDC_VISIBLERADIUS:
        {
            ...
            StringCchPrintf( str, MAX_PATH, L"Visible Radius: %.02f", g_fVisibleRadius );
            g_SampleUI.GetStatic( IDC_VISIBLERADIUS_STATIC )->SetText( str );
            break;
        }
        ...
        case IDC_DELETE_PACK_FILE:
        {
            ...
            {
                if( IDYES == MessageBox( NULL, L"You are about to delete the packfile.  Content Streaming will need to recreate this file the next time it runs.", L"Warning", MB_YESNO ) )
                {
                    DeleteFile( strPath );
                }
            }
...
void	CALLBACK CreateVertexBuffer10_Serial( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext )
{
    ...
    CVertexBufferProcessor *pProcessor = new CVertexBufferProcessor( pDev, ppBuffer, &BufferDesc, pData, g_pResourceReuseCache );
...
void	CALLBACK CreateIndexBuffer10_Serial( ID3D10Device* pDev, ID3D10Buffer** ppBuffer, D3D10_BUFFER_DESC BufferDesc, void* pData, void* pContext )
{
    ...
    CIndexBufferProcessor *pProcessor = new CIndexBufferProcessor( pDev, ppBuffer, &BufferDesc, pData, g_pResourceReuseCache );
