...
void InitApp()
{
    ...
    iY = 60;
    WCHAR str[MAX_PATH];
    StringCchPrintf( str, MAX_PATH, L"Soldiers: %d", g_iInstances );
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( skinnedlayout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pSkinnedVertexLayout ) );

    // Define our vertex data layout for post-transformed objects
    const D3D10_INPUT_ELEMENT_DESC transformedlayout[] =
    {
        { "POS", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0,		0, D3D10_INPUT_PER_VERTEX_DATA, 0 },
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( transformedlayout, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pTransformedVertexLayout ) );
            ...
            for( UINT m=0; m<g_SkinnedMesh.GetNumMeshes(); m++ )
            {
                ...
                for( UINT p = 0; p < techDesc.Passes; ++p )
                {
                    for( UINT subset = 0; subset < g_SkinnedMesh.GetNumSubsets(m); subset++ )
                    {
                        ...
                        PrimType = g_SkinnedMesh.GetPrimitiveType10( (SDKMESH_PRIMITIVE_TYPE)pSubset->PrimitiveType );
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_STREAMOUT: g_bUseStreamOut = !g_bUseStreamOut; break;
        case IDC_FETCHTYPE:
            {
        ...
        case IDC_INSTANCES:
            {
                ...
                StringCchPrintf( str, MAX_PATH, L"Soldlers: %d", g_iInstances );
...
void SetBoneMatrices( FETCH_TYPE ft, UINT iMesh )
{
    ...
    case FT_CONSTANTBUFFER:
        ...
        {
            D3DXMATRIX* pMat = g_SkinnedMesh.GetMeshInfluenceMatrix( iMesh, i );
