...
enum ENCODING_MODE { FP16, FP32, RGB16, RGBE8, RGB32, RGB9E5, RG11B10, NUM_ENCODING_MODES };

enum RENDER_MODE { DECODED, RGB_ENCODED, ALPHA_ENCODED, NUM_RENDER_MODES };

enum NORMAL_MODE { NORM_RGB8, NORM_FP32, NORM_FP16, NORM_FPRG11B10, NORM_BC5S, NORM_BC5U, NUM_NORMAL_MODES };
...
HRESULT GetSampleOffsets_DownScale2x2_Lum( DWORD dwSrcWidth, DWORD dwSrcHeight, DWORD dwDestWidth, DWORD dwDestHeight, D3DXVECTOR2 avSampleOffsets[] );
...
HRESULT CreateEncodedTexture( IDirect3DCubeTexture9* pTexSrc, IDirect3DCubeTexture9** ppTexDest, ENCODING_MODE eTarget );
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    { 
        ...
        {
            case DECODED:       g_pEffect9->SetTechnique( g_pCurTechnique9->FinalPass ); break;
            case RGB_ENCODED:   g_pEffect9->SetTechnique( g_hFinalPassEncoded_RGB ); break;
            case ALPHA_ENCODED: g_pEffect9->SetTechnique( g_hFinalPassEncoded_A ); break;
        }
    ...
    for( UINT iFace=0; iFace < 6; iFace++ )
    {
        ...
        for( UINT y=0; y < desc.Height; y++ )
        {
            ...
            for( UINT x=0; x < desc.Width; x++ )
            {
                switch( eTarget )
                {
                    case RGBE8: EncodeRGBE8_D3D9( pSrc, &pDest ); break;
