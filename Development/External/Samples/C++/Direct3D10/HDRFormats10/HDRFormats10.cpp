...
enum ENCODING_MODE { FP16, FP32, RGB16, RGBE8, RGB9E5, RG11B10, NUM_ENCODING_MODES };
ENCODING_MODE g_EncodingModes[] = { FP16, FP32, RGB16, RGBE8, RGB9E5, RG11B10 };
WCHAR* g_szEncodingModes[] = { L"FP16", L"FP32", L"RGB16", L"RGBE8", L"RGB9E5", L"RG11B10" };
DXGI_FORMAT g_EncodingFormats[] = { DXGI_FORMAT_R16G16B16A16_FLOAT,
...
enum RENDER_MODE { DECODED, RGB_ENCODED, ALPHA_ENCODED, NUM_RENDER_MODES };
...
HRESULT GetSampleOffsets_Bloom_D3D10( DWORD dwD3DTexSize, float afTexCoordOffset[15], D3DXVECTOR4* avColorWeight, float fDeviation, float fMultiplier=1.0f );
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    for( int i=0; i<NUM_ENCODING_MODES; i++ )
    {
            ...
            SupportCaps & D3D10_FORMAT_SUPPORT_TEXTURE2D )
        {
            ...
            if( SUCCEEDED(hr) )
            {
                V_RETURN( g_aSkybox[g_EncodingModes[i]].OnD3D10CreateDevice( pd3dDevice, 50, pCubeTexture, pCubeRV, L"skybox10.fx" ) );
                g_SampleUI.GetComboBox(IDC_ENCODING_MODE)->AddItem( g_szEncodingModes[i], (void*)g_EncodingModes[i] );
    ...
    for( int i=0; i<NUM_NORMAL_MODES; i++ )
    {
        ...
        if( SupportCaps & D3D10_FORMAT_SUPPORT_TEXTURE2D )
        {
            ...
            hr = D3DX10CreateShaderResourceViewFromFile( pd3dDevice, strPath, &LoadInfo, NULL, &g_aTexNormalRV[i], NULL );
            if( SUCCEEDED(hr) )
            {
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( scenelayout, 4, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pSceneLayout ) );
...
HRESULT CALLBACK OnD3D10ResizedSwapChain( ID3D10Device* pd3dDevice, IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    {
        case FP16: fmt = DXGI_FORMAT_R16_FLOAT; break;
        case FP32: fmt = DXGI_FORMAT_R32_FLOAT; break;
        case RGB16: fmt = DXGI_FORMAT_R16_UNORM; break;
        case RGBE8: fmt = DXGI_FORMAT_R8G8_UNORM; break;
        case RGB9E5: fmt = DXGI_FORMAT_R16_FLOAT; break;	// Again, we can't use R9G9B9E5 render targets, so use FP16
        case RG11B10: fmt = DXGI_FORMAT_R11G11B10_FLOAT; break;
    }
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( g_bUseMultiSample )
    {
        ...
        pd3dDevice->ResolveSubresource( g_pTexRender10, D3D10CalcSubresource( 0, 0, 1 ), g_pMSRT10, D3D10CalcSubresource( 0, 0, 1 ), Desc.Format );
        ID3D10RenderTargetView* aRTViews[ 1 ] = { g_pTexRenderRTV10 };
        pd3dDevice->OMSetRenderTargets( 1, aRTViews, pOrigDSV );
    }
    ...
    {
        case DECODED:       pTech = g_pCurTechnique10->FinalPass; break;
        case RGB_ENCODED:   pTech = g_pFinalPassEncoded_RGB; break;
        case ALPHA_ENCODED: pTech = g_pFinalPassEncoded_A; break;
    }
...
HRESULT RetrieveD3D10TechHandles()
{
    ...
    CHAR* techniques[] = { "Scene", "SceneTwoComp", "DownScale2x2_Lum", "DownScale3x3", "DownScale3x3_BrightPass", "FinalPass" };
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_BLOOM:            g_bBloom = !g_bBloom; break;
...
HRESULT BrightPassFilter10()
{
    ...
    DrawFullScreenQuad10(pd3dDevice, g_pCurTechnique10->DownScale3x3_BrightPass, pBackBufDesc->Width/8, pBackBufDesc->Height/8);
...
//       or RGB32 encoding
//-----------------------------------------------------------------------------
HRESULT CreateEncodedTexture10( ID3D10Device* pd3dDevice, WCHAR* strPath, ENCODING_MODE eTarget, ID3D10Texture2D** ppTexOut, ID3D10ShaderResourceView** ppCubeRV )
{
    HRESULT hr = S_OK;
    ...
    for( UINT iFace=0; iFace < 6; iFace++ )
    {
        ...
        for( UINT y=0; y < desc.Height; y++ )
        {
            ...
            for( UINT x=0; x < desc.Width; x++ )
            {
                switch( eTarget )
                {
                    case RGB9E5: EncodeRGB9E5( pSrc, &pDest ); break;
        ...
        pd3dDevice->UpdateSubresource( (*ppTexOut), iSubResource, NULL, MappedFaceDest.pData, MappedFaceDest.RowPitch, 0 );
