...
void InitApp()
{
    ...
    WCHAR str[MAX_PATH];
    StringCchPrintf( str, MAX_PATH, L"Morphs to Apply: %d", g_iMorphsToApply );
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( meshlayout, sizeof(meshlayout)/sizeof(meshlayout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pMeshLayout ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( scenelayout, sizeof(scenelayout)/sizeof(scenelayout[0]), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pSceneLayout ) );
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    for( UINT i=1; i<g_iMorphsToApply; i++ )
    {
        ...
        fAmount *= 0.5;
        fAmount += 0.5f;
        g_MorphObject.ApplyMorph( pd3dDevice, i, fAmount, g_pRender2DQuad, g_ptxVertData, g_pfBlendAmt, g_pvMaxDeltas );
    }
