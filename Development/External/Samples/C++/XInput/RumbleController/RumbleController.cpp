        ...
        if( g_Controllers[i].dwResult == ERROR_SUCCESS )
        {
            if( !g_Controllers[i].bLockVibration )
            {
                ...
                if( g_Controllers[i].state.Gamepad.bLeftTrigger > 0)
                    g_Controllers[i].vibration.wLeftMotorSpeed = ((g_Controllers[i].state.Gamepad.bLeftTrigger+1)*256)-1;
                else
                ...
                if( g_Controllers[i].state.Gamepad.bRightTrigger > 0)
                    g_Controllers[i].vibration.wRightMotorSpeed = ((g_Controllers[i].state.Gamepad.bRightTrigger+1)*256)-1;
                else
            ...
            {
                if( !(!g_Controllers[i].bLockVibration && g_Controllers[i].vibration.wRightMotorSpeed == 0 && g_Controllers[i].vibration.wLeftMotorSpeed == 0) )
...
LRESULT WINAPI MsgProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    switch( msg )
    {
        ...
        case WM_PAINT:
        {
            ...
            rect.top = 20;
            rect.left = 20;
            DrawText( hDC, L"Use the controller's left/right trigger to adjust the speed of the left/right rumble motor.\n"
