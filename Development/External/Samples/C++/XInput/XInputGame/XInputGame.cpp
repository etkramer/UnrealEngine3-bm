                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_GameState.player[0].vPosition = D3DXVECTOR2(pBackBufferSurfaceDesc->Width/2.0f, pBackBufferSurfaceDesc->Height/2.0f);
    g_GameState.player[1].vPosition = D3DXVECTOR2(pBackBufferSurfaceDesc->Width/2.0f - fSizeInPixels*4, pBackBufferSurfaceDesc->Height/2.0f - fSizeInPixels*4);
    g_GameState.player[2].vPosition = D3DXVECTOR2(pBackBufferSurfaceDesc->Width/2.0f - fSizeInPixels*4, pBackBufferSurfaceDesc->Height/2.0f + fSizeInPixels*4);
    g_GameState.player[3].vPosition = D3DXVECTOR2(pBackBufferSurfaceDesc->Width/2.0f + fSizeInPixels*4, pBackBufferSurfaceDesc->Height/2.0f - fSizeInPixels*4);
...
void UpdateGame( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    for( int iAmmo=0; iAmmo<MAX_AMMO; iAmmo++ )
    {
        ...
        for( int iPlayer=0; iPlayer<DXUT_MAX_CONTROLLERS; iPlayer++ )
        {
            ...
            float fPlayerCenterX = g_GameState.player[iPlayer].vPosition.x+g_GameState.player[iPlayer].vSizeInPixels.x/2.0f;
            float fPlayerCenterY = g_GameState.player[iPlayer].vPosition.y+g_GameState.player[iPlayer].vSizeInPixels.y/2.0f;
        ...
        for( int iAsteroid=0; iAsteroid<MAX_ASTEROID; iAsteroid++ )
        {
            ...
            float fAsterCenterX = g_GameState.AsteroidQ[iAsteroid].vPosition.x+g_GameState.AsteroidQ[iAsteroid].vSizeInPixels.x/2.0f;
            float fAsterCenterY = g_GameState.AsteroidQ[iAsteroid].vPosition.y+g_GameState.AsteroidQ[iAsteroid].vSizeInPixels.y/2.0f;
    ...
    for( int iAsteroid=0; iAsteroid<MAX_ASTEROID; iAsteroid++ )
    {
        ...
        if( pAsteroid->bDead )
        {
            ...
            g_GameState.AsteroidQ[iAsteroid].nAnimationState = min( (int)((1.1f - pAsteroid->fDeathCountdown) / 0.1f) + 1, 5 );
    ...
    for( int iPlayer=0; iPlayer<DXUT_MAX_CONTROLLERS; iPlayer++ )
    {
        ...
        g_GameState.player[iPlayer].vPosition.x += g_GamePads[iUserIndex].fThumbLX * fElapsedTime * g_GameState.player[iPlayer].vSizeInPixels.x * 10.0f;
        g_GameState.player[iPlayer].vPosition.y += -g_GamePads[iUserIndex].fThumbLY * fElapsedTime * g_GameState.player[iPlayer].vSizeInPixels.y * 10.0f;

        // Keep player within map bounds
        float fMaxBoardY = (float) DXUTGetD3D9BackBufferSurfaceDesc()->Height - g_GameState.player[iPlayer].vSizeInPixels.y;
        float fMaxBoardX = (float) DXUTGetD3D9BackBufferSurfaceDesc()->Width - g_GameState.player[iPlayer].vSizeInPixels.x; 
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        if( g_GameState.gameMode == GAME_MAIN_MENU )
        {
            ...
            txtHelper.Begin();
            txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width / 2 - 256, DXUTGetD3D9BackBufferSurfaceDesc()->Height / 2 - 50 );
        ...
        else
        {
            ...
            RECT rcTexture;
            SetRect( &rcTexture, 0, 0, DXUTGetD3D9BackBufferSurfaceDesc()->Width, DXUTGetD3D9BackBufferSurfaceDesc()->Height );
            ...
            for( int i=0; i<MAX_AMMO; i++ )
            {
                ...
                {
                    float fScaleX = (float) g_GameState.AmmoQ[i].vSizeInPixels.x / RectWidth( g_GameState.AmmoQ[i].rcTexture );
                    float fScaleY = (float) g_GameState.AmmoQ[i].vSizeInPixels.y / RectHeight( g_GameState.AmmoQ[i].rcTexture );
                    D3DXMatrixScaling( &matTransform, fScaleX, fScaleY, 1.0f );
                    g_pSprite->SetTransform( &matTransform );
                    D3DXVECTOR3 vPos( g_GameState.AmmoQ[i].vPosition.x / fScaleX, g_GameState.AmmoQ[i].vPosition.y / fScaleY, 0.0f );
                    g_pSprite->Draw( g_pSpriteTexture, &g_GameState.AmmoQ[i].rcTexture, NULL, &vPos, g_GameState.AmmoQ[i].clr );
                }
            }
            ...
            for( int i=0; i<MAX_ASTEROID; i++ )
            {
                if( g_GameState.AsteroidQ[i].bActive )
                {
                    ...
                    D3DXMatrixScaling( &matTransform, fScaleX, fScaleY, 1.0f );
                    g_pSprite->SetTransform( &matTransform );
                    D3DXVECTOR3 vPos( g_GameState.AsteroidQ[i].vPosition.x / fScaleX, g_GameState.AsteroidQ[i].vPosition.y / fScaleY, 0.0f );
            ...
            for( int i=0; i<DXUT_MAX_CONTROLLERS; i++ )
            {
                ...
                {
                    float fScaleX = (float) g_GameState.player[i].vSizeInPixels.x / RectWidth( g_GameState.player[i].rcTexture );
                    float fScaleY = (float) g_GameState.player[i].vSizeInPixels.y / RectHeight( g_GameState.player[i].rcTexture );
                    D3DXMatrixScaling( &matTransform, fScaleX, fScaleY, 1.0f );
                    g_pSprite->SetTransform( &matTransform );
                    D3DXVECTOR3 vPos( g_GameState.player[i].vPosition.x / fScaleX, g_GameState.player[i].vPosition.y / fScaleY, 0.0f );
                    g_pSprite->Draw( g_pSpriteTexture, &g_GameState.player[i].rcTexture, NULL, &vPos, g_GameState.player[i].clr );
                }
            }
...
void RenderText()
{
    ...
    for( int i=0; i<DXUT_MAX_CONTROLLERS; i++ )
    {
       if( !g_GameState.player[i].bActive )
        ...
        {
            case 0: txtHelper.SetInsertionPos( 5, 5 ); break;
            case 1: txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width-230, 5 ); break;
            case 2: txtHelper.SetInsertionPos( 5, DXUTGetD3D9BackBufferSurfaceDesc()->Height-70 ); break;
            case 3: txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width-230, DXUTGetD3D9BackBufferSurfaceDesc()->Height-30 ); break; 
        }

        WCHAR strName[256];
        switch( i )
        {
            case 0: txtHelper.SetForegroundColor( D3DXCOLOR( 0.0f, 1.0f, 0.0f, 1.0f ) ); StringCchCopy( strName, 256, L"Green" ); break;
            case 1: txtHelper.SetForegroundColor( D3DXCOLOR( 0.0f, 0.0f, 1.0f, 1.0f ) ); StringCchCopy( strName, 256, L"Blue" ); break;
            case 2: txtHelper.SetForegroundColor( D3DXCOLOR( 0.8f, 0.0f, 0.8f, 1.0f ) ); StringCchCopy( strName, 256, L"Purple" ); break;
            case 3: txtHelper.SetForegroundColor( D3DXCOLOR( 1.0f, 0.5f, 0.0f, 1.0f ) ); StringCchCopy( strName, 256, L"Red" ); break;
        }
    ...
    if( g_GameState.gameMode == GAME_CONTROLLER_UNPLUGGED )
    {
        ...
        txtHelper.SetInsertionPos( DXUTGetD3D9BackBufferSurfaceDesc()->Width/2-512, DXUTGetD3D9BackBufferSurfaceDesc()->Height / 2 );
        txtHelper.DrawFormattedTextLine( L"Player %d's controller is unplugged.  Please reconnect and push Start", g_GameState.nUnpluggedPlayer+1 );
    }
    txtHelper.End();
