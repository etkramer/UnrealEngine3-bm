...
VOID UpdateUI( HWND hDlg )
{
    ...
    {
        StringCchCopy( strExcepted, 2048, TEXT("For security reasons, background exclusive ") \
    ...
    else
    {
        if( bForeground )
        {
            StringCchCopy( strExcepted, 2048, TEXT("Foreground cooperative level means that the ") \
        ...
        {
            StringCchCopy( strExcepted, 2048, TEXT("Background cooperative level really means ") \
        ...
        if( bExclusive )
        {
            StringCchCat( strExcepted, 2048, TEXT("Exclusive mode prevents other applications from ") \
        ...
        {
            StringCchCat( strExcepted, 2048, TEXT("Nonexclusive mode means that other applications ") \
                                 TEXT("can acquire device in exclusive or nonexclusive mode.\n\n"));
        }

        if( bImmediate )
        {
            StringCchCat( strExcepted, 2048, TEXT("Immediate data is a snapshot of the current ") \
        ...
        {
            StringCchCat( strExcepted, 2048, TEXT("Buffered data is a record of events that are stored ") \
        ...
        StringCchCat( strExcepted, 2048, TEXT("The sample will read the mouse 12 times a second. ") \
...
HRESULT ReadBufferedData( HWND hDlg )
{
    ...
    for( i = 0; i < dwElements; i++ ) 
    {
        ...
        {
            case DIMOFS_BUTTON0:
                StringCchCat( strNewText, 128, TEXT("B0") );
                break;

            case DIMOFS_BUTTON1:
                StringCchCat( strNewText, 128, TEXT("B1") );
                break;

            case DIMOFS_BUTTON2:
                StringCchCat( strNewText, 128, TEXT("B2") );
                break;

            case DIMOFS_BUTTON3:
                StringCchCat( strNewText, 128, TEXT("B3") );
                break;

            case DIMOFS_BUTTON4:
                StringCchCat( strNewText, 128, TEXT("B4") );
                break;

            case DIMOFS_BUTTON5:
                StringCchCat( strNewText, 128, TEXT("B5") );
                break;

            case DIMOFS_BUTTON6:
                StringCchCat( strNewText, 128, TEXT("B6") );
                break;

            case DIMOFS_BUTTON7:
                StringCchCat( strNewText, 128, TEXT("B7") );
                break;

            case DIMOFS_X:
                StringCchCat( strNewText, 128, TEXT("X") );
                break;

            case DIMOFS_Y:
                StringCchCat( strNewText, 128, TEXT("Y") );
                break;

            case DIMOFS_Z:
                StringCchCat( strNewText, 128, TEXT("Z") );
                break;

            default:
                StringCchCat( strNewText, 128, TEXT("") );
        }
        ...
        switch( didod[ i ].dwOfs )
        {
            ...
            case DIMOFS_Z:
            {
                ...
                StringCchPrintf( strCoordValue, 20, TEXT("%d "), didod[ i ].dwData );
                StringCchCat( strNewText, 128, strCoordValue );
                break;
            }
        }
    }
