...
VOID UpdateUI( HWND hDlg )
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                StringCchCat( strExcepted, 2048, TEXT("The Windows key will also be disabled so that ") \
                                     TEXT("users cannot inadvertently break out of the ") \
                                     TEXT("application. ") );
            }

            StringCchCat( strExcepted, 2048, TEXT("\n\n") );
        }
        ...
        StringCchCat( strExcepted, 2048, TEXT("The sample will read the keyboard 12 times a second. ") \
...
HRESULT ReadImmediateData( HWND hDlg )
{
    ...
    for( i = 0; i < 256; i++ ) 
    {
        if( diks[i] & 0x80 ) 
        {
            StringCchPrintf( strElement, 10, TEXT("0x%02x "), i );
            StringCchCat( strNewText, 1281, strElement );
        }
    }
...
HRESULT ReadBufferedData( HWND hDlg )
{
    ...
    for( i = 0; i < dwElements; i++ ) 
    {
        ...
        StringCchPrintf( strLetter, 10, TEXT("0x%02x%s "), didod[ i ].dwOfs,
                                         (didod[ i ].dwData & 0x80) ? TEXT("D") : TEXT("U"));
        StringCchCat( strNewText, 1281, strLetter );
    }
