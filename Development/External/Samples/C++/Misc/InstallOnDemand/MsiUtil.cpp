...
int CMsiUtil::GetProgress() const
{
    ...
    if( GetFeatureProgress() == 0 )
        ...
        nCurrent = GetFeatureProgressMax() ? (__int64)GetFeatureProgress() * COMBINED_FEATUREPROGTICK / GetFeatureProgressMax()
    ...
    switch( iMessageType )
    {
        case INSTALLMESSAGE_INFO:
            if( wcsstr( szMessage, L"InstallFinalize" ) )
            {
                ...
                if( !wcsncmp( szMessage, L"Action ended", 12 ) )
                    m_bInstallFinalize = false;
            }
            break;
                ...
                if( ParseProgressString( const_cast<LPWSTR>(szMessage), rgiField ) )
                {
                    ...
                    switch( rgiField[0] )
                    {
                        ...
                        case 2:
                        {
                            ...
                            // Movement direction determined by bForwardProgress set by reset progress msg
                            InterlockedExchangeAdd( (LONG*)&m_nProgress, m_bForwardProgress ? rgiField[1] : -rgiField[1] );
                            break;
                        }
    ...
    while( !pCls->m_bThreadShutdown )
    {
        ...
        if( QSize > 0 )
        {
            ...
            if( PRODUCT == Request.Type || INSTALLSTATE_ADVERTISED == FeatureState )
            {
                ...
                INSTALLUI_HANDLER OldHandler = MsiSetExternalUI( StaticInstallUiHandler, INSTALLLOGMODE_PROGRESS|INSTALLLOGMODE_INFO, pData );
...
HRESULT CMsiUtil::UseFeature( LPCWSTR szFeature, HANDLE hEvent, bool &bInstalled )
{
    ...
    if( INSTALLSTATE_LOCAL == State || INSTALLSTATE_SOURCE == State )
    {
        bInstalled = true;
    }
    else
