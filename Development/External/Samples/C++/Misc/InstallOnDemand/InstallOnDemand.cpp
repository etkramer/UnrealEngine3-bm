...
enum LOADSTATE { AppState_Normal,        // The app is rendering the scene.
                 ...
                 AppState_Stats };       // The app is displaying the end-of-level statistics.
...
INT WINAPI wWinMain( HINSTANCE, HINSTANCE, LPWSTR, int )
{
    ...
    {
        MessageBox( NULL, L"InstallOnDemand is unable to create resources necessary for its execution. The program will shut down.",
...
void InitApp()
{
    ...
    g_StatResult.GetControl( IDC_STAT_YOURKILL_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    ...
    g_StatResult.GetControl( IDC_STAT_TEAMKILL_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    ...
    g_StatResult.GetControl( IDC_STAT_ENEMYKILL_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    ...
    g_StatResult.GetControl( IDC_STAT_YOURRAZED_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    ...
    g_StatResult.GetControl( IDC_STAT_TEAMRAZED_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    g_StatResult.AddStatic( IDC_STAT_ENEMYRAZED, L"Structures Razed by enemy", 0, iY += 30, 300, 24 );
    g_StatResult.GetControl( IDC_STAT_ENEMYRAZED )->GetElement( 0 )->dwTextFormat = DT_LEFT|DT_VCENTER|DT_WORDBREAK;
    g_StatResult.AddStatic( IDC_STAT_ENEMYRAZED_VALUE, L"0", 0, iY, 300, 24 );
    g_StatResult.GetControl( IDC_STAT_ENEMYRAZED_VALUE )->GetElement( 0 )->dwTextFormat = DT_RIGHT|DT_VCENTER|DT_WORDBREAK;
    ...
    g_StatResult.GetControl( IDC_STAT_ENEMYRAZED_VALUE )->GetElement( 0 )->FontColor.States[DXUT_STATE_NORMAL] = D3DCOLOR_ARGB( 255, 96, 128, 255 );
...
void ComputeMeshScaling( ID3DXMesh *pMesh, D3DXMATRIX *pmScalingCenter, float fTargetRadius )
{
    ...
    if( SUCCEEDED( pMesh->LockVertexBuffer( 0, &pVerts ) ) )
    {
        ...
        if( SUCCEEDED( D3DXComputeBoundingBox( (const D3DXVECTOR3*)pVerts,
                                               ...
                                               &vMin, &vMax ) ) )
        {
            D3DXMatrixTranslation( pmScalingCenter, -vMin.x - ( vMax.x - vMin.x ) * 0.5f, -vMin.y - ( vMax.y - vMin.y ) * 0.5f, -vMin.z - ( vMax.z - vMin.z ) * 0.5f );
            D3DXMATRIXA16 m;
...
void RenderLoadingScreen( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    {
        RECT rc = { pdsd->Width / 2 - 150, pdsd->Height - 90,
        ...
        {
            const D3DSURFACE_DESC *pdsd = DXUTGetD3D9BackBufferSurfaceDesc();
            RECT rc = { pdsd->Width / 2 - 50, pdsd->Height - 25,
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( WAIT_OBJECT_0 == dwWait )
    {
        ...
        // If at normal state, resources will need to be loaded now.
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        if( AppState_Stats == g_AppState )
        {
            RenderLoadingScreenBackground( pd3dDevice );
        }
        else
        if( AppState_Normal == g_AppState )
        {
            if( -1 != g_nCurrentLevel )
            {
                // Render the skybox
        ...
        if( g_bShowHelp )
        {
            ...
            RECT rc = { pdsd->Width - 150, pdsd->Height - 25,
...
void RenderText()
{
    ...
    if( g_bShowHelp )
    {
        ...
        if( g_Msi.GetCompletedPassiveFeatureCount() < g_Msi.GetRequestedPassiveFeatureCount() )
            txtHelper.DrawFormattedTextLine( L"Passive:\nFeatures %d / %d",
                                             g_Msi.GetCompletedPassiveFeatureCount(), g_Msi.GetRequestedPassiveFeatureCount() );
        else
    ...
    if( g_bShowHelp )
    {
        ...
        txtHelper.DrawTextLine( L"Press F1 to hide control and helper information,\nwhich simulates the end-user experience." );
    }
    else
    {
    ...
    *pbNoFurtherProcessing = g_StatResult.MsgProc( hWnd, uMsg, wParam, lParam );
    ...
    switch( uMsg )
    {
        case WM_ACTIVATEAPP:
            ...
            // Workaround the problem that we can get multiple deactivate message
