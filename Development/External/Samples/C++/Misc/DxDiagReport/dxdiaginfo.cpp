...
HRESULT CDxDiagInfo::GetSoundInfo( vector<SoundInfo*>& vSoundInfos, vector<SoundCaptureInfo*>& vSoundCaptureInfos )
{
    ...
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    {
        ...
        if( FAILED( hr = GetStringValue( pObject, L"szDriverVersion", EXPAND(pSoundInfo->m_szDriverVersion) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverLanguageEnglish", EXPAND(pSoundInfo->m_szDriverLanguageEnglish) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverLanguageLocalized", EXPAND(pSoundInfo->m_szDriverLanguageLocalized) ) ) )
            goto LCleanup; nCurCount++;
        ...
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFlags", &pSoundInfo->m_dwFlags ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMinSecondarySampleRate", &pSoundInfo->m_dwMinSecondarySampleRate ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxSecondarySampleRate", &pSoundInfo->m_dwMaxSecondarySampleRate ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwPrimaryBuffers", &pSoundInfo->m_dwPrimaryBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxHwMixingAllBuffers", &pSoundInfo->m_dwMaxHwMixingAllBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxHwMixingStaticBuffers", &pSoundInfo->m_dwMaxHwMixingStaticBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxHwMixingStreamingBuffers", &pSoundInfo->m_dwMaxHwMixingStreamingBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHwMixingAllBuffers", &pSoundInfo->m_dwFreeHwMixingAllBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHwMixingStaticBuffers", &pSoundInfo->m_dwFreeHwMixingStaticBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHwMixingStreamingBuffers", &pSoundInfo->m_dwFreeHwMixingStreamingBuffers ) ) )
            goto LCleanup; nCurCount++;
        ...
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxHw3DStaticBuffers", &pSoundInfo->m_dwMaxHw3DStaticBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxHw3DStreamingBuffers", &pSoundInfo->m_dwMaxHw3DStreamingBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHw3DAllBuffers", &pSoundInfo->m_dwFreeHw3DAllBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHw3DStaticBuffers", &pSoundInfo->m_dwFreeHw3DStaticBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHw3DStreamingBuffers", &pSoundInfo->m_dwFreeHw3DStreamingBuffers ) ) )
            goto LCleanup; nCurCount++;
        ...
        if( FAILED( hr = GetUIntValue( pObject,  L"dwFreeHwMemBytes", &pSoundInfo->m_dwFreeHwMemBytes ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwMaxContigFreeHwMemBytes", &pSoundInfo->m_dwMaxContigFreeHwMemBytes ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwUnlockTransferRateHwBuffers", &pSoundInfo->m_dwUnlockTransferRateHwBuffers ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetUIntValue( pObject,  L"dwPlayCpuOverheadSwBuffers", &pSoundInfo->m_dwPlayCpuOverheadSwBuffers ) ) )
            goto LCleanup; nCurCount++;
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectSound.DxDiag_SoundCaptureDevices", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    ...
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    {
        ...
        if( FAILED( hr = GetStringValue( pObject, L"szDriverVersion", EXPAND(pSoundCaptureInfo->m_szDriverVersion) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverLanguageEnglish", EXPAND(pSoundCaptureInfo->m_szDriverLanguageEnglish) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverLanguageLocalized", EXPAND(pSoundCaptureInfo->m_szDriverLanguageLocalized) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverAttributes", EXPAND(pSoundCaptureInfo->m_szDriverAttributes) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverDateEnglish", EXPAND(pSoundCaptureInfo->m_szDriverDateEnglish) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szDriverDateLocalized", EXPAND(pSoundCaptureInfo->m_szDriverDateLocalized) ) ) )
            goto LCleanup; nCurCount++;
        ...
        if( FAILED( hr = GetBoolValue( pObject, L"bDefaultSoundRecording", &pSoundCaptureInfo->m_bDefaultSoundRecording ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetBoolValue( pObject, L"bDefaultVoiceRecording", &pSoundCaptureInfo->m_bDefaultVoiceRecording ) ) )
            goto LCleanup; nCurCount++;
...
HRESULT CDxDiagInfo::GetInputInfo( InputInfo** ppInputInfo )
{
    ...
    *ppInputInfo = pInputInfo;
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectInput.DxDiag_DirectInputDevices", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    ...
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    {
        ...
        if( FAILED( hr = GetStringValue( pObject, L"szFFDriverName", EXPAND(pInputDevice->m_szFFDriverName) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szFFDriverDateEnglish", EXPAND(pInputDevice->m_szFFDriverDateEnglish) ) ) )
            goto LCleanup; nCurCount++;
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectInput.DxDiag_DirectInputGameports", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectInput.DxDiag_DirectInputUSBRoot", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectInput.DxDiag_DirectInputPS2Devices", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
...
//-----------------------------------------------------------------------------
HRESULT CDxDiagInfo::GatherInputRelatedDeviceInst( InputRelatedDeviceInfo* pInputRelatedDevice, IDxDiagContainer* pContainer )
{
    HRESULT            hr;
    WCHAR              wszContainer[256];
    IDxDiagContainer*  pChild            = NULL;
    ...
    if( FAILED( hr = GetStringValue( pContainer, L"szLocation", EXPAND(pInputRelatedDevice->m_szLocation) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pContainer, L"szMatchingDeviceId", EXPAND(pInputRelatedDevice->m_szMatchingDeviceId) ) ) )
        goto LCleanup; nCurCount++;
...
//-----------------------------------------------------------------------------
HRESULT CDxDiagInfo::GatherInputRelatedDeviceInstDrivers( InputRelatedDeviceInfo* pInputRelatedDevice, IDxDiagContainer* pChild )
{
    HRESULT            hr;
    WCHAR              wszContainer[256];
...
HRESULT CDxDiagInfo::GetNetworkInfo( NetInfo** ppNetInfo )
{
    ...
    *ppNetInfo = pNetInfo;
    ...
    if( FAILED( hr = GetStringValue( pObject, L"szTestResultEnglish", EXPAND(pNetInfo->m_szTestResultEnglish) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardFullDuplexTestLocalized", EXPAND(m_pNetInfo->m_szVoiceWizardFullDuplexTestLocalized) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardHalfDuplexTestLocalized", EXPAND(m_pNetInfo->m_szVoiceWizardHalfDuplexTestLocalized) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardMicTestLocalized", EXPAND(m_pNetInfo->m_szVoiceWizardMicTestLocalized) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardFullDuplexTestEnglish", EXPAND(m_pNetInfo->m_szVoiceWizardFullDuplexTestEnglish) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardHalfDuplexTestEnglish", EXPAND(m_pNetInfo->m_szVoiceWizardHalfDuplexTestEnglish) ) ) )
        goto LCleanup; nCurCount++;
    if( FAILED( hr = GetStringValue( pObject, L"szVoiceWizardMicTestEnglish", EXPAND(m_pNetInfo->m_szVoiceWizardMicTestEnglish) ) ) )
        goto LCleanup; nCurCount++;
    ...
    for( nItem = 0; nItem < nInstanceCount; nItem++ )
    {
        ...
        if( FAILED( hr = GetStringValue( pObject, L"szLauncherPath", EXPAND(pNetApp->m_szLauncherPath) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szLauncherVersionLocalized", EXPAND(pNetApp->m_szLauncherVersionLocalized) ) ) )
            goto LCleanup; nCurCount++;
        if( FAILED( hr = GetStringValue( pObject, L"szLauncherVersionEnglish", EXPAND(pNetApp->m_szLauncherVersionEnglish) ) ) )
            goto LCleanup; nCurCount++;
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectPlay.DxDiag_DirectPlayAdapters", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
    ...
    if( FAILED( hr = m_pDxDiagRoot->GetChildContainer( L"DxDiag_DirectPlay.DxDiag_DirectPlayVoiceCodecs", &pContainer ) ) )
        goto LCleanup;
    if( FAILED( hr = pContainer->GetNumberOfChildContainers( &nInstanceCount ) ) )
        goto LCleanup;
...
VOID CDxDiagInfo::DestroyInputInfo(InputInfo* pInputInfo)
{
    if( pInputInfo )
    {
        ...
        for( iter = pInputInfo->m_vDirectInputDevices.begin(); iter != pInputInfo->m_vDirectInputDevices.end(); iter++ )
        {
...
VOID CDxDiagInfo::DestroyNetworkInfo(NetInfo* pNetInfo)
{
    if( pNetInfo )
    {
        ...
        for( iterNetAdapter = pNetInfo->m_vNetAdapters.begin(); iterNetAdapter != pNetInfo->m_vNetAdapters.end(); iterNetAdapter++ )
        {
        ...
        for( iterNetCodec = pNetInfo->m_vNetVoiceCodecs.begin(); iterNetCodec != pNetInfo->m_vNetVoiceCodecs.end(); iterNetCodec++ )
        {
