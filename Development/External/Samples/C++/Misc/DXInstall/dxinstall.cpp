...
VOID DirectXGetVersion( HWND hWnd )
{
    ...
    {
        MessageBox( hWnd, L"dsetup.dll could not be found.  In order to use the DirectSetup API, copy the entire contents of the \\Redist folder from the DirectX SDK into the same folder as this sample.  See the documenation and source for further details.", L"Error", MB_OK|MB_ICONINFORMATION );
        return;
    }
